...
// tree (sorted lists throughout); only the header carries the date + commit.
...
function renderHeader(root) {
  ...
  } catch { /* not a git checkout — the body is what matters */ }
  return [
    '<!-- GENERATED by scripts/gen-codemap.mjs — DO NOT EDIT BY HAND. -->',
    '# mezo — Codebase Map',
    '',
    `> **Where things live.** Generated ${new Date().toISOString().slice(0, 10)} from commit \`${commit}\`.`,
    '> Regenerate: `node scripts/gen-codemap.mjs` · freshness gate: `node scripts/gen-codemap.mjs --check`.',
    '',
    BODY_MARKER,
    '',
  ].join('\n');
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(new URL(import.meta.url).pathname)) {
  ...
  if (process.argv.includes('--check')) {
    ...
    writeFileSync(outPath, renderHeader(REPO_ROOT) + body);
