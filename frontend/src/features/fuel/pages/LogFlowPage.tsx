...
interface DraftLine {
   ...
   *  Ported verbatim from the retired LogMealSheet (mezo-d20.9.1) so the ingredient
   *  fine-tuning block survives the unified flow. */
...
export function LogFlowPage({ initialSlot, prefill, aiPanelOpenOnMount, onClose }: LogFlowPageProps) {
  ...
  // filled AFTER the render that attached the photo and the thumbnail would never paint — the
  // preview AiLogSheet always had. State schedules the second render that shows it.
  ...
  // --- recipe ingredient overrides (mezo-ormb, ported from the retired LogMealSheet) --------
