...
//   · any up-move    → (U4: the 3D `t-up`; was the clay `i-growth`) + "Túlterhelés" + every lever spelled out, both
...
export function WorkoutOverloadLine({ overload }: { overload?: OverloadSummary | null }) {
  ...
  return (
    <div className="wo-overload wos-ovl">
      {/* Üvegesítés U4 (mezo-me75u.4): a flat cell led by the 3D direction mark — t-up for a
          day that climbs (i-growth means "overload" HERE, growth on Én, so it maps at this
          call site), t-down for a back-off-only day. */}
      <Icon3D name={ups > 0 ? 't-up' : 't-down'} size={32} className="wo-overload-icon" />
      <span className="wos-ovl-copy">
