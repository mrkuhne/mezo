...
// DS-migrated (mezo-setx.6.4): the DS ListItem row spec — 56px min height,
// 16/600 title, 14px meta line — and the Napiv notched corner (a clipPath
// polygon) gives way to the radius scale, which the DS treats as the one
// sanctioned corner vocabulary.
// ============================================================
...
export function GymDayCard({ day, onOpen }: GymDayCardProps) {
  ...
  return (
    ...
    >
      <div className="row" style={{ minHeight: 56, padding: '12px 16px', alignItems: 'center', gap: 12 }}>
        <div className="col" style={{ width: 40 }}>
          ...
          {isToday && <span className="label-mono mt-xs">MA</span>}
        </div>
        ...
        <div className="col flex-1" style={{ minWidth: 0 }}>
          <div className="row gap-xs" style={{ alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {/* A descriptor stamp, not a selectable filter → the DS Tag spec. */}
            {day.muscleAccent && (
              <span className="excat-tag" style={{ background: 'var(--wash-gym)', color: 'var(--tag-gym)' }}>
                focus
              </span>
            )}
          </div>
          {!isRest && (
            <div className="row gap-md mt-xs" style={{ fontVariantNumeric: 'tabular-nums', fontSize: 14 }}>
          ...
          {isRest && day.note && (
            <span className="text-meta-sm text-tertiary mt-xs">{day.note}</span>
          )}
        </div>

        {!isRest && (
          <Icon name="chevron-right" size={16} color={isToday ? 'var(--tag-gym)' : 'var(--text-tertiary)'} />
        )}
      </div>
    </button>
  )
