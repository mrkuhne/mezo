...
export function GoalSettingsPage() {
  ...
  if (goalError || overviewError) return <MozaikPage tone="rose" className="goal-detail-page goal-detail-settings-page uv-goal"><PageHead onBack={() => navigate('/settings/me', { state: origin.state })} label="Én beállításai" glass /><PageBody><p role="alert">A súlycél nem tölthető be. Próbáld újra később.</p></PageBody></MozaikPage>
  if (!goalPending && !goal) return <MozaikPage tone="rose" className="goal-detail-page goal-detail-settings-page uv-goal"><PageHead onBack={() => navigate('/settings/me', { state: origin.state })} label="Én beállításai" glass /><PageBody><PageHero art="t-weight" accent="var(--dv-rose)" name="Súlycél" big="Még nincs aktív cél" /><button className="cta-primary" onClick={() => navigate('/me/goals/weight')}>Cél beállítása</button></PageBody></MozaikPage>
  ...
  return <MozaikPage tone="rose" className="goal-detail-page goal-detail-settings-page uv-goal">
    ...
    {loading ? <div className="goal-detail-loading" role="status" aria-label="Betöltés…"><span /><span /><span /></div> : !overview || invalid ? (
      ...
      <PageBody principle="Az archiválás és törlés a szerkesztőn belül, másodlagos művelet marad.">
        ...
        <section className="goal-settings-grid glass rise">
        ...
        <div className="goal-settings-guards rise">
          <span className={goalResponse?.guards.includes('strength') ? 'is-on' : ''}><Icon3D name="t-shield" size={16} />Erővédelem</span>
          <span className={goalResponse?.guards.includes('muscle') ? 'is-on' : ''}><Icon3D name="t-shield" size={16} />Izomvédelem</span>
        </div>
