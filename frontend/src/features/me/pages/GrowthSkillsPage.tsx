    ...
    <MozaikPage tone="lav" className="grs-page">
      ...
      <PageBody principle="A szint visszajelzés, nem jutalom — semmi nem nyílik vagy zárul tőle. Az XP-idősort nem rajzoljuk: a contract nem hordoz sorozatot.">
        <EntranceGroup>
          ...
          {life.length > 0 && (
            <SkillBandCard delayMs={60} wash="lav" chipTone="lav" eyebrow="LIFE" art="t-compass" chip={`${life.length} skill · ${huInt(s.lifeXp)} XP`}
              rows={toRows(life, (k, n) => { const m = lifeMeta(k); return m ? <ContentIcon name={m.clayIcon} size={34} /> : initials(n) }, (k) => lifeMeta(k)?.name ?? k)}
              footer={typeof savings === 'number' && savings > 0 ? <><span>Megtakarítás (30 nap)</span><b>{huInt(savings)} Ft</b></> : undefined}
              goalChips={chips} />
          )}
          {athletic.length > 0 && (
            <SkillBandCard delayMs={120} wash="sage" chipTone="ok" eyebrow="Atlétikus" art="t-run"
          ...
          {muscle.length > 0 && (
            <SkillBandCard delayMs={180} wash="amber" chipTone="warn" eyebrow="Izom" art="t-muscle"
