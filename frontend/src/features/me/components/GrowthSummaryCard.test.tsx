...
describe('GrowthSummaryCard', () => {
  it('renders the total-XP + streak chip, the top-3 skills (highest level first) and the stats line', () => {
    const { container } = renderCard(progressionProfileMock)
    // total XP chip = Σ cumulativeXp across athletic + muscle + life (9350 + 8550 + 1085),
    // now carrying the streak text too (streak left the stats row for the chip).
    expect(screen.getByText(/18 985 XP · 5 hét sorozat/)).toBeInTheDocument()
    ...
    // .skl progress bars: one <i> per top-3 row, width driven by SkillLevel.progressPct.
    const bars = container.querySelectorAll('.skl .bar i')
    expect(bars).toHaveLength(3)
    expect(bars[0]).toHaveStyle({ width: '30%' }) // max_strength progressPct
    // stats footer meta row: athlete level + discipline (streak moved into the chip)
    expect(screen.getByText('4.3')).toBeInTheDocument()
    expect(screen.getByText('78%')).toBeInTheDocument()
    // 30-day savings stays in the footer meta row (hu-HU grouped Ft)
