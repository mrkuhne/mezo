...
export function TrainSettingsPage({ editor }: { editor?: 'gym' | 'sport' }) {
  ...
  return <SettingsFrame title={editor === 'gym' ? 'Gym-időpontok' : editor === 'sport' ? 'Rendszeres sport' : 'Train'} subtitle="Helyet a mozgásnak. A hét többi része igazodik." domain="train" parent={editor ? '/settings/train' : '/settings'}>
    <section className="settings-wash glass"><p className="settings-editorial">A terved mondja, mit.<br />Te döntöd el, mikor.</p>{train.schedulePending ? <p role="status">Az időpontjaid betöltése…</p> : train.scheduleError ? <p role="alert">Nem sikerült betölteni az időpontokat. Frissítsd az oldalt az újrapróbáláshoz.</p> : <><div className="settings-stats"><div><strong>{train.gymSlots.length}</strong><small>gym-időpont / hét</small></div><div><strong>{sessions.length}</strong><small>sportalkalom / hét</small></div></div><div className="settings-week" aria-label="Heti gym-időpontok">{['H', 'K', 'Sze', 'Cs', 'P', 'Szo', 'V'].map((day, i) => { const time = train.gymSlots.find(s => s.dayOfWeek === i)?.time; return <div className={time ? 'settings-day is-on' : 'settings-day'} key={day}>{day}<strong>{time ?? '—'}</strong></div> })}</div></>}</section>
    ...
    <SettingsRows domain="train">
      <SettingsRow to="/settings/train/gym" title="Heti gym-időpontok" description="Melyik nap, mikor kezdődjön az edzésed" icon="t-dumbbell" domain="train" />
      <SettingsRow to="/settings/train/sport" title="Rendszeres sport" description="Röplabda, cross és TRX · akár több alkalom egy napon" icon="t-volley" domain="train" />
      <SettingsRow to="/settings/notifications" title="Edzésemlékeztetők" description="Jelzés a kezdés előtt, a saját időpontjaidhoz igazítva" icon="t-bell" domain="nap" />
    </SettingsRows>
