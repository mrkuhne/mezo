...
export function SettingsPage() {
  ...
  return <SettingsFrame title="Legyen a tiéd." subtitle="Egy helyen minden, ami hozzád igazítja a Mezót." parent={null}>
    {current && <Link className={`settings-current glass settings-${current.id}`} to={`/settings/${current.id}`} state={origin.state}><Boop domain={current.id} size={48} /><div><span className="settings-kicker">Innen érkeztél</span><strong>{current.title}</strong><p>{current.detail}</p></div><span aria-hidden="true">↗</span></Link>}
    <h2 className="settings-section-label">A TE TERÜLETEID</h2>
    <div className="settings-domain-grid">{domains.map(d => <Link key={d.id} className={`settings-domain glass settings-${d.id}`} to={`/settings/${d.id}`} state={origin.state}><Boop domain={d.id} size={54} /><strong>{d.title}</strong><p>{d.description}</p><small>{d.detail}</small><span className="settings-domain-arrow" aria-hidden="true">↗</span></Link>)}</div>
    <h2 className="settings-section-label">AZ APP KÖRÜLÖTTED</h2>
    <SettingsRows domain="me">
      <SettingsRow to="/settings/notifications" title="Értesítések" description="Mikor és miről szóljon Mezo" icon="t-bell" domain="nap" />
      <SettingsRow to="/settings/general" title="Megjelenés és alkalmazás" description="Téma, kalauzok és fiókbeállítások" icon="t-palette" domain="me" />
      <SettingsRow to="/settings/account" title="Fiókod" description="Név, e-mail és belépés" icon="t-key" domain="nap" />
    </SettingsRows>
