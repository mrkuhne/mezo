  ...
  return <MozaikPage tone="lav">
    ...
    <PageBody><EntranceGroup className="col gap-md">
      ...
      {journal.isPending ? <GhostState message="A napi emlékek betöltése…" />
        ...
        : journal.summaries.map((day) => <Link key={day.date} to={`/mezo/emlekek/${day.date}`} className="mem-daycard rise" style={{ color: 'inherit', textDecoration: 'none' }}>
      ...
      <MemorySearchPanel onPick={(date) => navigate(`/mezo/emlekek/${date}`)} />
