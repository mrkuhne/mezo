...
function toView(pref: components['schemas']['NotificationPref']): NotificationPrefView {
  // The wire `category` is a plain string; the backend guarantees it is always one of the 11
  // known keys ("All 11 categories, always complete" — GET /api/notification/pref), so this
 ...
 * `notificationPrefSeed` (all 11, spec defaults) synchronously and never touches the network;
