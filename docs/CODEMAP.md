...
> **Where things live.** Generated 2026-08-21 from commit `a67f6c78`.
...
| [companion](#companion) | ✓ | 1 | · | · | [companion](features/companion.md), [journal](features/journal.md) |
...
*BE + API + FE-data* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-19, done)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/activity`
  ...
  - **services:** `ActivityClassifier`, `ActivityLedgerAdapter`, `ActivityService`, `DailyActivityAdapter`
...
*BE + API* · read next: [docs/features/me.md](features/me.md) (updated 2026-08-21, mixed) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-19, mixed)
...
*BE + API* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-21, mixed) ·
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  - **sub-features:** `advisor`, `embedding`, `llm`, `tools`
  - **entities→tables:** `AiConversationEntity`→`ai_conversation`, `AiMessageEntity`→`ai_message`,
    `DailySummaryEntity`→`daily_summary`, `KnowledgeFactEntity`→`knowledge_fact`, `LearnedFactEntity`→`learned_fact`,
    `MemoryEmbeddingEntity`→`memory_embedding`, `PatternEntity`→`pattern`, `PatternEventEntity`→`pattern_event`
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `DailySummaryRepository`,
    `KnowledgeFactRepository`, `LearnedFactRepository`, `MemoryEmbeddingRepository`, `PatternEventRepository`,
    `PatternRepository`
  - **services:** `ChatService`, `ChatStreamService`, `ChatTurnCompleted`, `ContextSnapshotAssembler`,
    ...
    `FactCandidateService`, `FactExtractionListener`, `FactExtractionService`, `HypothesisJob`,
    `HypothesisPipelineService`, `KnowledgeFactService`, `MemoryObservatoryService`, `MemoryRecallService`,
    `MesoContextAssembler`, `MesoReviewGenerator`, `MesoReviewListener`, `MetricDomain`, `MetricKey`,
  ...
  - **controllers→contract:** `CompanionController`→`CompanionApi`, `CompanionStreamController`,
    `CompanionVoiceController`→`CompanionVoiceApi`
  - **mappers:** `CompanionMapper`
  - **config:** `CompanionProperties`
  - **events/listeners:** `DecisionEmbeddingListener`, `JournalEmbeddingListener`, `TurnEmbeddingListener`
  - **other:** `AdvisedAnswer`, `AdvisorRetry`, `AdvisorViolation`, `BiometricsTools`, `ChatHistory`,
    ...
    `EmbeddingPort`, `FakeCompanionLlm`, `FakeEmbeddingAdapter`, `FuelTools`, `GeminiCompanionLlm`,
    `GeminiEmbeddingAdapter`, `GeminiRoundUsage`, `GeminiRoundUsageAdvisor`, `GeminiUsageExtractor`, `GoalTools`,
    `GrowthTools`, `HabitSuggestLlmAdapter`, `InsightsTools`, `MealCoachLlmAdapter`, `MealDraftLlmAdapter`,
    `MedicationTools`, `MemoryEmbeddingWriter`, `MemoryTools`, `PantryPhotoLlmAdapter`, `PantryScrapeLlmAdapter`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 57 IT + 11 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `ChatExtractionFlowIT`, `ChatExtractionSwitchOffIT`, `ChatServiceIT`,
    ...
    `FactExtractionServiceIT`, `FakeEmbeddingAdapterIT`, `HypothesisGatherContextIT`, `HypothesisJobSwitchOffIT`,
    `HypothesisPipelineServiceIT`, `KnowledgeFactServiceIT`, `LearnedFactPersistenceIT`, `MemoryEmbeddingRepositoryIT`,
    `MemoryEmbeddingWriterIT`, `MemoryRecallServiceIT`, `MemoryToolsRenderIT`, `MesoReviewGeneratorIT`,
    `MetricSeriesDerivedIT`, `MetricSeriesExpansionIT`, `MetricSeriesServiceIT`, `PatternDetectionJobSwitchOffIT`,
    `PatternDetectionServiceIT`, `ToolSelectionEvalIT`, `TurnEmbeddingListenerIT`, `TurnEmbeddingSwitchOffIT`,
    `TurnVerdictCheckIT`
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `BiometricProfilePopulator`,
    `CheckInPopulator`, `DailySummaryPopulator`, `DatabasePopulator`, `GamificationPopulator`, `GoalPlanLinkPopulator`,
    `GoalPopulator`, `HabitPopulator`, `IntentionPopulator`, `JournalPopulator`, `KnowledgeFactPopulator`,
    `LearnedFactPopulator`, `LevelUpEventPopulator`, `LlmLogPopulator`, `MealPopulator`, `MedicationDosePopulator`,
    `MedicationPopulator`, `MemoryEmbeddingPopulator`, `MentionPopulator`, `PantryItemPopulator`,
    ...
    `WaterLogPopulator`, `WeightLogPopulator`
...
*BE + API + FE-data* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-08-14, done)
...
*FE-data + FE-ui* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-21, mixed) ·
  [docs/features/insights.md](features/insights.md) (updated 2026-08-18, mixed)
...
- **FE ui** `frontend/src/features/insights`
  ...
  - **components:** ChatMessage.tsx, FactCandidateCard.tsx, GrowthWeekCard.tsx, KnowledgeExplainer.tsx,
    KnowledgeFactRow.tsx, LifecycleSection.tsx, MemoryAuditPanel.tsx, MemoryJournalPanel.tsx, MemoryLayerCard.tsx,
    MemoryLayersPanel.tsx, MemorySearchPanel.tsx, MetricCoverageRing.tsx, MotorStateHero.tsx, PatternDecisionCard.tsx,
    PatternImpactCard.tsx, PatternJournal.tsx, PatternScatter.tsx, PatternStrengthChart.tsx, SimilarDayCard.tsx,
    TokenColumns.tsx
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/journal`
  - **entities→tables:** `DecisionEntryEntity`→`decision_entry`, `JournalEntryEntity`→`journal_entry`
  - **repositories:** `DecisionEntryRepository`, `JournalEntryRepository`
  - **services:** `DecisionContextPort`, `DecisionEntrySavedEvent`, `DecisionService`, `JournalEntryDeletedEvent`,
    `JournalEntrySavedEvent`, `JournalService`
  - **controllers→contract:** `JournalController`→`JournalApi`
  - **mappers:** `DecisionMapper`, `JournalMapper`
  - **config:** `JournalProperties`
  - **other:** `DecisionContextEnvelope`
- **Contract** `api/feature/journal/journal.yml` — 7 operations
...
- **FE data** `frontend/src/data/journal`
  - **hooks (via `@/data/hooks`):** `isDecisionDue`, `useDecisionActions`, `useDecisions`, `useJournalActions`,
    `useJournalNotes`
  - **modules:** decisionApi.ts, decisionHooks.ts, decisionMock.ts, decisionTypes.ts, journalApi.ts, journalHooks.ts,
    journalMock.ts, journalTypes.ts
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/journal` — 9 IT + 0 unit
  - **ITs:** `DecisionApiCompanionOffIT`, `DecisionApiIT`, `DecisionEmbeddingEventIT`, `DecisionEntryPersistenceIT`,
    `JournalApiCompanionOffIT`, `JournalApiIT`, `JournalEmbeddingEventIT`, `JournalEntryPersistenceIT`,
    `JournalSwitchOffIT`
...
*FE-data + FE-ui* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-19, done) ·
  [docs/features/habit.md](features/habit.md) (updated 2026-08-14, done) ·
  [docs/features/journal.md](features/journal.md) (updated 2026-08-21, done) ·
  [docs/features/me.md](features/me.md) (updated 2026-08-21, mixed) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-19, mixed) ·
  [docs/features/_platform-data-layer.md](features/_platform-data-layer.md) (updated 2026-08-21, done) ·
...
- **FE ui** `frontend/src/features/me`
  ...
  - **components:** AiCallFilters.tsx, AiCallRow.tsx, AiCallUsage.tsx, AiFeatureBreakdown.tsx, AiModelBreakdown.tsx,
    ...
    GoalPlanSlots.tsx, GoalRecept.tsx, GoalStat.tsx, GoalTimeline.tsx, GrowthJournalCard.tsx, GrowthSummaryCard.tsx,
    KnowledgeFactCard.tsx, MeBioRow.tsx, MentionRow.tsx, NightArcCard.tsx, NightBodyScan.tsx, NightBreathing.tsx,
    NightWalk.tsx, NotificationCategoryRow.tsx, NotificationPreviewHeader.tsx, PerksCard.tsx, PersonCard.tsx,
    PhaseAverageCard.tsx, PhaseRail.tsx, PhaseReferenceRow.tsx, PushInstallGate.tsx, RemDurationCard.tsx,
    RoutinesTab.tsx, SkillBandCard.tsx, SleepChart.tsx, SleepEscalationCard.tsx, SleepLogRow.tsx, SleepStat.tsx,
    SleepStatCard.tsx, TimePicker.tsx, WeeklyWeightCard.tsx, WeightHero.tsx, WeightTrendChart.tsx
  - **logic:** biometricFields.ts, buildTdeeBreakdown.ts, goalLabels.ts, growthJournal.ts, habitMetricPalette.ts,
    llmCallFormat.ts, nightContent.ts, nightFlow.ts, nightTrace.ts, notificationForecast.ts, sleepEducation.ts,
    sleepEscalation.ts, sleepPhases.ts, sleepStats.ts, weightStats.ts
...
*BE + API + FE-data* · read next: [docs/features/needs.md](features/needs.md) (updated 2026-08-18, done)
...
*BE + API* · read next: [docs/features/proactive.md](features/proactive.md) (updated 2026-08-18, complete)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/proactive`
  ...
  - **services:** `ChallengeGenerator`, `ChallengeJob`, `ChallengeOutcomeEvaluator`, `CompanionMessageEventListener`,
    ...
    `ExperimentProposalGenerator`, `GrowthDigestBlock`, `MemoirGenerator`, `MemoirJob`, `MetricWindowEvaluator`,
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-19, done)
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/ritual.md](features/ritual.md) (updated 2026-08-19, done)
...
- **Contract** `api/feature/ritual/ritual.yml` — 2 operations
  - **endpoints:** GET /api/ritual/day/{date} · POST /api/ritual/close
...
- **FE ui** `frontend/src/features/ritual`
  ...
  - **components:** ArrivalStep.tsx, DayStoryStep.tsx, HarvestStep.tsx, LoopsStep.tsx, ReleaseStep.tsx
  - **logic:** harvestStages.ts, openLoops.ts, ritualWindow.ts
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/ritual` — 3 IT + 0 unit
  - **ITs:** `RitualApiIT`, `RitualDayEntityIT`, `RitualSwitchOffIT`
  - **populators:** `RitualPopulator`, `SleepGoalPopulator`, `UserPopulator`

### today

*FE-data + FE-ui* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-08-14, done) ·
  [docs/features/intention.md](features/intention.md) (updated 2026-08-11, done) ·
  [docs/features/needs.md](features/needs.md) (updated 2026-08-18, done) ·
  [docs/features/ritual.md](features/ritual.md) (updated 2026-08-19, done) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-19, mixed)
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `CompanionMessagePopulator`, `DailySummaryPopulator`, `ExperimentPopulator`, `FuelSettingsPopulator`,
  `GamificationPopulator`, `GoalPlanLinkPopulator`, `GoalPopulator`, `HabitPopulator`, `IntentionPopulator`,
  `JournalPopulator`, `KnowledgeFactPopulator`, `LearnedFactPopulator`, `LevelUpEventPopulator`, `LlmLogPopulator`,
  ...
  `PantryImportPopulator`, `PantryItemPopulator`, `PatternEventPopulator`, `PatternPopulator`, `PersonPopulator`,
  `PredictionPopulator`, `ProtocolPopulator`, `QuestPopulator`, `RecipePopulator`, `RitualPopulator`,
  `RunningPopulator`, `SkillProgressPopulator`, `SleepGoalPopulator`, `SleepLogPopulator`, `SupplementIntakePopulator`,
  `TrainPopulator`, `UserPopulator`, `WaterLogPopulator`, `WeeklySuggestionPopulator`, `WeightLogPopulator`
- **`ResetDatabase` TRUNCATE list** — 75 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `exercise`, `exercise_feedback`, `exercise_set`, `experiment`, `fuel_settings`, `gamification_profile`, `goal`,
    `goal_plan_link`, `gym_schedule_slot`, `habit_chain`, `habit_day`, `habit_def`, `intention_creed`,
    `intention_focus`, `journal_entry`, `knowledge_fact`, `learned_fact`, `level_up_event`, `llm_log_history`, `meal`,
    `meal_item`, `meal_slot_template`, `medication`, `medication_dose`, `memoir`, `memory_embedding`, `mention`,
    `meso_template`, `mesocycle`, `mesocycle_report`, `muscle_group_volume_log`, `needs_day`, `notification_pref`,
