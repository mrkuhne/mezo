...
> **Where things live.** Generated 2026-08-27 from commit `c293b868`.
...
| [companion](#companion) | ✓ | 4 | · | · | [companion](features/companion.md), [journal](features/journal.md) |
...
- **Contract** `api/feature/me-week/me-week.yml` — 1 operation
  - **endpoints:** GET /api/me/week/{start}
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 123 IT + 16 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `AmbientRecallEvalIT`, `AmbientRecallTuningIT`, `ChatExtractionFlowIT`,
    ...
    `LearnedFactPersistenceIT`, `LifeEventExtractionServiceIT`, `MeWeekControllerIT`, `MemoryEmbeddingAnnQueryIT`,
...
- **FE data** `frontend/src/data/me`
  ...
  - **modules:** biometricHooks.ts, biometricProfileApi.ts, biometricsApi.ts, goalApi.ts, goalHooks.ts, goalLinkApi.ts,
    ...
    peopleApi.ts, peopleHooks.ts, sleep.ts, sleepGoal.ts, sleepHooks.ts, sleepShot.ts, weightHooks.ts
- **FE ui** `frontend/src/features/me`
  - **pages:** AiCallDetailPage.tsx, AiUsagePage.tsx, GoalPlannerPage.tsx, GoalsPage.tsx, GoalsSkeleton.tsx,
    ...
    PeoplePage.tsx, ProfilePage.tsx, RoutineEditorPage.tsx, SleepPage.tsx, WeekPage.tsx, WeightPage.tsx, tabs.ts
  ...
  - **components:** AiCallFilters.tsx, AiCallRow.tsx, AiCallUsage.tsx, AiFeatureBreakdown.tsx, AiModelBreakdown.tsx,
    ...
    TimePicker.tsx, WeekDayCard.tsx, WeekScoreBars.tsx, WeeklyWeightCard.tsx, WeightHero.tsx, WeightTrendChart.tsx
  - **logic:** biometricFields.ts, buildTdeeBreakdown.ts, goalLabels.ts, gratitudeStreak.ts, growthJournal.ts,
    ...
    sleepEducation.ts, sleepEscalation.ts, sleepPhases.ts, sleepStats.ts, weekNav.ts, weightStats.ts
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/proactive`
  ...
  - **repositories:** `ChallengeRepository`, `CompanionMessageRepository`, `ExperimentRepository`, `MemoirRepository`,
    `PredictionRepository`, `WeeklyReviewRepository`, `WeeklySuggestionRepository`
  - **services:** `ChallengeGenerator`, `ChallengeJob`, `ChallengeOutcomeEvaluator`, `CompanionMessageEventListener`,
    ...
    `ProactivePredictionService`, `ProactiveWeeklySuggestionService`, `WeeklyReviewDigestService`,
    `WeeklyReviewGenerator`, `WeeklyReviewService`, `WeeklyReviewWeekWindow`, `WeeklySuggestionGenerator`,
    `WeeklySuggestionJob`
  ...
  - **other:** `ChallengeRefsEnvelope`, `CompanionMessageEnvelope`, `MemoirAnchorsEnvelope`,
    `WeeklyReviewDayNotesEnvelope`, `WeeklyReviewHighlightsEnvelope`
- **Contract** `api/feature/proactive/proactive.yml` — 12 operations
  - **endpoints:** GET /api/proactive/feed · GET /api/proactive/weekly-suggestion · GET /api/proactive/memoir ·
    ...
    POST /api/proactive/weekly-review/{start}/regenerate · GET /api/proactive/weekly-review/{start}/digest
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/proactive` — 42 IT + 0 unit
  - **ITs:** `ChallengeGeneratorIT`, `ChallengeJobIT`, `ChallengeJobSwitchOffIT`, `ChallengeOutcomeIT`,
    ...
    `ProactiveApiSwitchOffIT`, `WeeklyReviewControllerIT`, `WeeklyReviewGeneratorIT`, `WeeklySuggestionGeneratorIT`,
  ...
  - **populators:** `ActivityPopulator`, `ChallengePopulator`, `CheckInPopulator`, `CompanionMessagePopulator`,
    ...
    `UserPopulator`, `WeeklyReviewPopulator`, `WeeklySuggestionPopulator`, `WeightLogPopulator`
...
- **`ResetDatabase` TRUNCATE list** — 83 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `sport_schedule_slot`, `sport_session`, `supplement_intake`, `water_log`, `weekly_review`, `weekly_suggestion`,
