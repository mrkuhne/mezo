...
- **Contract** `api/feature/auth/auth.yml` — 6 operations
  - **endpoints:** POST /api/auth/login · POST /api/auth/register · PUT /api/auth/me · GET /api/auth/me ·
    POST /api/auth/change-password · POST /api/auth/onboarding-complete
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/auth` — 12 IT + 4 unit
  - **ITs:** `AccountUpdateIT`, `AdminInviteIT`, `AdminUserIT`, `AuthControllerIT`, `AuthIsolationIT`, `AuthMeIT`,
    `AuthOnboardingIT`, `AuthRegisterIT`, `CurrentUserIT`, `OwnerSeedDataIT`, `PromptPersonaIT`, `UserFanOutIT`
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  ...
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `CompanionFlagLogRepository`,
    ...
    `DenseMemoryQuery`, `FeedbackRollupRepository`, `GraphEdgeRepository`, `GraphNodeRepository`,
    `GraphStructureQuery`, `GraphTraversalQuery`, `KnowledgeFactRepository`, `KnowledgeFactRetrievalQuery`,
    `LearnedFactRepository`, `LexicalMemoryQuery`, `MemoryEmbeddingAnnQuery`, `MemoryEmbeddingRepository`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 257 IT + 85 unit
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `BiometricProfilePopulator`,
    ...
    `GoalPlanLinkPopulator`, `GoalPopulator`, `GraphPopulator`, `HabitPopulator`, `IntentionPopulator`,
...
*BE + API + FE-data* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-09-23, done)
...
- **FE data** `frontend/src/data/insights`
  - **hooks (via `@/data/hooks`):** `MemoryRetrievalFeedback`, `MemoryRetrievalFeedbackAction`,
    ...
    `useDiagnosisForWeek`, `useExperimentActions`, `useExperiments`, `useGraphEdgeCount`, `useKnowledge`,
    `useKnowledgeActions`, `useKnowledgeGraphActions`, `useKnowledgeGraphNodes`, `useLifeEventActions`,
    `useLifeEventCandidates`, `useLlmUsage`, `useMemoir`, `useMemoirArchive`, `useMemoryOverview`,
    `useMemoryRetrievalFeedback`, `useMemorySummaries`, `useObservationReply`, `useObservations`, `usePatternActions`,
...
*BE + API + FE-data* · read next: [docs/features/journal.md](features/journal.md) (updated 2026-09-23, done)
...
- **FE data** `frontend/src/data/me`
  - **hooks (via `@/data/hooks`):** `DayEvaluationResponse`, `NormalizedDayDimension`, `NormalizedDayEvaluation`,
    ...
    `useFeasibilityPreview`, `useGoal`, `useGoalActions`, `useGoalCreation`, `useGoalOverview`, `useGoalSettings`,
...
- **FE ui** `frontend/src/features/me`
  ...
  - **components:** AdminInviteRow.tsx, AdminUserRow.tsx, AiCallUsage.tsx, AiPayloadBlock.tsx, AiPriceSnapshot.tsx,
    ...
    GoalSettingsEditor.tsx, GoalSuggestionCard.tsx, GoalSuggestionDiffGrid.tsx, GratitudeRows.tsx,
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/proactive`
  ...
  - **services:** `AdviceActionCatalog`, `AdviceApplyService`, `AdviceCandidate`, `AdviceCardService`,
    ...
    `FatigueEvidenceCollector`, `FeatureAbandonmentDetector`, `FeedContext`, `FeedContextAssembler`,
    `FeedContinuityService`, `FeedEvidenceAssembler`, `FeedGenerationService`, `FeedMessageKindService`,
    `FlatFeedbackDetector`, `GeneratedFeedMessage`, `GrowthDigestBlock`, `HighlightCitationSourceAdapter`,
    `HydrationShortfallProbe`, `InterventionEventListener`, `InterventionService`, `LightenTomorrowAdapter`,
    `LogFreshnessProbe`, `MemoirGenerator`, `MemoirJob`, `MetricWindowEvaluator`, `OneTimeQuestionService`,
    ...
    `ProactiveWeeklySuggestionService`, `ProseNumberGuard`, `QuestionAnswerListener`, `QuestionAnswerService`,
    ...
    `WeeklySuggestionGenerator`, `WeeklySuggestionJob`, `WeightDecomposition`, `WeightDecompositionInputsAssembler`
  ...
  - **config:** `ContextualFeedProperties`, `DiagnosisProperties`, `ProactiveProperties`, `QuestionProperties`,
    `SetupCheckProperties`
  - **other:** `AdviceActionKey`, `ChallengeRefsEnvelope`, `CompanionMessageEnvelope`, `DiagnosisEvidenceEnvelope`,
    `DiagnosisSuspectsEnvelope`, `FeedGenerationTrace`, `MemoirAnchorsEnvelope`, `WeeklyReviewDayNotesEnvelope`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/proactive` — 104 IT + 8 unit
  - **ITs:** `AdviceApplyServiceIT`, `AdviceCardServiceIT`, `AdviceObserverPortsIT`, `AdviceProseGeneratorIT`,
    ...
    `ContextSnapshotOwnerZoneIT`, `ContextualFeedPropertiesIT`, `DiagnosisControllerIT`, `DiagnosisExperimentIT`,
    `DiagnosisGeneratorIT`, `DiagnosisGeneratorMemoryDisabledIT`, `DiagnosisGeneratorMemoryIT`, `ExperimentJobIT`,
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `ChallengePopulator`,
    ...
    `GraphPopulator`, `HabitPopulator`, `JournalPopulator`, `KnowledgeFactPopulator`, `LearnedFactPopulator`,
    `LifeGoalPopulator`, `LlmLogPopulator`, `MealPopulator`, `MedicationDosePopulator`, `MedicationPopulator`,
    `MemoirPopulator`, `MemoryEmbeddingPopulator`, `MemoryItemPopulator`, `MentionPopulator`, `PantryItemPopulator`,
...
*BE + API* · read next: [docs/features/recipe.md](features/recipe.md) (updated 2026-09-23, done)
...
- **FE ui** `frontend/src/features/settings`
  - **pages:** AccountSettingsPage.tsx, MeSettingsPage.tsx, MezoPersonalPage.tsx, MezoSettingsPage.tsx,
    NapSettingsPage.tsx, SettingsPage.tsx, TrainSettingsPage.tsx
  - **components:** SettingsFrame.tsx, UnsavedChangesGuard.tsx
  - **root:** personal-settings.css, settings.css, settingsRoutes.tsx
