...
> **Where things live.** Generated 2026-08-22 from commit `451b99e5`.
...
| [companion](#companion) | ✓ | 2 | · | · | [companion](features/companion.md), [journal](features/journal.md) |
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  - **sub-features:** `advisor`, `embedding`, `feedback`, `llm`, `tools`
  - **entities→tables:** `AiConversationEntity`→`ai_conversation`, `AiMessageEntity`→`ai_message`,
    `DailySummaryEntity`→`daily_summary`, `FeedbackRollupEntity`→`feedback_rollup`,
  ...
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `DailySummaryRepository`,
    `FeedbackRollupRepository`, `KnowledgeFactRepository`, `LearnedFactRepository`, `MemoryEmbeddingAnnQuery`,
  ...
  - **services:** `ChatService`, `ChatStreamService`, `ChatTurnCompleted`, `ContextSnapshotAssembler`,
    ...
    `FeedbackLearningJob`, `FeedbackLearningService`, `HypothesisJob`, `HypothesisPipelineService`,
  ...
  - **mappers:** `CompanionMapper`, `MessageFeedbackMapper`
  ...
  - **other:** `AdvisedAnswer`, `AdvisorRetry`, `AdvisorViolation`, `BiometricsTools`, `ChatHistory`,
    ...
    `GeminiRoundUsageAdvisor`, `GeminiUsageExtractor`, `GoalTools`, `GrowthTools`, `HabitSuggestLlmAdapter`,
    `InsightsTools`, `MealCoachLlmAdapter`, `MealDraftLlmAdapter`, `MedicationTools`, `MemoryEmbeddingWriter`,
    `MemoryTools`, `NarrativeNoteSource`, `NoteEmbeddingCatchUp`, `PantryPhotoLlmAdapter`, `PantryScrapeLlmAdapter`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 72 IT + 12 unit
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `BiometricProfilePopulator`,
    ...
    `MealPopulator`, `MedicationDosePopulator`, `MedicationPopulator`, `MemoryEmbeddingPopulator`, `MentionPopulator`,
    ...
    `UserPopulator`, `WaterLogPopulator`, `WeightLogPopulator`
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `FuelSettingsPopulator`, `GamificationPopulator`, `GoalPlanLinkPopulator`, `GoalPopulator`, `HabitPopulator`,
  ...
  `SupplementIntakePopulator`, `TrainPopulator`, `UserPopulator`, `WaterLogPopulator`, `WeeklySuggestionPopulator`,
  `WeightLogPopulator`
- **`ResetDatabase` TRUNCATE list** — 78 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `habit_day`, `habit_def`, `intention_creed`, `intention_focus`, `journal_entry`, `knowledge_fact`, `learned_fact`,
    ...
    `pantry_import`, `pantry_item`, `pattern`, `pattern_event`, `perk_unlock`, `person`, `prediction`, `protocol`,
    `protocol_item`, `push_log`, `push_subscription`, `recipe`, `recipe_ingredient`, `ritual_day`, `run_session_log`,
    `running_block`, `skill_progress`, `sleep_goal`, `sleep_log`, `sport_event`, `sport_schedule_slot`,
    `sport_session`, `supplement_intake`, `water_log`, `weekly_suggestion`, `weight_log`, `workout_session`
