...
> **Where things live.** Generated 2026-08-24 from commit `ac40238a`.
...
*BE + API* · read next: [docs/features/me.md](features/me.md) (updated 2026-08-24, mixed) ·
...
*BE + API* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-24, mixed) ·
  [docs/features/journal.md](features/journal.md) (updated 2026-08-24, done)

- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  - **sub-features:** `advisor`, `embedding`, `feedback`, `flags`, `graph`, `llm`, `profile`, `tools`
  ...
  - **services:** `ChatService`, `ChatStreamService`, `ChatTurnCompleted`, `ConsolidationJob`,
    ...
    `FeedMessageKindSource`, `FeedbackLearningJob`, `FeedbackLearningService`, `FlagEvaluator`, `FlagKey`, `FlagRaise`,
    `FlagService`, `GraphEdgeLineRenderer`, `GraphEdgeStructurer`, `GraphEdgeSuggestion`, `GraphMaintenanceJob`,
    `GraphMaintenanceResult`, `GraphMaintenanceService`, `GraphPromotionListener`, `GraphPromotionService`,
    `GraphPromptAssembler`, `GraphService`, `GraphTraversalService`, `HypothesisJob`, `HypothesisPipelineService`,
  ...
  - **config:** `CompanionProperties`, `FeedbackLearningProperties`, `FlagProperties`, `ProfileProperties`
  ...
  - **other:** `AdvisedAnswer`, `AdvisorRetry`, `AdvisorViolation`, `BiometricsTools`, `ChatHistory`,
    ...
    `GraphProposedEdge`, `GrowthTools`, `HabitSuggestLlmAdapter`, `InsightsTools`, `MealCoachLlmAdapter`,
    `MealDraftLlmAdapter`, `MedicationTools`, `MemoryEmbeddingWriter`, `MemoryTools`, `NarrativeNoteSource`,
    `NoteEmbeddingCatchUp`, `PantryPhotoLlmAdapter`, `PantryScrapeLlmAdapter`, `PatternCritiqueEnvelope`,
    `PatternEventPayloadEnvelope`, `PatternEvidenceEnvelope`, `PracticeTools`, `ProfileMetaEnvelope`,
    `RecalledMemoriesEnvelope`, `RecipeBreakdownLlmAdapter`, `RecordingToolCallback`, `RefsEnvelope`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 108 IT + 15 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `AmbientRecallEvalIT`, `AmbientRecallTuningIT`, `ChatExtractionFlowIT`,
    ...
    `CompanionFeedbackSwitchOffIT`, `CompanionFlagLogPersistenceIT`, `CompanionLlmFakeIT`,
    ...
    `FactCandidateServiceIT`, `FactExtractionServiceIT`, `FakeEmbeddingAdapterIT`, `FeedbackLearningJobSwitchOffIT`,
    `FeedbackLearningPropertiesIT`, `FeedbackLearningServiceIT`, `FeedbackRollupPersistenceIT`,
    `FlagEvaluatorMomentumRecoveryIT`, `FlagEvaluatorStressSleepIT`, `FlagPropertiesIT`, `FlagServiceIT`, `GraphApiIT`,
    ...
    `ProfileAssemblerJobSwitchOffIT`, `ProfilePromptAssemblerIT`, `ProfilePropertiesIT`, `ProfileSourceFindersIT`,
...
*BE + API* · read next: [docs/features/goal-engine.md](features/goal-engine.md) (updated 2026-08-15, done) ·
  [docs/features/me.md](features/me.md) (updated 2026-08-24, mixed)
...
*FE-data + FE-ui* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-24, mixed) ·
...
*BE + API + FE-data* · read next: [docs/features/journal.md](features/journal.md) (updated 2026-08-24, done)
...
- **FE ui** `frontend/src/features/me`
  ...
  - **components:** AiCallFilters.tsx, AiCallRow.tsx, AiCallUsage.tsx, AiFeatureBreakdown.tsx, AiModelBreakdown.tsx,
    ...
    PhaseReferenceRow.tsx, ProfileNodeCard.tsx, PushInstallGate.tsx, RemDurationCard.tsx, RoutinesTab.tsx,
    SkillBandCard.tsx, SleepChart.tsx, SleepEscalationCard.tsx, SleepLogRow.tsx, SleepStat.tsx, SleepStatCard.tsx,
    TimePicker.tsx, WeeklyWeightCard.tsx, WeightHero.tsx, WeightTrendChart.tsx
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `CompanionMessagePopulator`, `DailySummaryPopulator`, `ExperimentPopulator`, `FeedbackPopulator`, `FlagLogPopulator`,
...
- **`ResetDatabase` TRUNCATE list** — 82 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    `check_in`, `coin_event`, `companion_flag_log`, `companion_message`, `daily_intention`, `daily_quest`,
    `daily_summary`, `decision_entry`, `exercise`, `exercise_feedback`, `exercise_set`, `experiment`,
    `feedback_rollup`, `fuel_settings`, `gamification_profile`, `goal`, `goal_plan_link`, `gratitude_entry`,
    `gym_schedule_slot`, `habit_chain`, `habit_day`, `habit_def`, `intention_creed`, `intention_focus`,
    ...
    `memory_embedding`, `mention`, `meso_template`, `mesocycle`, `mesocycle_report`, `message_feedback`,
    ...
    `sport_schedule_slot`, `sport_session`, `supplement_intake`, `water_log`, `weekly_suggestion`, `weight_log`,
    `workout_session`
