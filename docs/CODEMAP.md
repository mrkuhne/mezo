...
> **Where things live.** Generated 2026-09-03 from commit `55c5dca31`.
...
- **FE data** `frontend/src/data/fuel`
  - **hooks (via `@/data/hooks`):** `DIET_SETTINGS_GHOST`, `useDietSettings`, `useDietSettingsActions`, `useFuelDay`,
    ...
    `useStackActions`, `useStackDay`, `useWaterActions`
...
- **`ResetDatabase` TRUNCATE list** — 89 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `daily_intention`, `daily_quest`, `daily_summary`, `decision_entry`, `diet_settings`, `exercise`,
