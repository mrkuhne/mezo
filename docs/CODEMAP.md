...
> **Where things live.** Generated 2026-08-22 from commit `91343d2e`.
...
| [companion](#companion) | ✓ | 3 | · | · | [companion](features/companion.md), [journal](features/journal.md) |
...
| [me](#me) | · | · | ✓ | ✓ | [growth](features/growth.md), [habit](features/habit.md), [journal](features/journal.md), [me](features/me.md), [today](features/today.md), [_platform-data-layer](features/_platform-data-layer.md), [_platform-notifications](features/_platform-notifications.md) |
...
| [proactive](#proactive) | ✓ | 1 | · | · | [proactive](features/proactive.md) |
...
*BE + API + FE-data* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-21, done)
...
*BE + API* · read next: [docs/features/_platform-notifications.md](features/_platform-notifications.md) (updated 2026-08-21, mixed)
...
*BE + API* · read next: [docs/features/me.md](features/me.md) (updated 2026-08-21, mixed) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-21, mixed)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/biometrics`
  ...
  - **repositories:** `BiometricProfileRepository`, `CheckInRepository`, `SleepGoalRepository`, `SleepLogRepository`,
    `WeightLogRepository`
  - **services:** `BiometricProfileService`, `CheckInService`, `SleepAnchorPort`, `SleepAnchorResolver`,
    `SleepGoalService`, `SleepLogSavedEvent`, `SleepLogService`, `SleepShotDraftValidator`, `SleepShotLlm`,
    `SleepShotService`, `WeightLogSavedEvent`, `WeightLogService`, `WeightTrendService`
...
*BE + API* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-22, mixed) ·
  [docs/features/journal.md](features/journal.md) (updated 2026-08-21, done)

- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  - **sub-features:** `advisor`, `embedding`, `feedback`, `graph`, `llm`, `tools`
  - **entities→tables:** `AiConversationEntity`→`ai_conversation`, `AiMessageEntity`→`ai_message`,
    `DailySummaryEntity`→`daily_summary`, `FeedbackRollupEntity`→`feedback_rollup`, `GraphEdgeEntity`→`knowledge_edge`,
    ...
    `PatternEventEntity`→`pattern_event`
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `DailySummaryRepository`,
    `FeedbackRollupRepository`, `GraphEdgeRepository`, `GraphNodeRepository`, `KnowledgeFactRepository`,
    `LearnedFactRepository`, `MemoryEmbeddingAnnQuery`, `MemoryEmbeddingRepository`, `MessageFeedbackRepository`,
    `PatternEventRepository`, `PatternRepository`
  - **services:** `ChatService`, `ChatStreamService`, `ChatTurnCompleted`, `ContextSnapshotAssembler`,
    `ConversationService`, `DailySummaryJob`, `DailySummaryService`, `DecisionContextAssemblerAdapter`,
    `FactCandidateService`, `FactExtractionListener`, `FactExtractionService`, `FeedMessageKindSource`,
    `FeedbackLearningJob`, `FeedbackLearningService`, `GraphPromotionService`, `GraphService`, `HypothesisJob`,
    `HypothesisPipelineService`, `KnowledgeFactService`, `MemoryObservatoryService`, `MemoryRecallService`,
    `MesoContextAssembler`, `MesoReviewGenerator`, `MesoReviewListener`, `MessageFeedbackService`, `MetricDomain`,
    `MetricKey`, `MetricSeriesService`, `PatternDetectionJob`, `PatternDetectionService`, `PatternGate`,
    `PatternImpactSource`, `PatternMonitorService`, `PatternPairDetailService`, `PatternService`, `PearsonCorrelation`,
    `PromptMemoryAssembler`, `TranscriptionService`
  ...
  - **config:** `CompanionProperties`, `FeedbackLearningProperties`
  ...
  - **other:** `AdvisedAnswer`, `AdvisorRetry`, `AdvisorViolation`, `BiometricsTools`, `ChatHistory`,
    `CheckInNoteSourceAdapter`, `ClinicalOutputCheck`, `CompanionAdvisorChain`, `CompanionHelloRunner`, `CompanionLlm`,
    `CompanionToolRegistry`, `EmbeddingPort`, `FakeCompanionLlm`, `FakeEmbeddingAdapter`,
    `FeedbackRollupStatsEnvelope`, `FuelTools`, `GeminiCompanionLlm`, `GeminiEmbeddingAdapter`, `GeminiRoundUsage`,
    `GeminiRoundUsageAdvisor`, `GeminiUsageExtractor`, `GoalTools`, `GraphEdgeEvidence`, `GrowthTools`,
    `HabitSuggestLlmAdapter`, `InsightsTools`, `MealCoachLlmAdapter`, `MealDraftLlmAdapter`, `MedicationTools`,
    `MemoryEmbeddingWriter`, `MemoryTools`, `NarrativeNoteSource`, `NoteEmbeddingCatchUp`, `PantryPhotoLlmAdapter`,
    `PantryScrapeLlmAdapter`, `PatternCritiqueEnvelope`, `PatternEventPayloadEnvelope`, `PatternEvidenceEnvelope`,
    `PracticeTools`, `RecalledMemoriesEnvelope`, `RecipeBreakdownLlmAdapter`, `RecordingToolCallback`, `RefsEnvelope`,
    `SleepShotLlmAdapter`, `SlotPlanLlmAdapter`, `StackPlacementLlmAdapter`, `TodayActivitySource`, `TodayQuestSource`,
...
- **Contract** `api/feature/knowledge-graph/knowledge-graph.yml` — 2 operations
  - **endpoints:** GET /api/companion/graph/node · POST /api/companion/graph/node/{id}/archive
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 77 IT + 12 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `ChatExtractionFlowIT`, `ChatExtractionSwitchOffIT`,
    `ChatServiceAmbientRecallIT`, `ChatServiceIT`, `ChatStreamAdvisorIT`, `ChatStreamServiceIT`,
    ...
    `CompanionMemorySwitchOffIT`, `CompanionPatternApiIT`, `CompanionPatternMonitorApiIT`,
    ...
    `CompanionToolsRenderIT`, `CompanionTranscribeApiIT`, `ContextSnapshotAssemblerIT`, `ConversationServiceIT`,
    `DailySummaryJobIT`, `DailySummaryJobSwitchOffIT`, `DailySummaryServiceIT`, `FactCandidateServiceIT`,
    `FactExtractionServiceIT`, `FakeEmbeddingAdapterIT`, `FeedbackLearningJobSwitchOffIT`,
    `FeedbackLearningPropertiesIT`, `FeedbackLearningServiceIT`, `FeedbackRollupPersistenceIT`, `GraphApiIT`,
    `GraphEntityPersistenceIT`, `GraphPromotionServiceIT`, `GraphServiceIT`, `GraphSwitchOffIT`,
    `HypothesisGatherContextIT`, `HypothesisJobSwitchOffIT`, `HypothesisPipelineServiceIT`, `KnowledgeFactServiceIT`,
    `LearnedFactPersistenceIT`, `MemoryEmbeddingAnnQueryIT`, `MemoryEmbeddingRepositoryIT`, `MemoryEmbeddingWriterIT`,
    `MemoryRecallServiceIT`, `MemoryToolsRenderIT`, `MesoReviewGeneratorIT`, `MessageFeedbackPersistenceIT`,
    `MetricSeriesDerivedIT`, `MetricSeriesExpansionIT`, `MetricSeriesServiceIT`, `NoteEmbeddingBudgetIT`,
    `NoteEmbeddingCatchUpIT`, `NoteEmbeddingSwitchOffIT`, `NoteEmbeddingWriterIT`, `PatternDetectionJobSwitchOffIT`,
    `PatternDetectionServiceIT`, `PromptMemoryAssemblerIT`, `PromptMemoryAssemblerSwitchOffIT`, `ToolSelectionEvalIT`,
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `BiometricProfilePopulator`,
    ...
    `GamificationPopulator`, `GoalPlanLinkPopulator`, `GoalPopulator`, `GraphPopulator`, `HabitPopulator`,
    `IntentionPopulator`, `JournalPopulator`, `KnowledgeFactPopulator`, `LearnedFactPopulator`,
    ...
    `SupplementIntakePopulator`, `TrainPopulator`, `UserPopulator`, `WaterLogPopulator`, `WeightLogPopulator`

### feedback

*FE-data* · read next: [docs/features/insights.md](features/insights.md) (updated 2026-08-21, mixed)
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/fuel.md](features/fuel.md) (updated 2026-08-16, done) ·
  [docs/features/_platform-api-backend.md](features/_platform-api-backend.md) (updated 2026-08-21, done) ·
...
- **FE ui** `frontend/src/features/fuel`
  - **pages:** FuelKamraPage.tsx, FuelMaiPage.tsx, FuelMedicationPage.tsx, FuelPlanPage.tsx, FuelRecipesPage.tsx,
    FuelSection.tsx, FuelSlotsPage.tsx, FuelStackPage.tsx, KamraItemDetailPage.tsx, KamraSkeleton.tsx,
    RecipeDetailPage.tsx, RecipeEditorPage.tsx, RecipesSkeleton.tsx, tabs.ts
  - **sheets:** AddPantryItemSheet.tsx, AiLogSheet.tsx, CategoryFilterSheet.tsx, EnergyBreakdownSheet.tsx,
    FuelSettingsSheet.tsx, ImportItemSheet.tsx, IngredientPickerSheet.tsx, LogDoseSheet.tsx, LogMealSheet.tsx,
    MealPickerSheet.tsx, MealScoreSheet.tsx, ReplanSheet.tsx, StackItemSheet.tsx, StackPickerSheet.tsx,
    WaterLogSheet.tsx
  - **components:** ContextPanel.tsx, DimensionCard.tsx, DoneWindowsCapsule.tsx, KamraCard.tsx, KeretHero.tsx,
    MacroCells.tsx, MacroPanel.tsx, MealScoreChip.tsx, MedicationCycleBar.tsx, MedicationWeekStrip.tsx, MicroPanel.tsx,
    ...
    ScoreBreakdownBody.tsx, ScoreHero.tsx, ServingToggle.tsx, SourceBadge.tsx, StackMealMatch.tsx, StackZoneCard.tsx,
    SuggestionCard.tsx, WeekRhythmGrid.tsx, WeeklySupplementGrid.tsx, WindowIsland.tsx
  - **logic:** buildDayPlan.ts, buildEnergyBreakdown.ts, buildProtocol.ts, compileTemplate.ts, dayZones.ts,
    deriveMealName.ts, heroWindow.ts, kamraItems.ts, keretHero.ts, matchMealsToStack.ts, mealDisplayName.ts,
    projectStackDay.ts, recipeRole.ts, resolveDayType.ts, validateSlotPlan.ts, windowIslands.ts
...
*BE + API* · read next: [docs/features/goal-engine.md](features/goal-engine.md) (updated 2026-08-15, done) ·
  [docs/features/me.md](features/me.md) (updated 2026-08-21, mixed)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/goal`
  ...
  - **services:** `GoalEngineService`, `GoalEvaluationService`, `GoalFeasibilityService`, `GoalPlanLinkService`,
    `GoalProjectionService`, `GoalService`, `GoalTimelineService`, `GuardEvaluationService`, `TdeeBootstrapService`
...
*BE + API + FE-data* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-08-21, done)
...
*FE-data + FE-ui* · read next: [docs/features/companion.md](features/companion.md) (updated 2026-08-22, mixed) ·
  [docs/features/insights.md](features/insights.md) (updated 2026-08-21, mixed)

- **FE data** `frontend/src/data/insights`
  - **hooks (via `@/data/hooks`):** `NEW_CHAT`, `useChat`, `useChatActions`, `useConversations`,
    `useExperimentActions`, `useExperiments`, `useInsights`, `useKnowledge`, `useKnowledgeActions`, `useLlmUsage`,
    `useMemoir`, `useMemoryOverview`, `useMemorySummaries`, `usePatternActions`, `usePatternMonitor`,
    `usePatternPairDetail`, `usePatterns`, `usePredictions`, `useSimilarDays`, `useTranscribe`, `useWeekly`
  - **modules:** chat.ts, chatApi.ts, chatHooks.ts, experimentsApi.ts, experimentsHooks.ts, growthWeekApi.ts,
    insights.ts, insightsHooks.ts, knowledge.ts, knowledgeApi.ts, knowledgeHooks.ts, memoirApi.ts, memoirHooks.ts,
    memory.ts, memoryApi.ts, memoryHooks.ts, monitorApi.ts, monitorHooks.ts, patternDetailApi.ts,
    patternDetailHooks.ts, patternsApi.ts, patternsHooks.ts, predictionsApi.ts, predictionsHooks.ts, weeklyHooks.ts,
    weeklySuggestionApi.ts
- **FE ui** `frontend/src/features/insights`
  - **pages:** ChatPage.tsx, ExperimentsPage.tsx, InsightsSection.tsx, KnowledgeListPage.tsx, MemoirPage.tsx,
    MemoryPage.tsx, PatternDetailPage.tsx, PatternsPage.tsx, PredictionsPage.tsx, WeeklyPage.tsx, tabs.ts
  - **sheets:** ConversationPickerSheet.tsx
  - **components:** ChatMessage.tsx, FactCandidateCard.tsx, FeedbackChips.tsx, GrowthWeekCard.tsx,
    KnowledgeExplainer.tsx, KnowledgeFactRow.tsx, LifecycleSection.tsx, MemoryAuditPanel.tsx, MemoryJournalPanel.tsx,
    MemoryLayerCard.tsx, MemoryLayersPanel.tsx, MemorySearchPanel.tsx, MetricCoverageRing.tsx, MotorStateHero.tsx,
    PatternDecisionCard.tsx, PatternImpactCard.tsx, PatternJournal.tsx, PatternScatter.tsx, PatternStrengthChart.tsx,
    RecalledMemoriesRow.tsx, SimilarDayCard.tsx, TokenColumns.tsx
  - **logic:** domains.ts, factCopy.ts, findings.ts, lifecycle.ts, metricFormat.ts, patternHistory.ts,
    useStickToBottom.ts, useVoiceInput.ts, verdicts.ts

### intention

*BE + API + FE-data* · read next: [docs/features/intention.md](features/intention.md) (updated 2026-08-11, done)
...
*BE + API + FE-data* · read next: [docs/features/journal.md](features/journal.md) (updated 2026-08-21, done)
...
*FE-data + FE-ui* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-21, done) ·
  [docs/features/habit.md](features/habit.md) (updated 2026-08-21, done) ·
  [docs/features/journal.md](features/journal.md) (updated 2026-08-21, done) ·
  [docs/features/me.md](features/me.md) (updated 2026-08-21, mixed) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-21, mixed) ·
  [docs/features/_platform-data-layer.md](features/_platform-data-layer.md) (updated 2026-08-21, done) ·
  [docs/features/_platform-notifications.md](features/_platform-notifications.md) (updated 2026-08-21, mixed)
...
- **FE data** `frontend/src/data/me`
  - **hooks (via `@/data/hooks`):** `useBiometricActions`, `useBiometricProfile`, `useFeasibilityPreview`, `useGoal`,
    ...
    `usePeople`, `useProfile`, `useSleep`, `useSleepGoal`, `useSleepGoalActions`, `useSleepShot`, `useWeight`
  - **modules:** biometricHooks.ts, biometricProfileApi.ts, biometricsApi.ts, goalApi.ts, goalHooks.ts, goalLinkApi.ts,
    goals.ts, llmUsageApi.ts, llmUsageHooks.ts, meHooks.ts, people.ts, peopleApi.ts, peopleHooks.ts, sleep.ts,
    sleepGoal.ts, sleepHooks.ts, sleepShot.ts, weightHooks.ts
- **FE ui** `frontend/src/features/me`
  - **pages:** AiCallDetailPage.tsx, AiUsagePage.tsx, GoalPlannerPage.tsx, GoalsPage.tsx, GoalsSkeleton.tsx,
    GrowthPage.tsx, JournalPage.tsx, KnowledgePage.tsx, MeSection.tsx, NightPage.tsx, NotificationsPage.tsx,
    PeoplePage.tsx, ProfilePage.tsx, RoutineEditorPage.tsx, SleepPage.tsx, WeightPage.tsx, tabs.ts
  ...
  - **components:** AiCallFilters.tsx, AiCallRow.tsx, AiCallUsage.tsx, AiFeatureBreakdown.tsx, AiModelBreakdown.tsx,
    AiPayloadBlock.tsx, AiPriceSnapshot.tsx, AiTokenBar.tsx, AiUsageCard.tsx, AiUsageHero.tsx, BadgesCard.tsx,
    BiometricCard.tsx, CategoryHeader.tsx, DetailStat.tsx, FieldRow.tsx, GoalGate.tsx, GoalMiniCard.tsx,
    GoalPlanSlots.tsx, GoalRecept.tsx, GoalStat.tsx, GoalTimeline.tsx, GratitudeStreakCard.tsx, GrowthJournalCard.tsx,
    GrowthSummaryCard.tsx, KnowledgeFactCard.tsx, MeBioRow.tsx, MentionRow.tsx, NightArcCard.tsx, NightBodyScan.tsx,
    NightBreathing.tsx, NightWalk.tsx, NotificationCategoryRow.tsx, NotificationPreviewHeader.tsx, PerksCard.tsx,
    PersonCard.tsx, PhaseAverageCard.tsx, PhaseRail.tsx, PhaseReferenceRow.tsx, PushInstallGate.tsx,
    RemDurationCard.tsx, RoutinesTab.tsx, SkillBandCard.tsx, SleepChart.tsx, SleepEscalationCard.tsx, SleepLogRow.tsx,
    SleepStat.tsx, SleepStatCard.tsx, TimePicker.tsx, WeeklyWeightCard.tsx, WeightHero.tsx, WeightTrendChart.tsx
  - **logic:** biometricFields.ts, buildTdeeBreakdown.ts, goalLabels.ts, gratitudeStreak.ts, growthJournal.ts,
    habitMetricPalette.ts, llmCallFormat.ts, nightContent.ts, nightFlow.ts, nightTrace.ts, notificationForecast.ts,
    sleepEducation.ts, sleepEscalation.ts, sleepPhases.ts, sleepStats.ts, weightStats.ts

### meal

*BE + API* · read next: [docs/features/fuel.md](features/fuel.md) (updated 2026-08-16, done)
...
- **Contract** `api/feature/medication/medication.yml` — 4 operations
  - **endpoints:** GET /api/medication · PUT /api/medication/{id} · POST /api/medication/{id}/dose ·
    DELETE /api/medication/{id}/dose/{doseId}
...
*BE + API + FE-data* · read next: [docs/features/needs.md](features/needs.md) (updated 2026-08-21, done)
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/_platform-notifications.md](features/_platform-notifications.md) (updated 2026-08-21, mixed)
...
- **FE ui** `frontend/src/features/notification`
  - **components:** NotificationBell.tsx, NotificationPanel.tsx
  - **logic:** groupByDay.ts
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/notification` — 12 IT + 4 unit
  - **ITs:** `AnchorResolverDecisionIT`, `AnchorResolverFeedIT`, `AnchorResolverIT`, `AnchorResolverRitualSwitchOffIT`,
    `NotificationApiIT`, `NotificationDispatchJobIT`, `NotificationPrefApiIT`, `NotificationPrefRepositoryIT`,
    `NotificationScheduleApiIT`, `PushSenderIT`, `PushSubscriptionRepositoryIT`, `PushSubscriptionServiceIT`
  - **populators:** `AppNotificationPopulator`, `CompanionMessagePopulator`, `DatabasePopulator`, `JournalPopulator`,
    ...
    `TrainPopulator`, `UserPopulator`, `WeeklySuggestionPopulator`

### nutrition

*BE* · read next: [docs/features/fuel.md](features/fuel.md) (updated 2026-08-16, done)
...
- **Contract** `api/feature/people/people.yml` — 2 operations
  - **endpoints:** GET /api/people · POST /api/people/{personId}/mentions
...
*BE + API* · read next: [docs/features/proactive.md](features/proactive.md) (updated 2026-08-21, complete)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/proactive`
  - **entities→tables:** `ChallengeEntity`→`challenge`, `CompanionMessageEntity`→`companion_message`,
    `ExperimentEntity`→`experiment`, `MemoirEntity`→`memoir`, `PredictionEntity`→`prediction`,
    `WeeklySuggestionEntity`→`weekly_suggestion`
  - **repositories:** `ChallengeRepository`, `CompanionMessageRepository`, `ExperimentRepository`, `MemoirRepository`,
    `PredictionRepository`, `WeeklySuggestionRepository`
  - **services:** `ChallengeGenerator`, `ChallengeJob`, `ChallengeOutcomeEvaluator`, `CompanionMessageEventListener`,
    `CompanionMessageGenerator`, `CompanionMessageJob`, `ExperimentJob`, `ExperimentOutcomeService`,
    `ExperimentProposalGenerator`, `FeedMessageKindService`, `GrowthDigestBlock`, `MemoirGenerator`, `MemoirJob`,
    `MetricWindowEvaluator`, `OverloadChallengeGenerator`, `PatternImpactService`, `PredictionGenerator`,
  ...
  - **controllers→contract:** `ProactiveController`→`ProactiveApi`
  - **mappers:** `ChallengeDisplay`, `ProactiveMapper`
  - **config:** `ProactiveProperties`
  - **other:** `ChallengeRefsEnvelope`, `CompanionMessageEnvelope`, `MemoirAnchorsEnvelope`
- **Contract** `api/feature/proactive/proactive.yml` — 9 operations
  - **endpoints:** GET /api/proactive/feed · GET /api/proactive/weekly-suggestion · GET /api/proactive/memoir ·
    ...
    POST /api/proactive/challenge/{id}/decision
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/proactive` — 36 IT + 0 unit
  - **ITs:** `ChallengeGeneratorIT`, `ChallengeJobIT`, `ChallengeJobSwitchOffIT`, `ChallengeOutcomeIT`,
    `ChallengePersistenceIT`, `CompanionMessageEventIT`, `CompanionMessageGeneratorIT`, `CompanionMessageJobIT`,
    `CompanionMessageJobSwitchOffIT`, `CompanionMessagePersistenceIT`, `ExperimentJobIT`, `ExperimentJobSwitchOffIT`,
    `ExperimentOutcomeIT`, `ExperimentPersistenceIT`, `ExperimentProposalGeneratorIT`, `GrowthDigestBlockIT`,
    ...
    `ProactiveApiFeedIT`, `ProactiveApiIT`, `ProactiveApiSwitchOffIT`, `WeeklySuggestionGeneratorIT`,
  ...
  - **populators:** `ActivityPopulator`, `ChallengePopulator`, `CheckInPopulator`, `CompanionMessagePopulator`,
    `DailySummaryPopulator`, `ExperimentPopulator`, `KnowledgeFactPopulator`, `MemoirPopulator`, `PatternPopulator`,
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/growth.md](features/growth.md) (updated 2026-08-21, done)
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/ritual.md](features/ritual.md) (updated 2026-08-21, done)
...
*FE-data + FE-ui* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-08-21, done) ·
  [docs/features/intention.md](features/intention.md) (updated 2026-08-11, done) ·
  [docs/features/needs.md](features/needs.md) (updated 2026-08-21, done) ·
  [docs/features/ritual.md](features/ritual.md) (updated 2026-08-21, done) ·
  [docs/features/today.md](features/today.md) (updated 2026-08-21, mixed)
...
- **FE ui** `frontend/src/features/today`
  - **pages:** TodayPage.tsx, TodaySkeleton.tsx
  - **sheets:** ActivityLogSheet.tsx, CheckInSheet.tsx, CreedSheet.tsx, IntentionSheet.tsx, NeedRingSheet.tsx,
    ReflectSheet.tsx
  - **components:** ActivityLogCard.tsx, AnchorIsland.tsx, ChainCelebrations.tsx, DailyQuestList.tsx,
    DailyQuestsCard.tsx, DailyQuestsChip.tsx, DailyQuestsSheet.tsx, DayGroups.tsx, DaypartDay.tsx, DaypartEvening.tsx,
    DaypartMorning.tsx, DaypartPanel.tsx, DaypartTabs.tsx, IntentionBanner.tsx, MezoChip.tsx, MezoMessagesSheet.tsx,
    NeedsRow.tsx, TodayList.tsx, TodayRow.tsx, TodayStats.tsx, VulnerabilityCard.tsx
  - **logic:** dayArc.ts, dayFace.ts, growthToday.ts, habitAction.ts, islandFacts.ts, itemIcon.ts, mezoMessages.ts,
    needs.ts, needsInputs.ts, needsNudges.ts, questAction.ts, rowAccessory.ts, todayItems.ts, useChainCelebration.ts,
    useMinuteTick.ts, useNeeds.ts, useWindDownPhase.ts, windDown.ts
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/train`
  ...
  - **services:** `CatalogMediaResolver`, `ClosingBlockService`, `ExerciseCatalogService`, `ExerciseHistoryResolver`,
    `ExerciseRecordService`, `GymScheduleService`, `MedalEvaluator`, `MedalService`, `MesoTemplateService`,
    `MesoWeeks`, `MesocycleReportService`, `MuscleGroup`, `Prescription`, `ProgressionDecider`, `RunningService`,
    `SetRecommendationService`, `SportService`, `TrainService`, `VolumeArcService`, `VolumeDecider`,
    `VolumeProgressionService`, `WeeklyScheduledActivityService`, `WorkoutAutoCloseService`, `WorkoutService`,
  ...
  - **other:** `ClosingBlockGate`, `ExerciseCatalogLoader`, `GymExerciseJson`, `GymSignalCalculator`,
    ...
    `VolumeRecomputeJson`
- **Contract** `api/feature/train/train.yml` — 53 operations
  - **endpoints:** GET /api/train/mesocycles · POST /api/train/mesocycles/{id}/activate ·
    POST /api/train/mesocycles/{id}/close · GET /api/train/mesocycles/{id}/report ·
    POST /api/train/mesocycles/{id}/report/regenerate · GET /api/train/mesocycles/{id}/volume-arc ·
    PUT /api/train/mesocycles/{id}/days/{dayId}/exercises · POST /api/train/mesocycles/{id}/rerun ·
    GET /api/train/meso-templates · POST /api/train/meso-templates · PUT /api/train/meso-templates/{id} ·
    DELETE /api/train/meso-templates/{id} · POST /api/train/meso-templates/{id}/start · GET /api/train/custom-workouts ·
    POST /api/train/custom-workouts · PUT /api/train/custom-workouts/{id} · DELETE /api/train/custom-workouts/{id} ·
    GET /api/train/exercises · POST /api/train/exercises · PUT /api/train/exercises/{id} ·
    DELETE /api/train/exercises/{id} · PUT /api/train/exercises/{id}/video · PUT /api/train/exercises/{id}/images ·
    GET /api/train/exercise-records · GET /api/train/medals · GET /api/train/sport-sessions ·
    POST /api/train/sport-sessions · GET /api/train/sport-schedule · PUT /api/train/sport-schedule ·
    GET /api/train/sport-events · POST /api/train/sport-events · DELETE /api/train/sport-events/{id} ·
    ...
    POST /api/train/workouts/{id}/finish · GET /api/train/running-blocks · POST /api/train/running-blocks ·
    PUT /api/train/running-blocks/{id} · DELETE /api/train/running-blocks/{id} ·
...
- **FE data** `frontend/src/data/train`
  - **hooks (via `@/data/hooks`):** `useChallengeActions`, `useChallenges`, `useCustomWorkoutActions`,
    `useCustomWorkouts`, `useMedals`, `useMesoReport`, `useMesoTemplates`, `useMesocycleVolumeArc`, `useRunning`,
    `useTrain`, `useWeekMuscleLog`, `useWeekWorkouts`, `useWorkoutDetail`
  - **modules:** challengeApi.ts, challengeHooks.ts, customWorkoutHooks.ts, medalApi.ts, medalEvaluator.ts,
    ...
    weekMuscleLogHooks.ts, workoutDetailHooks.ts
- **FE ui** `frontend/src/features/train`
  - **pages:** ActiveWorkoutPage.tsx, CustomWorkoutBuilderPage.tsx, ExercisesPage.tsx, ExercisesSkeleton.tsx,
    GymPage.tsx, GymSkeleton.tsx, MedalsPage.tsx, MesoComparePage.tsx, MesoOverviewPage.tsx, MesoReportPage.tsx,
    MesoTemplateEditorPage.tsx, MesoTemplatesPage.tsx, MesoTemplatesSkeleton.tsx, MesocycleBuilderPage.tsx,
    MesocycleLibraryPage.tsx, MesocyclePlannerPage.tsx, MesocycleSkeleton.tsx, RunningBlockBuilderPage.tsx,
    RunningPage.tsx, SportPage.tsx, SportSkeleton.tsx, TrainSection.tsx, TrainTodayPage.tsx, TrainTodaySkeleton.tsx,
    TrainWeekPage.tsx, TrainWeekSkeleton.tsx, WorkoutReviewPage.tsx, tabs.ts
  ...
  - **components:** ActiveMesoCard.tsx, ArchivedMesoCard.tsx, ChallengeCard.tsx, ChallengesCarousel.tsx,
    CompactStepper.tsx, CrossLoadRow.tsx, DayBreakdownCard.tsx, DayStrip.tsx, DoneBar.tsx, ExerciseAccordionRow.tsx,
    ExerciseImage.tsx, ExerciseRecipeRow.tsx, FinalStat.tsx, GymDayCard.tsx, GymStat.tsx, LastWeekStat.tsx,
    LoadTiles.tsx, MedalChip.tsx, MedalToast.tsx, MesoEditor.tsx, MesoEditorHero.tsx, MesoExercises.tsx,
    MesoOverview.tsx, MesoTemplateCard.tsx, MesoVolume.tsx, MetaStat.tsx, MiniBar.tsx, MiniStat.tsx,
    MorningTrainingCard.tsx, MuscleArcSwitch.tsx, PhaseCurveBars.tsx, PhaseDots.tsx, PlannedMesoCard.tsx,
    PlannerExerciseRow.tsx, PrepExerciseCard.tsx, PrepHero.tsx, ProgressionBanner.tsx, RestTimerBar.tsx,
    RunCrossLoadCard.tsx, RunSessionCard.tsx, RunWeekEditor.tsx, RunWeekStrip.tsx, SetBudgetCard.tsx, SetStepper.tsx,
    SportSessionCard.tsx, SportStat.tsx, StructureLintCard.tsx, TodaySessionCard.tsx, VideoDemo.tsx,
    VolumeArcChart.tsx, VolumeBar.tsx, WeekZoneCard.tsx, WeekdayGrid.tsx, WeeklyDayRow.tsx, WorkoutSummary.tsx,
    ZoneMiniGrid.tsx, ZoneTrack.tsx
  - **logic:** agenda.ts, challengeOutcome.ts, dayStripItems.ts, exerciseDefaults.ts, growthForecast.ts,
    ...
    muscleWeek.ts, offDay.ts, planner.ts, prepBriefing.ts, programFit.ts, restTimer.ts, runToTemplate.ts,
...
- **lib:** audio.ts, cn.ts, dates.ts, daypart.ts, grams.ts, huNum.ts, markdown.tsx, nudgeSeen.ts, pct.ts,
  resizeImage.ts, safeMarkdown.tsx, screenScroll.ts, seenMessages.ts, theme.ts, toastBus.ts
- **ui:** AdherenceBar.tsx, Chip.tsx, CoachBubble.tsx, CountUp.tsx, Cta.tsx, DatePicker.tsx, DayNavigator.tsx,
  ...
  Skeleton.tsx, SortableList.tsx, Spinner.tsx, StatCell.tsx, StatStrip.tsx, Stepper.tsx, SubNavDropdown.tsx,
  ToastProvider.tsx, Toggle.tsx, ToolChip.tsx, ToolChipRow.tsx, TrendChart.tsx, sectionLabel.ts
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `CompanionMessagePopulator`, `DailySummaryPopulator`, `ExperimentPopulator`, `FeedbackPopulator`,
  `FuelSettingsPopulator`, `GamificationPopulator`, `GoalPlanLinkPopulator`, `GoalPopulator`, `GraphPopulator`,
  `HabitPopulator`, `IntentionPopulator`, `JournalPopulator`, `KnowledgeFactPopulator`, `LearnedFactPopulator`,
  `LevelUpEventPopulator`, `LlmLogPopulator`, `MealPopulator`, `MealSlotTemplatePopulator`, `MedicationDosePopulator`,
  ...
  `PatternPopulator`, `PersonPopulator`, `PredictionPopulator`, `ProtocolPopulator`, `QuestPopulator`,
  ...
  `WeeklySuggestionPopulator`, `WeightLogPopulator`
- **`ResetDatabase` TRUNCATE list** — 80 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    `check_in`, `coin_event`, `companion_message`, `daily_intention`, `daily_quest`, `daily_summary`, `decision_entry`,
    ...
    `habit_day`, `habit_def`, `intention_creed`, `intention_focus`, `journal_entry`, `knowledge_edge`,
    ...
    `notification_schedule`, `owned_title`, `pantry_import`, `pantry_item`, `pattern`, `pattern_event`, `perk_unlock`,
    ...
    `sport_event`, `sport_schedule_slot`, `sport_session`, `supplement_intake`, `water_log`, `weekly_suggestion`,
...
- **scripts/:** backup-live-db.sh, com.mezo.db-backup.plist, gen-codemap.mjs, gen-codemap.test.mjs,
  import-exercise-images.mjs, lint-docs.mjs, lint-liquibase.mjs
