...
> **Where things live.** Generated 2026-08-23 from commit `d6de71f2`.
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  ...
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `DailySummaryRepository`,
    ...
    `MessageFeedbackRepository`, `PatternEventRepository`, `PatternRepository`
  - **services:** `ChatService`, `ChatStreamService`, `ChatTurnCompleted`, `ContextSnapshotAssembler`,
    ...
    `GraphPromotionService`, `GraphPromptAssembler`, `GraphService`, `GraphTraversalService`, `HypothesisJob`,
    `HypothesisPipelineService`, `KnowledgeFactPromotedEvent`, `KnowledgeFactService`, `LifeEventCandidateService`,
    `LifeEventExtractionService`, `LifeEventSuggestion`, `MemoryObservatoryService`, `MemoryRecallService`,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 89 IT + 13 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `ChatExtractionFlowIT`, `ChatExtractionSwitchOffIT`,
    `ChatServiceAmbientRecallIT`, `ChatServiceGraphBlockFailureIT`, `ChatServiceGraphBlockIT`,
