...
> **Where things live.** Generated 2026-08-21 from commit `4eb80b76`.
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `WeightLogPopulator`
- **`ResetDatabase` TRUNCATE list** — 77 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `goal_plan_link`, `gratitude_entry`, `gym_schedule_slot`, `habit_chain`, `habit_day`, `habit_def`,
    `intention_creed`, `intention_focus`, `journal_entry`, `knowledge_fact`, `learned_fact`, `level_up_event`,
