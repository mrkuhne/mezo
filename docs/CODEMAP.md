...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/admin`
  ...
  - **services:** `AdminConvenienceViews`, `AdminDataBrowserService`, `AdminMemoryGraphMapper`,
...
- **Contract** `api/feature/admin-insights/admin-insights.yml` — 6 operations
...
- **FE ui** `frontend/src/features/admin`
  ...
  - **components:** AdminTile.tsx, AiCallFilters.tsx, AiCallRow.tsx, AiFeatureBreakdown.tsx, AiModelBreakdown.tsx,
    AiUsageHero.tsx, AiUserFilter.tsx, DataTable.tsx, JsonCell.tsx, MatrixGrid.tsx, ScreenUsageTable.tsx,
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/admin` — 14 IT + 3 unit
  - **ITs:** `AdminDataBrowserIT`, `AdminMemoryGraphIT`, `AdminMemoryHealthGraphOffIT`, `AdminMemoryHealthIT`,
    ...
    `AdminOverviewIT`, `AdminScreenUsageIT`, `AdminTableCatalogIT`, `AdminUsageIT`, `AdminUserDetailIT`,
    `AdminUserInsightIT`
  - **populators:** `AiConversationPopulator`, `AiMessagePopulator`, `GraphPopulator`, `MemoryEmbeddingPopulator`,
    `MemoryItemPopulator`
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/companion`
  ...
  - **entities→tables:** `AiConversationEntity`→`ai_conversation`, `AiMessageEntity`→`ai_message`,
    ...
    `DailySummaryEntity`→`daily_summary`, `DayReviewEntity`→`day_review`, `FeedbackRollupEntity`→`feedback_rollup`,
    `GraphEdgeEntity`→`knowledge_edge`, `GraphNodeEntity`→`knowledge_node`, `KnowledgeFactEntity`→`knowledge_fact`,
    `LearnedFactEntity`→`learned_fact`, `MemoryEmbeddingEntity`→`memory_embedding`, `MemoryItemEntity`→`memory_item`,
  ...
  - **repositories:** `AiConversationRepository`, `AiMessageRepository`, `CompanionFlagLogRepository`,
    ...
    `LexicalMemoryQuery`, `MemoryEmbeddingAnnQuery`, `MemoryEmbeddingRepository`, `MemoryHealthQuery`,
    ...
    `MemoryVectorPointQuery`, `MemoryVectorRepository`, `MessageFeedbackRepository`, `PatternEventRepository`,
  ...
  - **services:** `AdviceRankPort`, `ChatDaySignalService`, `ChatMemoryContextAdapter`, `ChatMentionListener`,
    ...
    `DayReviewLlm`, `DayReviewService`, `DayScoreService`, `DecisionContextAssemblerAdapter`, `DenseMemoryRetriever`,
    `DerivedSeriesService`, `FactCandidateService`, `FactExtractionListener`, `FactExtractionService`,
    `FactMemoryRetriever`, `FeedMessageKindSource`, `FeedbackLearningJob`, `FeedbackLearningService`, `FlagCatalog`,
    ...
    `FlagVerdict`, `GraphEdgeLineRenderer`, `GraphEdgeStructurer`, `GraphEdgeSuggestion`, `GraphMaintenanceJob`,
    ...
    `KnowledgeFactPromotedEvent`, `KnowledgeFactService`, `LexicalMemoryRetriever`, `LifeEventCandidateService`,
    `LifeEventExtractionService`, `LifeEventSuggestion`, `LifeGoalSnapshotBlock`, `LlmMemoryQueryRewriter`,
    `LlmMemoryReranker`, `MeWeekService`, `MemoryCandidateFusion`, `MemoryContextBlock`, `MemoryContextRenderer`,
    `MemoryContextSelector`, `MemoryContextService`, `MemoryItemFeedbackService`, `MemoryObservatoryService`,
    `MemoryProjectionEvent`, `MemoryProjectionListener`, `MemoryProjectionService`, `MemoryProjectionWriter`,
    ...
    `MetricValueKind`, `NudgeSendPort`, `ObservationBudget`, `ObservationFeedService`, `ObservationSourceIcon`,
    ...
    `TextSignalSeriesService`, `TextSignalService`, `TraceDisposition`, `TranscriptionService`, `UnavailableReason`,
  ...
  - **config:** `CompanionProperties`, `DayEvaluationProperties`, `FeedbackLearningProperties`, `FlagProperties`,
    `LlmProvider`, `MemoryPlatformProperties`, `ModelTier`, `ProfileProperties`, `QuarterlyProperties`,
  ...
  - **other:** `AcuteBadDayRule`, `AdvisedAnswer`, `AdvisorRetry`, `AdvisorViolation`, `AllHealthyRule`,
    ...
    `DayReviewJson`, `DayReviewLlmAdapter`, `EmbeddingPort`, `EnergyDipMealTimingRule`, `FakeCompanionLlm`,
    `FakeEmbeddingAdapter`, `FeedbackRollupStatsEnvelope`, `FlagPayloadEnvelope`, `FuelTools`, `GeminiCompanionLlm`,
    ...
    `JointOveruseRule`, `LateEatingRule`, `LifeGoalGraphSource`, `LifeGoalProposeLlmAdapter`, `LifeGoalProposePort`,
    `LifeGoalSource`, `LifeGoalText`, `LifeGoalTools`, `LlmModelRouter`, `LlmRoundUsage`, `LlmRoundUsageAdvisor`,
    `LlmUsageExtractor`, `LoadFuelMismatchRule`, `LoggingGapRule`, `MealCoachLlmAdapter`, `MealDraftLlmAdapter`,
    `MealRhythmDriftRule`, `MedicationTools`, `MemoryCandidate`, `MemoryContext`, `MemoryContextItem`,
...
- **Contract** `api/feature/companion/companion.yml` — 24 operations
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/companion` — 211 IT + 57 unit
  - **ITs:** `AiMessageJsonbRoundTripIT`, `AmbientRecallEvalIT`, `AmbientRecallTuningIT`, `AnchoredConversationIT`,
    ...
    `ChatSeedReplyIT`, `ChatServiceAmbientRecallIT`, `ChatServiceGraphBlockFailureIT`, `ChatServiceGraphBlockIT`,
    ...
    `ReflectionMemoryGatewayIT`, `ReflectionReplyServiceIT`, `SleepLogDetailRenderIT`, `TextSignalCatchUpIT`,
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `BiometricProfilePopulator`,
    ...
    `MealPopulator`, `MealSlotTemplatePopulator`, `MedicationDosePopulator`, `MedicationPopulator`,
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/fuel.md](features/fuel.md) (updated 2026-09-06, done) ·
  [docs/features/pantry.md](features/pantry.md) (updated 2026-09-06, done) ·
  [docs/features/recipe.md](features/recipe.md) (updated 2026-09-06, done) ·
...
*BE + API* · read next: [docs/features/goal-engine.md](features/goal-engine.md) (updated 2026-09-06, done) ·
  [docs/features/me.md](features/me.md) (updated 2026-09-07, mixed)
...
*BE + API + FE-data* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-09-07, done)
...
- **FE data** `frontend/src/data/insights`
  - **hooks (via `@/data/hooks`):** `MemoryRetrievalFeedback`, `MemoryRetrievalFeedbackAction`,
    `MemoryRetrievalFeedbackHandle`, `NEW_CHAT`, `useChat`, `useChatActions`, `useCoachingCard`, `useCoachingTrace`,
    `useConversationActions`, `useConversations`, `useDiagnoses`, `useDiagnosis`, `useDiagnosisActions`,
...
*BE + API + FE-data* · read next: [docs/features/intention.md](features/intention.md) (updated 2026-09-06, done)
...
*BE + API + FE-data* · read next: [docs/features/journal.md](features/journal.md) (updated 2026-09-06, done)
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/journal` — 13 IT + 1 unit
...
*BE + API + FE-data* · read next: [docs/features/lifegoal.md](features/lifegoal.md) (updated 2026-09-06, in-progress)
...
- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/lifegoal`
  ...
  - **services:** `LifeGoalCompanionAdapter`, `LifeGoalEvalJob`, `LifeGoalPillarService`, `LifeGoalProgressService`,
    `LifeGoalProposeService`, `LifeGoalService`, `LifeGoalSignalService`, `LifeGoalTemplateProposer`,
    `LifeGoalTriggerListener`, `LifeGoalTriggerRules`, `LifeGoalTriggerService`, `LifeGoalXpService`
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/lifegoal` — 18 IT + 2 unit
  - **ITs:** `LifeGoalApiIT`, `LifeGoalCompanionAdapterCompanionOffIT`, `LifeGoalCompanionAdapterIT`,
    ...
    `LifeGoalSeedDataIT`, `LifeGoalSignalsLivenessIT`, `LifeGoalTodayApiIT`, `LifeGoalTriggerIT`, `LifeGoalXpIT`,
    `SignalSourceIT`, `WeightGoalSignalSourceIT`
...
- **FE ui** `frontend/src/features/me`
  ...
  - **components:** AdminInviteRow.tsx, AdminUserRow.tsx, AiCallUsage.tsx, AiPayloadBlock.tsx, AiPriceSnapshot.tsx,
    ...
    MaStrip.tsx, MentionRow.tsx, NightArcCard.tsx, NightBodyScan.tsx, NightBreathing.tsx, NightWalk.tsx,
...
*BE + API* · read next: [docs/features/fuel.md](features/fuel.md) (updated 2026-09-06, done)
...
*BE + API + FE-data* · read next: [docs/features/needs.md](features/needs.md) (updated 2026-09-06, done)
...
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/proactive` — 83 IT + 4 unit
  - **ITs:** `AdviceApplyServiceIT`, `AdviceCardServiceIT`, `AdviceObserverPortsIT`, `AdviceProseGeneratorIT`,
    ...
    `CompanionMessageSetupPersistenceIT`, `ContextSnapshotOwnerZoneIT`, `DiagnosisControllerIT`,
    ...
    `MemoirPersistenceIT`, `OneTimeQuestionServiceIT`, `OneTimeQuestionSwitchOffIT`, `OverloadChallengeGeneratorIT`,
    ...
    `ProactiveApiSwitchOffIT`, `ProactiveMemoirArchiveIT`, `ProactiveMemoirArchiveSwitchOffIT`, `QuestionAnswerIT`,
    `QuestionPropertiesIT`, `ReflectionDigestMorningIT`, `RetroLoggingProbeIT`, `RetroLoggingPropertiesIT`,
    `SetupCheckJobSwitchOffIT`, `SetupCheckPropertiesIT`, `SetupCheckServiceIT`, `SleepAnchorShiftAdapterIT`,
    `SleepDiagnosisIT`, `SportSlotSkipAdapterIT`, `WeeklyLessonServiceIT`, `WeeklyReviewContextSourcesIT`,
  ...
  - **populators:** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`, `ChallengePopulator`,
    ...
    `MealPopulator`, `MedicationDosePopulator`, `MedicationPopulator`, `MemoirPopulator`, `MemoryEmbeddingPopulator`,
    `MemoryItemPopulator`, `MentionPopulator`, `PatternEventPopulator`, `PatternPopulator`, `PeriodSummaryPopulator`,
    `PersonPopulator`, `PredictionPopulator`, `QuestPopulator`, `SleepGoalPopulator`, `SleepLogPopulator`,
    `SportSlotSkipPopulator`, `TrainPopulator`, `UserPopulator`, `WaterLogPopulator`, `WeeklyReviewPopulator`,
...
*BE + API* · read next: [docs/features/recipe.md](features/recipe.md) (updated 2026-09-06, done)
...
*BE + API + FE-data + FE-ui* · read next: [docs/features/ritual.md](features/ritual.md) (updated 2026-09-06, done)
...
*BE + API + FE-data* · read next: **none — no HOW doc exists for this feature yet**

- **Backend** `backend/src/main/java/io/mrkuhne/mezo/feature/telemetry`
  - **entities→tables:** `ScreenEventEntity`→`screen_event`
  - **repositories:** `ScreenDayRow`, `ScreenEventRepository`, `ScreenUsageRow`
  - **services:** `ScreenEventRetentionJob`, `ScreenEventService`
  - **controllers→contract:** `TelemetryController`→`TelemetryApi`
  - **config:** `TelemetryProperties`
- **Contract** `api/feature/telemetry/telemetry.yml` — 1 operation
  - **endpoints:** POST /api/telemetry/screen-events
- **FE data** `frontend/src/data/telemetry`
  - **modules:** telemetryClient.ts
- **Tests** `backend/src/test/java/io/mrkuhne/mezo/feature/telemetry` — 3 IT + 1 unit
  - **ITs:** `ScreenEventIngestIT`, `ScreenEventRetentionJobIT`, `TelemetrySwitchOffIT`
  - **populators:** `UserPopulator`

### today

*FE-data + FE-ui* · read next: [docs/features/habit.md](features/habit.md) (updated 2026-09-07, done) ·
...
- **FE ui** `frontend/src/features/train`
  ...
  - **components:** ActiveMesoCard.tsx, ArchivedMesoCard.tsx, ChallengeCard.tsx, ChallengeGenerationLoader.tsx,
    CompactStepper.tsx, CrossLoadRow.tsx, DayBreakdownCard.tsx, DayLoadPanel.tsx, DayStrip.tsx, DayStripTile.tsx,
    DerivationSteps.tsx, DoneBar.tsx, ExerciseAccordionRow.tsx, ExerciseCard.tsx, ExerciseImage.tsx,
    ExerciseRecipeRow.tsx, ExerciseReview.tsx, LoadTile.tsx, LoadTiles.tsx, MedalChip.tsx, MedalToast.tsx,
    MesoDayEditor.tsx, MesoEditor.tsx, MesoEditorHero.tsx, MesoExercises.tsx, MesoTemplateCard.tsx, MesoWeekEditor.tsx,
    ...
    StructureLintCard.tsx, TodaySessionCard.tsx, VideoDemo.tsx, VolumeArcChart.tsx, VolumeBand.tsx, WeekLoadPanel.tsx,
    WeekZoneCard.tsx, WeekdayGrid.tsx, WeeklyBandsCard.tsx, WeeklyDayRow.tsx, WorkoutSummary.tsx, ZoneBar.tsx,
  ...
  - **logic:** actualDuration.ts, agenda.ts, challengeOutcome.ts, dayStripItems.ts, exerciseDefaults.ts,
    ...
    mesoLoad.ts, mesoPlan.ts, mesoWeek.ts, morningWindow.ts, muscleColors.ts, muscleFilters.ts, musclePriorities.ts,
    muscleWeek.ts, offDay.ts, peakWeekFit.ts, prepBriefing.ts, restTimer.ts, runToTemplate.ts, sessionLength.ts,
    sessionState.ts, setBudget.ts, sportKinds.ts, sportMuscleLoad.ts, structureLint.ts, summaryStats.ts,
...
- **populators (`support/populator/`):** `ActivityPopulator`, `AiConversationPopulator`, `AiMessagePopulator`,
  ...
  `WeightLogPopulator`, `WorkoutDayAdjustmentPopulator`
- **`ResetDatabase` TRUNCATE list** — 109 tables; a new owned domain table MUST be added here in the same change:
  - **tables:** `activity_log`, `ai_conversation`, `ai_message`, `app_notification`, `biometric_profile`, `challenge`,
    ...
    `companion_flag_trace`, `companion_message`, `daily_intention`, `daily_quest`, `daily_summary`, `day_review`,
    `decision_entry`, `diagnosis`, `diet_settings`, `exercise`, `exercise_feedback`, `exercise_set`, `experiment`,
    `feedback_rollup`, `fuel_settings`, `gamification_profile`, `goal`, `goal_plan_link`, `goal_suggestion`,
    `gratitude_entry`, `gym_schedule_slot`, `habit_chain`, `habit_day`, `habit_def`, `intention_creed`,
    `intention_focus`, `invite`, `journal_entry`, `knowledge_edge`, `knowledge_fact`, `knowledge_node`, `learned_fact`,
    `level_up_event`, `life_goal`, `life_goal_pillar`, `life_goal_pillar_day`, `llm_log_history`, `meal`, `meal_item`,
...
- **scripts/:** backup-live-db.sh, check-beads-backup.mjs, check-generator-toolchain.mjs, com.mezo.db-backup.plist,
  gen-codemap.mjs, gen-codemap.test.mjs, import-exercise-images.mjs, lint-conflict-markers.mjs, lint-docs.mjs,
  lint-liquibase.mjs, purge-restart.sql, setup-git.sh
