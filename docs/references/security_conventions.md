...
- Catalog tables are the one exception: `exercise_catalog` and `pantry_catalog` rows with
  `created_by IS NULL` are master data visible to everyone; user-authored rows are visible to
  everyone but editable only by author or OWNER (ADR 0035 decision K1, slice S4 / `mezo-qw37.4`,
  landed). Per-user state (`pantry_item.catalog_id NOT NULL`) stays fully owned; see
  [`pantry.md`](../features/pantry.md) §4.
