...
**Ownership exception — shared catalogs.** `exercise_catalog` is the one deliberate departure from "every domain table `created_by NOT NULL`": it is a hybrid table (`ExerciseCatalogEntity`, since `mezo-52zg`) where `createdBy == null` marks master content upserted by the startup loader from a JSON seed, and `createdBy` set marks a user-authored row. Every row — master or user-authored — is readable by everyone; a user-authored row is editable only by its author or an OWNER. Reads never filter `created_by = currentUser` on this table; the composite index leads with the natural key (slug), not `created_by`, per [`liquibase_conventions.md`](../references/liquibase_conventions.md). **A shared `pantry_catalog` on the same shape is ACCEPTED but NOT YET BUILT** (ADR 0035 decision K1, slice S4 / `mezo-qw37.4`) — at HEAD `pantry_item` (the only pantry table) is fully owned per-user (`created_by uuid NOT NULL`, no master rows), and `PantryCatalogLoader` seeds only the single demodata OWNER's own `pantry_item` rows, not a shared table. Do not write pantry code against shared-visibility semantics until S4 lands.
...
**The multi-user epic (`mezo-qw37`) at this HEAD — S1, S3 and S6 are built; S2, S4, S5 are not.** Design spec: [`docs/superpowers/specs/2026-09-02-multi-user-accounts-design.md`](../superpowers/specs/2026-09-02-multi-user-accounts-design.md).
...
- **S6 (`mezo-qw37.6`, this branch) — done:** `PromptPersona`/`PersonaContext`/`UserFanOut` (§4), the cron presence guard fix, push endpoint re-bind, `userScope` storage namespacing, the `useProfile`/`useMe` real-mode swap, and this doc itself.
- **Still genuinely ahead:**
  - **S2 (`mezo-qw37.2`) — onboarding.** Not built: `AuthGate`'s `AuthPhase` union (`pending | signedOut | mustChangePassword | ready | failed`) still has no `onboarding` state (verified: `frontend/src/app/auth/authState.ts`), and `useAuthActions().completeOnboarding()` has no UI caller anywhere in the frontend (verified: `grep` for `completeOnboarding(` outside the hook/API-client/test files returns nothing) — the backend endpoint exists (S1) but nothing collects the O2-lite wizard fields (name, birth date, sex, weight, height) and calls it.
  - **S4 (`mezo-qw37.4`) — the pantry catalog split.** Not built: see §4's "Ownership exception" note — `pantry_item` is still fully owned per-user, `pantry_catalog` does not exist. Plan: `docs/superpowers/plans/2026-09-02-s4-pantry-catalog.md`.
  - **S5 (`mezo-qw37.5`) — the exercise catalog slice.** Whatever S5 changes about `exercise_catalog` beyond its existing hybrid shape (which predates this epic, `mezo-52zg`) is not yet landed at this HEAD.
