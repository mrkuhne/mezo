...
related: [_platform-data-layer, _platform-api-backend, _platform-notifications, me, admin-hub]
---
...
**Ownership exception — the admin hub's cross-user reads.** A second, much broader exception: the `/admin/*` route family (`mezo-d5iy`, [`admin-hub.md`](admin-hub.md)) lets the `OWNER` read *any* account's rows across every owned table, gated by the same `CurrentUser.requireOwner()` used throughout this doc, via an `information_schema`-driven allowlisted row browser rather than a `created_by = currentUser` filter. Unlike the shared catalogs above, this is not a permanent schema-level departure — it is a time-boxed product decision resting on the beta testers' signed consent to admin visibility of their data for the duration of the beta, and it must be revisited before general availability. See [ADR 0038](../decisions/0038-admin-hub-cross-user-reads.md) for the full decision and its consequences (identifier allowlisting, explicit soft-delete predicates, credential-column exclusion).
...
- **Admin ↔ `requireOwner()` (S3, `mezo-qw37.3`).** *Contract:* `AdminController`'s every method calls `currentUser.requireOwner()` before `AdminService` runs — invite mint/list/delete, user list, temp-password reset (which also stamps `tokensValidFrom` to revoke the target's existing sessions), enable/disable (blocked on self via `ADMIN_SELF_STATUS`). See §4 and [`admin-hub.md`](admin-hub.md).
- **LLM-usage ↔ owner-only + `byUser` (S3, `mezo-qw37.3`) / per-user filter (`mezo-qw37.7`).** *Contract:* the installation-wide `/api/llm-usage/*` surface is `requireOwner()`-gated and reports a `byUser` cost/token split (rows with `created_by IS NULL` — pre-S6 cron traffic, stream writes — land in an unattributed "background" bucket, never guessed at). The companion Memória/Audit panel's `GET /api/companion/memory/llm-usage` takes the opposite shape: no owner gate, but `MemoryObservatoryService.llmUsage(userId, days)` filters `created_by = :userId` so every account — owner or not — sees only its own spend; `created_by IS NULL` rows are excluded there too, never attributed to the caller. See [`admin-hub.md`](admin-hub.md) §9. On the frontend, `BeallitasokPage`'s `useLlmUsageSummary({ enabled: isOwner })` (`isOwner = useMe().data?.role === 'OWNER'`) skips the installation-wide-summary fetch entirely for a non-owner — unconditionally firing it used to guarantee a 403 for a plain `USER` (twice, with TanStack Query's default retry).
...
- **`mezo-qw37.7` — companion memory observatory's LLM-usage was cross-account.** ✅ CLOSED (`MemoryObservatoryService.llmUsage`/`LlmUsageService.perDay` gained a `created_by = :userId` filter) — see §5, [`admin-hub.md`](admin-hub.md) §9.
