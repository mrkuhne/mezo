...
**Ownership exception — shared catalogs.** `exercise_catalog` and `pantry_catalog` are the one deliberate departure from "every domain table `created_by NOT NULL`": both are hybrid tables (`ExerciseCatalogEntity`/`PantryCatalogEntity`, since `mezo-52zg`) where `createdBy == null` marks master content upserted by the startup loader from a JSON seed, and `createdBy` set marks a user-authored row. Every row — master or user-authored — is readable by everyone; a user-authored row is editable only by its author or an OWNER. Reads never filter `created_by = currentUser` on these two tables; the composite index leads with the natural key (slug), not `created_by`, per [`liquibase_conventions.md`](../references/liquibase_conventions.md).
