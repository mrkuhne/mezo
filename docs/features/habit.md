...
related: [today, growth, me, fuel, train, _platform-data-layer, _platform-api-backend]
---
...
- **Afternoon (`delutan`)** → a single compact summary row (`🔁 Reggeli rutin {done}/{morning.length}` + `este: {done}/{evening.length}`) that links to `/me/growth` — the midday "not now" state.
...
- **Three evaluation-closure classes** (spec [D7](../superpowers/specs/2026-07-19-morning-evening-routine-habit-engine-design.md), the metric sets live in `HabitEvaluator`):
  - **INTRADAY** (`INTRADAY_METRICS` — M1 `sleep_wake_window`, M3 `weight_logged_before`, M4 `stim_intake_before`, M5 `training_done_today`, M6 `breakfast_protein`; MANUAL M2/E3 nominally here but skipped): re-evaluated on **every today-read** (`evaluateIntraday`); a `pending → done` transition on this read carries its level-up out on the response.
...
**Catalog** (`content/habit-catalog.json`, fail-fast `HabitCatalog` loader — the `QuestCatalog` pattern): **10 `HabitDef`s** (6 MORNING M1–M6 + 4 EVENING E1–E4). Each carries `key`, `chain`, `position` (stacking order), `title`/`why`/`anchorCopy` (all HU), `mode` (DERIVED|MANUAL), `metric`, `skillKey` (a LIFE skill), `skillKind` (`LIFE`), `xp` (5–15). The loader validates chain/mode membership, `skillKind == LIFE`, `5 ≤ xp ≤ 15`, and the **MANUAL ⇔ metric == "manual"** invariant — an out-of-band row fails startup. See spec §3 for the full 10-row table.
