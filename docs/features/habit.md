...
**Catalog** (`content/habit-catalog.json`, fail-fast `HabitCatalog` loader — the `QuestCatalog` pattern): **14 `HabitDef`s** (9 MORNING + 5 EVENING — the base 6+4, plus the two `mezo-a686` intention habits: `daily_intention` MORNING pos 9 / `intention_reflect` EVENING pos 5, plus the two habits inserted right after morning sunlight: `morning_pushups` MORNING pos 3 (MANUAL, `mindset`, 10 XP) / `morning_video` MORNING pos 4 (MANUAL, `learning`, 5 XP, carrying a `linkUrl`), all DERIVED unless noted). Each carries `key`, `chain`, `position` (stacking order), `title`/`why`/`anchorCopy` (all HU), `mode` (DERIVED|MANUAL), `metric`, `skillKey` (a LIFE skill), `skillKind` (`LIFE`), `xp` (5–15), and an **optional `linkUrl`** (an external resource opened straight from the row — currently only `morning_video`). The loader validates chain/mode membership, `skillKind == LIFE`, `5 ≤ xp ≤ 15`, and the **MANUAL ⇔ metric == "manual"** invariant — an out-of-band row fails startup. See the habit spec §3 for the base 10-row table + [intention.md §2](intention.md) for the two intention rows.
...
- **Backend** (`backend/src/main/java/io/mrkuhne/mezo/feature/habit/`): `HabitCatalog.java` + `content/habit-catalog.json` (14 habits — incl. the 2 `mezo-a686` intention habits + `morning_pushups`/`morning_video`; `HabitDef` carries the optional `linkUrl`) · `entity/HabitDayEntity.java` · `repository/HabitDayRepository.java` · `service/{HabitService,HabitEvaluator,HabitTargets,HabitJob}.java` (**`HabitTargets` is now a thin adapter over `feature/biometrics/sleep/service/SleepAnchorPort`** — `mezo-dbsr`; **`HabitEvaluator` reads the caffeine cutoff via `feature/fuel/service/CaffeineCutoffPort`** — `mezo-53su`) · `controller/HabitController.java` · `mapper/HabitMapper.java` · `config/HabitProperties.java` (**lost `defaultWake`/`defaultBed`** in `mezo-dbsr`, **and `caffeineCutoff`** in `mezo-53su`). Progression seam: `feature/progression/habit/HabitSignal.java` + `ProgressionService.{applyHabit,revertHabit}` (`SOURCE_HABIT`). Switches: `FeaturesConfiguration.{HABIT_SWITCH,HABIT_JOB_SWITCH}`.
