...
**Goal creation (`useGoalCreation`, `frontend/src/data/goalHooks.ts:103-124`, G4a):** the first goal **write** hook. Returns `{ submit, pending }`; `submit(input: GoalCreationInput, { onSuccess })` runs a `useMutation` whose `mutationFn` (real mode) runs the writes **in order**: `biometricProfileApi.upsert(input.profile)` (PUT) → `goalApi.create(input.goal)` (POST) → `if (input.activate) goalApi.activate(created.id)`, then `onSuccess` invalidates `['goals']`. **Mock mode no-ops** (returns `null`) so the wizard's `onSuccess(null)` still fires and navigates back. `GoalCreationInput = { profile: BiometricProfileUpsertRequest; goal: GoalUpsertRequest; activate: boolean }` is consumed verbatim by `GoalPlanner.save` (`goalHooks.ts:97-101`).
...
- `frontend/src/features/me/GoalPlanner.tsx` — the 3-step goal-creation wizard (G4a; `Step1`/`Step2` co-located), route `/me/goals/new`
