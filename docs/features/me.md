...
- **Goal command-center was issue `mezo-tji` (goal-system slice G4b)** — the `<GoalTimeline>` lane view replaced the "Cél alatt fut" `LinkedMesoCard`s; `EditGoalSheet` became a manage sheet (Archiválás + Törlés via `useGoalActions`); the hero reads the raw `GoalResponse` (trajectory/guards/window/weights), retiring `startDate`/`targetDate`/`unit` from `toGoal` (Decision C); and the **attach/detach hub** landed — `GoalPlanSlots` (Mesociklus + Futóblokk slots: ＋ Tervezd launches the planner, ＋ Csatolj meglévőt opens `AttachPlanSheet`) over `attachPlan`, plus the per-bar ✕ `detachPlan`. FE-only (reuses the G1 endpoints + G3 timeline). The `ambient` volleyball-band prop is intentionally omitted (the band renders constant), and the recept strip is a static G5 placeholder inside `GoalTimeline`.
- **Deferred to later goal-system slices (G4b-tail/G5):** an existing-goal **field editor** (the sheet's field rows are still read-only) + a fuller native `GoalResponse` render (more `toGoal` retirement as consumers migrate); the TDEE/prescription engine + real weight/goal trends (G5); a standalone `useBiometricProfile` read hook + profile editor. (Goal *creation* shipped in G4a; the **attach/detach hub** + timeline lane shipped in G4b; the Train→`Cél` mesocycle plan-links that fill `Goal.mesocycles` shipped in G3; the Súly-tab UI move shipped in G2.)
...
- `frontend/src/features/me/components/{WeightChart,SleepChart,SleepLogRow,TrendCell,SleepCell,FactorCard,InsightCard,GoalTimeline,GoalStat,LinkedMesoCard,RitualCard,PersonCard,MentionRow,RelationPatternCard,ConcentricAvatar,ProfileStat,EntryCard,TimePicker,...}.tsx` — view-local cards/charts (`GoalTimeline` = G4b goal lane view; `GoalPlanSlots` = G4b attach/detach hub slots under the timeline; `LinkedMesoCard` now only feeds `FuelStackView`)
