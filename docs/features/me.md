...
### `Cél` (`views/GoalsView.tsx`) — ✅ goal/strategy + goal creation (G4a)
The long-term goal & strategy surface — **no daily weight log here anymore** (that moved to `Súly`, G2). A tappable goal hero (kg progress track start→target, tempo/projection stats, identity frame) opens **`EditGoalSheet`** — **display-only by deliberate decision** (the existing-goal *edit* writer is still G4b; `EditGoalSheet` does not yet POST). In real mode the hero renders the **server's active goal** (mapped via `toGoal`); in mock it is the static `mockGoal`. **Empty-state guard (`GoalsView.tsx:39-58`, `mezo-72d`):** when `useGoal()` returns `goal === null` (real mode, no active goal), `GoalsView` short-circuits to a `GhostState` "set up a goal" card with a `＋ Új cél` CTA (→ `/me/goals/new`) **before** any `goal.X` read — real users with no goal no longer see the `mockGoal` placeholder hero (only mock mode keeps showing the demo hero). Below the hero: Mezo `InsightCard`s (`weightTrends.insights`), `FactorCard`s + a decorative `ToolChipRow`, and linked-mesocycle cards (`LinkedMesoCard`) — **real in real mode since G3** (built from the goal's plan-link timeline; static `mockGoal.mesocycles` in mock). `GoalsView` calls `useGoal()` (goal + linkedMesocycles) and `useWeight()` **only for `weightTrends`** (the hero's tempo/projection stats) — `GoalsView.tsx:30-31`. The `WeightChart`/`TrendCell`/`WeightLogSheet`/period-toggle it used to own are now in `WeightView`.
...
- **`toGoal(res, weightLog)`** is a **back-compat adapter**: maps the new `GoalResponse` onto the existing `Goal` domain type so `GoalsView` stays untouched (G4 restructures it). Notable mappings: trajectory `maintain → 'maintenance'` `GoalKind`; `rateTargetPctPerWeek` kept as a raw `%` magnitude with unit `'%/hét'` + arrow derived from trajectory (legacy mock was kg/hét — G4 reworks this panel); `mesocycles: []` from `toGoal` itself (then **filled from the timeline** — next bullet).
- **Real linked plans (G3):** a third `useQuery(['goal', goalId, 'timeline'], () => goalLinkApi.timeline(goalId), { enabled: !mock && !!goalId })` fetches the active goal's plan-link timeline; `toLinkedMesocycles(timeline)` maps each `GoalTimelineResponse.links[i].plan` (`GoalPlanRef`) into the legacy `LinkedMeso` (`plan.title→shortTitle`, `plan.status→status`, `huMonthDay(plan.startDate/endDate)`, `plan.weeks`), and `goal.mesocycles = links.map(l => l.planId)`. So `GoalsView`'s "Cél alatt fut" `LinkedMesoCard`s render **real** links with no GoalsView change.
- Returns `{ goal, linkedMesocycles }` where **`goal: Goal | null`** — in real mode both are server-derived (`goal === null` + `linkedMesocycles === {}` when there is no active goal); mock keeps `mockGoal` + the static `linkedMesocycles`. Consumers that read `goal.X` (`GoalsView`, `WeightView`) must null-guard.
...
`Goal.mesocycles: string[]` holds plan IDs resolved via `linkedMesocycles` and rendered by `LinkedMesoCard`. **Real since G3:** `useGoal` fetches `goalLinkApi.timeline(goalId)` (`GET /api/goals/{id}/timeline`) and builds both `goal.mesocycles` (= `links.map(l => l.planId)`) and `linkedMesocycles` (`Record<planId, LinkedMeso>` from each `link.plan`). In **mock mode** they remain the static strings + `data/goals.ts` `linkedMesocycles`. **Contract crossing the seam:** `GoalTimelineResponse` (`links[]` of `GoalPlanLinkResponse`, each with an embedded `GoalPlanRef` plan summary).
...
The **Goal backend is live** (G1): contract `api/feature/goal/goal.yml`, `feature/goal/` package, `goalApi` client (`frontend/src/lib/goalApi.ts` — `list/get/create/update/remove/activate/archive`), a real read-path in `useGoal()`, and (G4a) the first **write** via `useGoalCreation` (`goalHooks.ts:103`, POST `create` + `activate` + `biometricProfileApi.upsert`) driving the `GoalPlanner` wizard. What remains for **G4b** is *editing an existing goal*: make `EditGoalSheet` *write* (add an `updateGoal`/`activate`/`archive` mutator to `useGoal()`, dual-mode, mirroring `useGoalCreation`), wire the sheet, and restructure `GoalsView` to render the real `GoalResponse` natively (dropping the `toGoal` back-compat adapter), plus the timeline-lane UI + attach/detach hub. G3 wired the plan-links that fill `Goal.mesocycles`; G5 adds the TDEE/prescription engine. (The Súly-tab UI move shipped in G2 — `WeightView` at `/me/weight`.) The **`biometric_profile`** is now written through the wizard (`biometricProfileApi.upsert`); a standalone `useBiometricProfile()` read hook + profile editor is still a later slice.
...
- `data/goalHooks.test.tsx` (G1+G3, `mezo-72d`): asserts `useGoal` picks the active goal from `GET /api/goals` (MSW) and that `toGoal` maps `GoalResponse → Goal` (currentWeight from the latest weight entry, trajectory→kind); a second test stubs `GET /api/goals/g1/timeline` with one mesocycle link and asserts `goal.mesocycles === ['meso-1']` + `linkedMesocycles['meso-1']` is built from the link's plan; an empty-state test (`GET /api/goals → []`) asserts `goal === null` + `linkedMesocycles === {}`.
- `data/meData.test.tsx`, `meData2.test.tsx`, `meHooks.test.tsx`: mock-data shape/hook tests (profile, people).
- Per-sheet: `WeightLogSheet.test.tsx`, `SleepLogSheet.test.tsx`, `EditGoalSheet.test.tsx`, `PersonLogSheet.test.tsx`, `PersonDetailSheet.test.tsx`, `SettingsSheet.test.tsx`. Per-view: `WeightView.test.tsx` (G2 — Súly header + trend cells + log CTA opens `WeightLogSheet`), `GoalsView.test.tsx` (mock-mode: hero/factors/linked-mesos + trend cells **absent** since G2; real-mode no-goal: the `Még nincs aktív célod` `GhostState` + `＋ Új cél` CTA render, **not** the `mockGoal` placeholder — `mezo-72d`), `SleepView.test.tsx`, `ProfileView.test.tsx`, `PeopleView.test.tsx`. Plus `MeScreen.test.tsx`, `MeSubNav.test.tsx` (asserts the `Súly` tab links to `/me/weight`), `TimePicker.test.tsx`, shared-card tests, and `app/navigation.test.tsx` (exercises `/me` + the settings theme toggle).
...
- **Deferred to later goal-system slices (G4b/G5):** editing an existing goal (writable `EditGoalSheet`) + native `GoalResponse` rendering in `GoalsView` (dropping `toGoal`) (G4b); the TDEE/prescription engine + real weight/goal trends (G5); a standalone `useBiometricProfile` read hook + profile editor; the timeline-lane UI + attach/detach hub (G4b). (Goal *creation* shipped in G4a; the Train→`Cél` mesocycle plan-links that fill `Goal.mesocycles` shipped in G3; the Súly-tab UI move shipped in G2.)
...
- `frontend/src/data/goalHooks.ts` — `useGoal` (real active-goal read + real plan-link timeline; `toGoal` + `toLinkedMesocycles` adapters) + `useGoalCreation` (G4a write chain at :103; `GoalCreationInput` at :97)
