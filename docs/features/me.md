...
`Goal.mesocycles: string[]` holds plan IDs resolved via `linkedMesocycles`; **since G4b the `GoalsView` lane is `<GoalTimeline>`** (consumes the raw `timeline.links[]` for start/end-week positions — `LinkedMeso` has no lane data — and `detachPlan` for the ✕ affordance), while `FuelStackView` still reads `linkedMesocycles` for the active meso title. **Real since G3:** `useGoal` fetches `goalLinkApi.timeline(goalId)` (`GET /api/goals/{id}/timeline`) and builds `goal.mesocycles` (= `links.map(l => l.planId)`), `linkedMesocycles` (`Record<planId, LinkedMeso>` from each `link.plan`), and exposes the raw `timeline`. In **mock mode** they come from `data/goals.ts` (`linkedMesocycles` + the static `goalTimeline`). **Contract crossing the seam:** `GoalTimelineResponse` (`links[]` of `GoalPlanLinkResponse`, each with an embedded `GoalPlanRef` plan summary; plus `gaps[]`).
