...
- **Deferred to later goal-system slices (G5b/Phase-3):** an existing-goal **field editor** (the sheet's field rows are still read-only) + a fuller native `GoalResponse` render (more `toGoal` retirement as consumers migrate); the **adaptive** TDEE leg (back-calc from Fuel intake + the EWMA blend — needs Fuel Slice C) + the protein guard going live (`proteinMonitored`); a standalone `useBiometricProfile` read hook + profile editor. (Goal *creation* shipped in G4a; the **attach/detach hub** + timeline lane shipped in G4b; the Train→`Cél` mesocycle plan-links that fill `Goal.mesocycles` shipped in G3; the Súly-tab UI move shipped in G2.)
