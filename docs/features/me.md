...
**Goal creation (G4a, reshaped to 2 steps in G6 `mezo-06n`):** a `+ Új cél` chip in the `GoalsView` header (`GoalsView.tsx:49-51`) `navigate`s to the sibling route `/me/goals/new` → **`GoalPlanner`** (`frontend/src/features/me/GoalPlanner.tsx`), a **2-step** wizard (`STEP_COUNT = 2`):
