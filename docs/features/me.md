...
|---|---|---|---|---|
| `Profil` | `/me` (index) | ✅ | ✅ **Biometria card + editor** (G6) — `useBiometricProfile` reads `biometric_profile`; the rest of `Profil` is mock | ✅ `biometric_profile` (G1) + derived `tdeeBootstrap` (G6) |
| `Cél` (goal/strategy) | `/me/goals` (+ `/me/goals/new` wizard) | ✅ | ✅ active goal + **goal creation** (G4a) — "Új cél" **hard-gated** on a complete biometric profile (G6, `GoalGate.tsx`), the gate also enforced at the **route level** (`GoalPlanner.tsx:37-42` redirects `/me/goals/new` → `/me/goals` via `<Navigate replace>` when the profile is incomplete) | ✅ `goal` (G1) + `goal_plan_link` links (G3); insights/factors still mock |
...
The daily body-weight log + trend, split out of `Cél` so weight is a first-class tab (`/me/weight`). A daily-log hero shows the latest weight number + 7-day rate/avg and a **"Súly naplózása"** CTA that opens **`WeightLogSheet`** (±0.1/±0.5 stepper, note ≤200 chars, a contextual "mezo observation" line) → `logWeight`. A period toggle (`7d` / `30d` / `all`) feeds **`WeightChart`** (start/target reference lines from the goal — see below). Below: two **`TrendCell`**s (7d / 4w). `WeightView` calls `useWeight()` (`weightLog`/`weightTrends`/`logWeight`) and `useGoal()` **only for the chart's `startWeight`/`targetWeight` reference lines + a `currentWeight` fallback when `weightLog` is empty** (`WeightView.tsx:14-15,19-22,66`) — the same goal-coupling pattern flagged in `mezo-4nu` (the target line on the weight chart is a real product feature). **Null-safe since `mezo-72d`:** `useGoal()` now returns `goal === null` in real mode when no goal exists, so `WeightView` falls back to the latest logged weight for the hero number and the chart's start/target reference lines (`chartStart`/`chartTarget`, `WeightView.tsx:19-22`); the chart still renders (flat reference lines) without a goal.
...
`data/me.ts` (profile cards/settings), `data/people.ts` (`peopleSummary`, `people`, `initialMentions`, `relationPatterns`, plus `affectColor`/`affectLabel` helpers), and the still-mock `weightTrends`/`linkedMesocycles` + the `mockGoal` (`data/goals.ts`, used by `useGoal` **only in mock mode** now — `mezo-72d` dropped the real-mode no-goal fallback, which returns `null` instead) and `sleepTrends` (`data/sleep.ts`). When People graduates to Phase-2, `initialMentions`/`people` are where the backend will plug in (see §7).
...
`Goal.mesocycles: string[]` holds plan IDs resolved via `linkedMesocycles`; **since G4b the `GoalsView` lane is `<GoalTimeline>`** (consumes the raw `timeline.links[]` for start/end-week positions — `LinkedMeso` has no lane data — and `detachPlan` for the ✕ affordance), while `FuelStackView` still reads `linkedMesocycles` for the active meso title. **G4b also wires the attach direction (`GoalPlanSlots`/`AttachPlanSheet`):** the slots launch the Train planners (meso `/train/mesocycles/new`; run create-then-navigate to `/train/futas/:id`) and the attach sheet reads the Train plan lists (`useTrain().mesocycles` / `useRunning().runningBlocks`) to attach an owned plan to the goal (`attachPlan` → POST `/api/goals/{goalId}/plans`), excluding plans already in `timeline.links[]`. **Real since G3:** `useGoal` fetches `goalLinkApi.timeline(goalId)` (`GET /api/goals/{id}/timeline`) and builds `goal.mesocycles` (= `links.map(l => l.planId)`), `linkedMesocycles` (`Record<planId, LinkedMeso>` from each `link.plan`), and exposes the raw `timeline`. In **mock mode** they come from `data/goals.ts` (`linkedMesocycles` + the static `goalTimeline`). **Contract crossing the seam:** `GoalTimelineResponse` (`links[]` of `GoalPlanLinkResponse`, each with an embedded `GoalPlanRef` plan summary; plus `gaps[]`).
...
Notes:
- **`useGoals()` no longer exists** — it was split into `useGoal()` (goal + `linkedMesocycles`) and `useWeight()` (`weightLog`/`weightTrends`/`logWeight`). A view that needs both calls both (e.g. `WeightView` calls `useWeight` for the log/trend and `useGoal` for the chart's reference lines; `GoalsView` calls `useGoal` for the hero and `useWeight` only for `weightTrends`). `useGoal()` is **read-only**; goal **creation** is a separate hook — **`useGoalCreation()`** (G4a, `{ submit, pending }`, drives `GoalPlanner`). Editing an *existing* goal in place is still G4b.
- `useProfile()` returns `{ user, identityGoal, areas, quickSettings, notifSettings, version }` (all static). The **biometric profile is a separate hook** — `useBiometricProfile()` (read, 404→null, `isComplete`) + `useBiometricActions()` (upsert), both from `@/data/hooks` (G6, §3) — consumed by `ProfileView`'s Biometria card/sheet (and reused by the goal-creation hard gate).
...
- `WeightView` tolerates an empty `weightLog` because `WeightChart` handles `[]` and the hero falls back to `goal?.currentWeight ?? 0` (goal may be `null` since `mezo-72d`); `SleepView` must guard `lastNight` explicitly. `GoalsView` guards `goal === null` up front (empty-state CTA).
...
- `frontend/src/features/me/views/{ProfileView,GoalsView,WeightView,SleepView,PeopleView,KnowledgeView}.tsx` — the six views (`GoalsView` = goal/strategy + `+ Új cél` entry, `WeightView` = daily weight log + trend, G2)
- `frontend/src/features/me/GoalPlanner.tsx` — the 2-step goal-creation wizard (G4a; reshaped to 2 steps + the live `FeasibilityPanel` in G6 `mezo-06n`; `Step1` + `FeasibilityPanel` co-located), route `/me/goals/new`
- `frontend/src/app/router.tsx:38-41,90-102` — Me routes (`goals`→`GoalsView`, `weight`→`WeightView`) + the sibling `me/goals/new`→`GoalPlanner` (`router.tsx:102`) + `ProfileRoute` wrapper
...
- `frontend/src/features/me/components/{WeightChart,SleepChart,SleepLogRow,TrendCell,SleepCell,FactorCard,InsightCard,GoalTimeline,GoalRecept,GoalStat,LinkedMesoCard,RitualCard,PersonCard,MentionRow,RelationPatternCard,ConcentricAvatar,ProfileStat,EntryCard,TimePicker,...}.tsx` — view-local cards/charts (`GoalTimeline` = G4b goal lane view; `GoalRecept` = G5 segmented-recept + verdict + guard-status card / evaluate CTA; `GoalPlanSlots` = G4b attach/detach hub slots under the timeline; `LinkedMesoCard` now only feeds `FuelStackView`)

**Frontend — data layer**
- `frontend/src/data/hooks.ts` — the boundary (`useProfile`:75, `useSleep`:79, `usePeople`:106; re-exports `useWeight`:179, `useGoal`/`useGoalCreation`/`useGoalActions`/`useFeasibilityPreview`:180, `useBiometricProfile`/`useBiometricActions`:181)
- `frontend/src/data/weightHooks.ts` — `useWeight` (weight log+trend, dual-mode)
- `frontend/src/data/goalHooks.ts` — `useGoal` (real active-goal read + raw `GoalResponse`/`timeline` + plan-link timeline; thin `toGoal` + `toLinkedMesocycles` adapters), `useGoalActions` (G4b — archive/remove/activate/attach/detach, dual-mode), `useGoalCreation` (G4a write chain; `GoalCreationInput`)
- `frontend/src/data/{me,goals,sleep,people}.ts` — mock data (`mockGoal` + `mockGoalResponse` + `linkedMesocycles` + the static `goalTimeline` in `goals.ts`) + people affect helpers
...
- `frontend/src/lib/goalLinkApi.ts` — `goalLinkApi` (`timeline`/`attach`/`detach`; G3 — `useGoal` consumes `timeline`)
- `frontend/src/lib/biometricProfileApi.ts` — `biometricProfileApi` (`get` [404→null] / `upsert`); consumed by `biometricHooks.ts`
- `frontend/src/data/biometricHooks.ts` — `useBiometricProfile` (read, 404→null, `isComplete`) + `useBiometricActions` (upsert → invalidate `['biometricProfile']`+`['goals']`) (G6)
...
- `frontend/src/features/me/BiometricSheet.tsx` — the biometric editor sheet (Nem/Magasság/szül.dátum/Testzsír/Aktivitás → `upsert`) (G6)
- `frontend/src/features/me/biometricFields.ts` — shared `ACTIVITY_LEVELS`/PAL + `ageFromBirthDate`/`palLabel` helpers for the card + sheet (G6)
- `frontend/src/lib/mode.ts` (`isMockMode`), `frontend/src/lib/api.ts` (`apiFetch`)
- Consumers: `frontend/src/features/me/views/WeightView.tsx:14-15` (`useWeight` log/trend + `useGoal` chart reference lines), `frontend/src/features/me/views/GoalsView.tsx:45-48` (`useGoal` hero + `useGoalActions` + `useWeight` for `weightTrends` + `useBiometricProfile().isComplete` for the goal-creation gate), `frontend/src/features/fuel/views/FuelStackView.tsx` (`useGoal().linkedMesocycles`)
