...
| `useGoal` (read-only) | Goal (`Cél`) | ✅ **dual-mode read, real backend done** (goal-system G1 `mezo-2hp` + G3 `mezo-3sc`). Hook `data/goalHooks.ts`, clients `lib/goalApi.ts` + `lib/goalLinkApi.ts`. Reads the active goal **and its real linked plans** (G3 timeline). **Editing** an existing goal is still G4b; goal **creation** ships as a separate write hook ↓. |
| `useGoalCreation` (`submit`/`pending`) | Goal creation (`Cél`) | ✅ **dual-mode write, real backend done** (goal-system G4a `mezo-pqt`). Hook `data/goalHooks.ts:103`. The first goal **write** path: `submit` runs a `useMutation` that (real) `biometricProfileApi.upsert` → `goalApi.create` → optional `goalApi.activate`, then invalidates `['goals']`; **mock no-ops** (resolves `null` so the wizard navigates back). Drives the `GoalPlanner` wizard. |
...
- **Read-only, with a derived/mapped result** (no mutation): `useGoal` (`data/goalHooks.ts:54`, G1+G3) reads `['goals']` (`queryFn = mock ? () => null : goalApi.list`), picks the active goal, and maps `GoalResponse → Goal` via `toGoal` so the unchanged `GoalsView` consumes the legacy domain shape. It **also reads `['weightLog']` with no `queryFn`** — deliberately sharing the cache `useWeight` owns — to derive `currentWeight` from the latest weight entry without a second fetch. Two queries on one cache key (one owns the fetch, the other rides it) is the idiom G1 introduces. **G3 adds a third query** `['goal', goalId, 'timeline']` (`queryFn = goalLinkApi.timeline`, `enabled: !mock && !!goalId`) — its `links` are mapped to the legacy `linkedMesocycles` (`Record<planId, LinkedMeso>`) and `goal.mesocycles` (`toLinkedMesocycles`, `data/goalHooks.ts:39`), so the goal hero renders **real** linked plans instead of the mock map. Mock mode still returns the static `linkedMesocycles` + `mockGoal.mesocycles`.
- **Sequential multi-write mutation** (G4a): `useGoalCreation` (`data/goalHooks.ts:103`) is the data layer's first **chained** write — its `mutationFn` awaits three calls **in order** (`biometricProfileApi.upsert` → `goalApi.create` → optional `goalApi.activate`) before `onSuccess` invalidates `['goals']`. Mock mode returns `null` (no-op). Exposes `{ submit, pending }` and takes a per-call `{ onSuccess }` so the `GoalPlanner` wizard can navigate back; the input is the typed `GoalCreationInput = { profile: BiometricProfileUpsertRequest; goal: GoalUpsertRequest; activate: boolean }` (`goalHooks.ts:97`).
...
- **Me ← biometrics / goal**: `GoalsView` (`useGoal` for the goal + `useWeight`/`logWeight` for the log — it calls both, since G1 split `useGoals`), `SleepView` (`useSleep`/`logSleep`), `FuelStackView` (`useGoal().linkedMesocycles`), `ProfileView`/`PeopleView`/`KnowledgeView` (`useProfile`/`usePeople`/`useKnowledge`, all mock). **Contract:** `WeightEntry[]`, `SleepEntry[]`, `CheckinSlot[]`, and the `Goal` domain shape (mapped from `GoalResponse`) from `data/types.ts`.
...
- `frontend/src/data/goalHooks.ts` — `useGoal` (G1+G3; real active-goal read, `toGoal` back-compat adapter at :15, shares the `['weightLog']` cache; G3 `toLinkedMesocycles` at :41 builds real `linkedMesocycles` from the `goalLinkApi.timeline` query) + `useGoalCreation` (G4a; chained profile-upsert→create→activate write at :103, `GoalCreationInput` at :97).
