...
| `useToday`, `useProfile`, `useFuelPreview`, `useFuelTimeline`/`useFuelWeek`, `useStack`, `useProtocol`, `useReplanScenarios`, `useStackRecommendations`, `useInsights`, `useKnowledge`, `useChat`, `usePeople` (`logMention`) | Today, Me/Profile, **Fuel**, **Insights**, **People** | 🔶 **MOCK-ONLY.** Plain static returns or local `useState`; no API client, no `isMockMode` branch, no backend table. (The Fuel **meal / water / recipe / medication** hooks — `useFuelDay`/`useMealActions`/`useWaterActions`/`useRecipeLogs`/`useRecipes`/`useMedication` — are already **dual-mode / real** since mezo-arb/lns/d94/0z5; see `fuel.md`.) Remaining Fuel sub-slices (stack/protocol/week) → D (Insights seed) → E (People) remain (`docs/milestones/roadmap.md`). |
