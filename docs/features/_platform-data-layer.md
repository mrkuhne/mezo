...
- `questApi` (`data/quest/questApi.ts`, gamified growth E1 `mezo-df7q`): `GET /api/quest/day/{date}` (`day` — quests + `levelUps` + `rerollsLeft`), `POST /api/quest/{id}/reroll` (`reroll`), **+ `GET /api/quest/history?from&to` (`history`, `mezo-rmhr`)**. Consumed by `useDailyQuests`/`useQuestActions` + **`useQuestHistory`** (`data/quest/questHooks.ts`) — the day read is a manual dual-mode shape (not `useDualQuery`: it carries side-channel fields beside the list), the history read is `useDualQuery`, real mode returns the empty day/list while unresolved. See [`growth.md`](growth.md).
- `activityApi` (`data/activity/activityApi.ts`, gamified growth E2 `mezo-jzca`): `GET /api/activity/day/{date}` (`day`), `POST /api/activity` (`create`), `POST /api/activity/{id}/category` (`categorize`), **+ `GET /api/activity/history?from&to` (`history`, `mezo-rmhr`)** — each write returns `ActivityWriteResult{entry, completedQuest?, levelUps[]}`. Consumed by `useActivities` (`useDualQuery`, `['activities', date]`) + `useActivityActions` + **`useActivityHistory`** (`data/activity/activityHooks.ts`); the write invalidates `['activities', date]` + `['dailyQuests', date]` + `['progressionProfile']`. Mock mode has a deterministic classifier. See [`growth.md`](growth.md).
- `progressionApi` (`data/progression/progressionApi.ts`): `GET /api/progression/profile` (`getProfile` — P4) **+ `GET /api/progression/achievements` (`getAchievements`, `mezo-rmhr`)**. Consumed by `useProgressionProfile` + **`useAchievements`** (`data/progression/progressionHooks.ts`, both `useDualQuery`) — the Profil `GrowthSummaryCard` + the `/me/growth` page. See [`growth.md`](growth.md) / [`me.md`](me.md).
...
- `pantryApi` (`data/fuel/pantryApi.ts`, Fuel slice C `mezo-9xu`): `GET /api/pantry` (the kind-split `PantryResponse` → `{ ingredients, stash }`), `POST /api/pantry`, `PUT/DELETE /api/pantry/:id`, + since Fuel P6 (`mezo-bka`) `GET /api/pantry-import/lookup` and `POST /api/pantry-import`, + since Fuel P8 (`mezo-8vum`) `POST /api/pantry-import/scrape`. Types `PantryResponse`/`PantryItemRequest`/`PantryItemResponse`/`IngredientResponse`/`SupplementStashResponse` + `PantryLookupResponse`/`PantryImportRequest`/`PantryImportEntryResponse`/`PantrySuggestionResponse` + `PantryScrapeRequest`/`PantryScrapeResponse`/`PantryScrapeResult`. (`usePantry` consumes `list`; `usePantryActions` consumes `create`/`update`/`remove`/`importItem`/`lookup`/`scrape`.)
