...
| `useGoal` (read-only) | Goal (`Cél`) | ✅ **dual-mode read, real backend done** (goal-system G1 `mezo-2hp` + G3 `mezo-3sc`). Hook `data/goalHooks.ts`, clients `lib/goalApi.ts` + `lib/goalLinkApi.ts`. Reads the active goal **and its real linked plans** (G3 timeline); **no mutation yet** (writable editor + attach/detach UI are G4). |
...
- `frontend/src/data/goalHooks.ts` — `useGoal` (G1+G3; real active-goal read, `toGoal` back-compat adapter at :13, shares the `['weightLog']` cache; G3 `toLinkedMesocycles` at :39 builds real `linkedMesocycles` from the `goalLinkApi.timeline` query).
