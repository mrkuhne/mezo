...
**`"Kamra"` (`FuelKamraView`):** the pantry, **redesigned to Direction A** (`docs/design/kamra-mockup-v3-A.html`, mezo-9xu follow-up). `buildKamraItems(ingredients, stash)` produces a unified `PantryItem[]` (food + supplements + stimulants + medication). The primary axis is a **type segmented switcher** (`Mind/Étel/Supp/Stim` with live counts); a trimmed **source filter** (`Minden forrás`/`Saját`) + search refine it; items render in **type-grouped sections** (a colored square nub divider per kind) as lighter design-system **meal-card** `KamraCard`s (44px stock slot · Antonio name · source/brand · `P/C/F · NOVA · lejár` · right kcal/dose; supplement/stim get a left inset tint). A 4-cell stats strip (total / food / low-expiry / low-stock) + a **needs-attention strip** (when something expires < 3 days) sit above; an **empty-state** CTA shows when the pantry is empty (real mode starts empty). The view renders directly into the app-shell `.screen-content` — the old nested `screen-content` + `paddingTop:96` wrapper double-offset the scrollport and left a large top gap (removed). The page-header **`＋ Új tétel`** chip opens **`AddPantryItemSheet`** (manual add). **Placeholder/mock UI removed** (returns with the real features in later slices): the scrape-feed card + `ImportItemSheet`, the `SuggestionCard` feed (`pantrySuggestions`), and the scrape-vendor source filters — `imports`/`suggestions` are already `[]` in real mode. A card opens **`IngredientDetailSheet`** (macro hero OR supplement protocol, micro-density bars, stock+price, used-in-recipes list) whose actions are now **real CRUD** (mezo-9xu): **`Szerkesztés`** reopens `AddPantryItemSheet` in edit mode (`editId` + an `initial` derived by `inputFromItem`), **`Frissítés · +1 polcra`** is a quick `updateItem` stock bump, **`Törlés a kamrából`** is `deleteItem` + close; **`Logolás`** stays disabled (no logging slice). **`AddPantryItemSheet`** (real `Sheet` shell) is the manual add/edit form: kind toggle (`food`→`kcal` / others→`dose`) + name, calling `usePantryActions().addItem`/`updateItem`. (**`ImportItemSheet`** — the 3-phase scrape wizard with a hardcoded `"Görög joghurt 10%"` preview — still exists but is no longer mounted; it returns when scrape/import lands.)
...
- Sheet tests: `MealScoreSheet`, `ReplanSheet`, `RecipeDetailSheet`, `NewRecipeSheet`, `StackPickerSheet`, `ImportItemSheet`, `IngredientDetailSheet`, `AddPantryItemSheet` (asserts a real `usePantry` cache append on submit), `GymScheduleSheet`.
...
- **Sheets** (`features/fuel/`): `MealScoreSheet.tsx`, `ReplanSheet.tsx`, `RecipeDetailSheet.tsx`, `NewRecipeSheet.tsx`, `IngredientPickerSheet.tsx`, `StackPickerSheet.tsx`, `ImportItemSheet.tsx`, `IngredientDetailSheet.tsx`, `AddPantryItemSheet.tsx` (manual add/edit form → `usePantryActions`), `GymScheduleSheet.tsx`.
