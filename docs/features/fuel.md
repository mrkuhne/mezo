...
**`"Kamra"` (`FuelKamraView`):** the pantry. `buildKamraItems(ingredients, stash)` produces a unified `PantryItem[]` (food + supplements + stimulants + medication). Type filters (`all/food/supplement/stim/med`) + source filters (`all/kifli.hu/myprotein.hu/tesco.hu/manual`) + search; cards grouped by category. Stats: total items, food count, low-expiry, low-stock. The page-header **`＋ Új tétel`** chip opens **`AddPantryItemSheet`** (manual add); the scrape-feed card (last 3 imports) opens the **`ImportItemSheet`** scrape wizard; `SuggestionCard`s surface `pantrySuggestions`. A card opens **`IngredientDetailSheet`** (macro hero OR supplement protocol, micro-density bars, stock+price, used-in-recipes list, scrape footer) whose actions are now **real CRUD** (mezo-9xu): **`Szerkesztés`** reopens `AddPantryItemSheet` in edit mode (`editId` + an `initial` derived by `inputFromItem`), **`Frissítés · +1 polcra`** is a quick `updateItem` stock bump, **`Törlés a kamrából`** is `deleteItem` + close; **`Logolás`** stays disabled (no logging slice). **`AddPantryItemSheet`** (real `Sheet` shell) is the manual add/edit form: kind toggle (`food`→`kcal` / others→`dose`) + name, calling `usePantryActions().addItem`/`updateItem`. **`ImportItemSheet`** is a 3-phase scrape wizard (input → scraping with auto-advancing steps → preview of a **hardcoded fixture** `"Görög joghurt 10%"`); `"Polcra"` just closes (scrape persistence still deferred).
