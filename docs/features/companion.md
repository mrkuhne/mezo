...
| Shared chat retrieval rollout | ✅ `mezo-6dii.6` · **production runs `NEW` since 2026-09-17 (`mezo-i7x5v`)** | One adapter feeds sync and SSE. `OLD` serves the frozen facts + ambient + graph path; `SHADOW` serves the identical payload and audits unified retrieval asynchronously; `NEW` serves the hybrid `MemoryContext` and falls back to OLD only on an audited total retriever outage. `application.yml` keeps `SHADOW` as the default — production sets `MEZO_MEMORY_SERVING_MODE=NEW` on the Deployment, so dev/CI/tests are untouched and the rollback is one deleted env entry. Decided on the `mezo-6dii.9` measurement (recall@5 0.155 → 0.837, invented-memory rate 0.833 → 0.000, p95 +8 ms), whose aggregate FAIL the product owner deliberately overrode: [`2026-09-17-memory-retrieval-gemini-eval.md`](../research/raw/measurements/2026-09-17-memory-retrieval-gemini-eval.md). Not decidable before `mezo-iddo` — until then the platform's dense retriever timed out on 55 of 55 production runs. |
...
| Karakter contextual self-report | mezo-njcgs | `CharacterReplyMemorySource` implements `NarrativeNoteSource` for `character_reply`. Durable replies are projected immediately by the reply worker and recovered by the same nightly note catch-up (including replies whose evaluation failed) seam. Source context and user text remain explicitly attributed as self-report; evaluation failure does not erase that evidence. Recent replies also enter the character prompt source. See [character.md](character.md). |
...
- **The `[Észrevételek]` chat block.** `ReflectionPromptBlock.render(userId)` renders the five
  ...
  `ChatService.turnContext` (the volatile half since `mezo-ozri.5`) right after the fresh-pattern-facts block. A companion that
...
- **Evidence is served structured, not as a stored label (mezo-d6ivw.1).** `ObservationResponse.evidence` is `ObservationEvidenceItem[]` — `{type: record|tag, source?, date?, time?, fields?, quote?, ref?, text?}`. For a `grounded`-channel event's canonical refs, `ObservationFeedService.evidenceItems` re-reads each ref through `ObservationContextService.fetch` (§ above — a lossless structured re-read of the current source, bypassing the possibly-truncated label persisted at generation time) and emits a `record` item; a ref that no longer resolves (deleted/foreign source) falls back to a `tag` carrying the stored label that followed it in the event payload when one was persisted (event snapshots interleave `ref, label` pairs), or is dropped otherwise. A non-canonical entry (`observation-topic:` markers dropped; anything else) is a plain `tag`. A `watching`/`confirmed` ROW card's evidence is additionally capped to its newest `ROW_EVIDENCE_LIMIT` (5) `record` items — the publisher keeps APPENDING refs to a row on every merge, and the row card opens its evidence by default, so an unbounded read would render an ever-growing list; new refs land at the end, so the cap keeps the tail. Event cards render one event's own refs and stay uncapped. Legacy (non-`grounded`) events' refs go through `legacyItems`: `QuickNoticeService` puts a canonical-shaped ref first (e.g. `journal_entry:<uuid>`), so each canonical-shaped entry is re-read the same way a grounded ref is and becomes a `record` — but not every legacy source is in the re-read catalogue (`gratitude` is not; `gratitude_entry` is the catalogue name), so an unresolvable canonical-shaped ref is DROPPED silently rather than shown as raw machine text. Free (non-canonical) text still passes through verbatim as a `tag`. FE consumption: `shared/ui/evidence/observationEvidence.ts`'s `mapEvidence` renders a `record` into an icon/title/value-pill/quote row and anything else into a plain tag; see [`today.md`](today.md) §2 and [`insights.md`](insights.md) §3 for the two renderers (`EvidenceList`, shared by the Today card and the team feed observation post).
...
batch cannot hide a newly requested fact. Caps: five batches and 15 total reads; repeated identical reads do not
...
`TurnEmbeddingListener` and `FactExtractionListener` bind `ChatTurnCompleted.userId` inside their
async work. This keeps per-user cost attribution across thread boundaries without leaving an actor
...
The restored evidence is read from `tool_outcomes.outcomes[].text` — the ONE storage place for
tool result text (mezo-rj214.10 unification, §6.6) — while each entry's name and args come from the
positionally parallel `tool_calls.calls[]` ask half, zipped BY INDEX and tolerant of a null side or
a length mismatch. `TurnProvenance.build` writes both halves under the same bounded budget (8,000
characters per result, 40,000 per assistant row, plus omission markers and call metadata). The
consequence of the 90-day retention decision, and the intended one: once `ProvenanceRetentionJob`
NULLs an old row's `tool_outcomes`, that row replays its text but NO tool evidence — nothing is
reconstructed from the ask half, because an ask without its result is not evidence. REST tool chips
remain `{type,name}`. Legacy JSON, and the rows from the brief window when result text lived inside
`tool_calls`, still load. The recent transcript labels restored evidence with its timestamp and as historical data;
...
`GoalTools`/`MedicationTools`/`MemoryTools` — the V0.5–V2.3 batch — the mezo-xixu trio
`GrowthTools`/`PracticeTools`/`InsightsTools` — plus `LifeGoalTools`, mezo-iizd.10) via `ToolCallbacks.from`
...
marks "at" as a preposition instead. No data reference at all ⇒ `CHAT`; a data reference with no
question shape returns `Optional.empty()` — UNSURE, not `CHAT`. `TurnGearRouter` resolves an UNSURE
result through `GearClassifier` (`service/GearClassifier.java`) — one tool-free, history-less cheap
call (`GearClassifier.PROMPT`) asking the model to answer with exactly one of the three words — only
when `mezo.companion.turn.gear.classifier-enabled` is true (§4); the classifier fails OPEN (any
...
**One storage place (mezo-rj214.10 unification).** Tool RESULT text is persisted in exactly one
column, `ai_message.tool_outcomes`, and only by `TurnProvenance.build`. `tool_calls` carries the
...
the conversation-history replay's measured values. `TurnProvenance` reads those keys directly;
`mezo.companion.turn.provenance` keeps only `retention-days` and `cron`. The card is unaffected by
the larger values — `ToolWorkStrip` clamps the outcome to 2 lines with tap-to-expand — and two
knobs for one budget is exactly the drift this unification removes.

Truncation is per-outcome then total, and an entry is never dropped — only its text is replaced:
each outcome's returned text is clamped to `conversation.result-max-chars` with the
suffix `" …(rövidítve)"`; once the running total across the turn's outcomes passes
`conversation.results-max-chars`, every further outcome's text becomes
...
`SYSTEM_PROMPT` (`ChatService.java:69-148`) is now **named blocks**, not one instruction stream:
...
**The prompt is SPLIT in two for the provider's cache (`mezo-ozri.5`).** The order above is
unchanged — what changed is how the two halves travel. `ChatService.stableSystemPrompt(userId)`
returns the persona alone (`SYSTEM_PROMPT` through `PromptPersona`), and
`ChatService.turnContext(..)` returns everything volatile, in exactly its old order: snapshot →
`[Heti adatok]` → facts → pattern-ack → reflection → `[Karakter]` → `[Rólad tanultam]` →
`[Emlékek]` → `[Összefüggések]` → `TONE_REMINDER`. `PreparedTurn` carries both;
`SpringAiCompanionLlm` sends the stable half as the leading system message and the volatile half as
a SECOND system message appended to the message list, i.e. after the history and immediately before
the user's turn. **A `TurnGear.CHAT` turn (see "The turn gear" above) gets a much shorter volatile
half instead** — `ChatService.chatGearContext(userId, today)` returns just `"Ma: <date>"` +
`[Rólad tanultam]` + `TONE_REMINDER`, none of the other nine blocks in the list above.

*Why.* OpenAI renders a request's cacheable prefix as *instructions → tool definitions →
conversation history* (verified 2026-09-07 against
[the prompt-caching guide](https://developers.openai.com/api/docs/guides/prompt-caching)); caching
is on by default, needs a **1,024-token** prefix on GPT-5.6 and later, stays eligible for **30
minutes** after its last use, and bills a hit at **0.1×** the input rate. While the volatile blocks
lived inside the leading system message, the prefix changed on every single turn — so all 46 `@Tool`
schemas, the single largest fixed input on the chat path, were re-billed at full price every time.
With the split the prefix (persona + tool schemas + closed history) is byte-stable across a
conversation's turns. `OpenAiCompanionLlm` additionally sends a `prompt_cache_key` of
`<feature>:<accountId>` — a ROUTING hint only, per the same guide, and deliberately carries no
prompt text and nothing identifying.

*What the split may never break.* `stableSystemPrompt(..) + turnContext(..)` must equal the old
single string **character for character** — the halves are joined with nothing between them
(`CompanionLlm.joinInstructions`, guarded by `CompanionLlmJoinInstructionsTest`). Three things read
that identity: the `llm_log_history.system_prompt` audit column (the adapter records the join, not
the stable half), `FakeCompanionLlm`'s `startsWith` marker dispatch, and its
`PREFIX + " system=[…]"` echo, which dozens of chat ITs assert on. The fake never sees the split at
all: it inherits the port's joining defaults. The advisor chain's corrective retry block
(`AdvisorRetry.block`) now appends to the VOLATILE half for the same reason — appended to the stable
prompt it would have thrown the cached prefix away on exactly the round that pays twice.

*How it is measured.* Not by estimate: `llm_log_history.cached_tokens` is the provider's own
`cached_tokens` count, `LlmLogRepository.aggregateByStatusSince` sums it beside `prompt_tokens` (raw
counts — cached is a SUBSET of prompt), `LlmUsageTotals.{promptTokens,cachedTokens}` carries the
pair, and the AI-napló header renders the quotient as *„N% gyorsítótárból"*. A period with no
...
default and needed zero changes. Since `mezo-ozri.5` there is also a **6-arg** pair carrying the
turn context (`system, turnContext, history, user, tools, toolContext`) — added as DEFAULTS that
re-join the halves, so `FakeCompanionLlm` and the three test-only `implements CompanionLlm` stubs
needed no change either; only `SpringAiCompanionLlm` overrides them to actually split the messages.
`SpringAiCompanionLlm.request` builds
`.system(sp).messages(history + optional SystemMessage(turnContext)).user(um)` — Spring
AI's actual outgoing message order (`[SYSTEM, history-in-order…, (SYSTEM turn-context), USER]`) is pinned by
...
| `get_goal(scope)` | Active goal progress: direction, week, start→target, actual/planned rate and current prescription. Progress labels the full measured date range and trailing 28-day EWMA slope with data sufficiency, includes raw weights (last entry per day within `tools.max-window-days`) and both the difference between the last two measured dates and the recent raw direction over at most seven days. Raw-versus-smoothed difference is explicitly not a daily change; short-window slope is unavailable with fewer than two observations. `recept` renders every segment with kcal/protein/carbs/fat, training/rest-day kcal, sleep/rest days/rate/rationale; `guards` renders strength/muscle guard details; `feasibility` retains all notes; `timeline` retains all plan links and uncovered weeks. Unevaluated prescriptions remain explicitly absent. Archived goals and complete envelopes use `goal`/`goal_plan_link` reads. | `Goal`/title |
...
- The RESULT-text storage caps are NOT under `turn.provenance`: since the mezo-rj214.10
  unification there is one storage place and therefore one budget, read from
  `mezo.companion.conversation.result-max-chars` (**8000**) and `results-max-chars` (**40000**)
  — see that section. `TurnProvenance.build` clamps each outcome at the former with the suffix
  `" …(rövidítve)"` and, past the latter, replaces further outcomes' text with
  `"…(a többi részlet nem fér ide)"`; an ASK line is never touched by either.
...
All model access goes through the `CompanionLlm` port (`CompanionLlm.java`) — retired: the old
two-string `complete(systemPrompt, userMessage) → String` shape this paragraph used to describe
(mezo-rj214.8). **Contract crossing the seam today** is the **six-arg stable/volatile pair**
(mezo-ozri.5) — `complete`/`stream(systemPrompt, turnContext, history, userMessage, tools,
toolContext)`, DEFAULTS that re-join the halves for every implementation but `SpringAiCompanionLlm`
(§3 "The prompt is SPLIT in two" has the full inversion history) — plus **two tool-free SMART-tier
entry points** (mezo-rj214.7): `completeSmart`/`streamSmart(systemPrompt, turnContext, history,
userMessage)` carry no tools, by contract, so a CHAT-gear turn is the only conversational shape that
can carry reasoning on OpenAI Chat Completions (§3 "The turn gear"). Real traffic runs through
`SpringAiCompanionLlm` (`llm/SpringAiCompanionLlm.java`), the shared base both provider adapters
extend; **`OpenAiCompanionLlm` is the production adapter** (`@Primary` under
`mezo.companion.llm.provider: openai`), with `GeminiCompanionLlm` staying live as the chat fallback
and the TRANSCRIBE/VISION delegate (§3 "LLM adapter selection" has the full two-adapter table); test
fake `FakeCompanionLlm`. Provider swap = one new `SpringAiCompanionLlm` subclass + one starter swap
(ADR 0008 + its mezo-ozri.2 amendment). **Since mezo-78rn the port also
...
**V0.5 tools seam (✅ wired, now 18 tools across 10 toolsets, mezo-iizd.10).** The read tools in
...
**V0.5 test additions (grown to 18 tools' worth by mezo-iizd.10):**
...
- `backend/src/main/java/io/mrkuhne/mezo/feature/companion/service/ConversationHistory.java` — transcript restoration and render-time clipping (read side only; persistence is `TurnProvenance`).
...
**Backend — tools (V0.5, expanded to 18 tools across 10 toolsets by mezo-iizd.10)**
...
- `backend/src/main/java/io/mrkuhne/mezo/feature/companion/tools/{TrainTools,BiometricsTools,FuelTools,GoalTools,MedicationTools,MemoryTools}.java` — the 14 `@Tool` reads from the V0.5–V2.3 batch.
- `backend/src/main/java/io/mrkuhne/mezo/feature/companion/tools/{GrowthTools,PracticeTools,InsightsTools}.java` — the mezo-xixu trio of new beans (`get_growth`/`get_daily_practice`/`get_insights`), bringing the total to 17 `@Tool` reads; `LifeGoalTools` (the 10th toolset, see §11) adds `get_life_goals`, reaching 18 across 10 toolsets.
...
- `backend/src/test/java/io/mrkuhne/mezo/feature/companion/tools/{CompanionToolsRenderIT,CompanionToolRegistryIT,ToolCallAuditTest,RecordingToolCallbackTest,ToolTextTest}.java` — the V0.5–mezo-iizd.10 tool batch (77 render tests over what was 15 tools at write time, now 18) + `ToolTextTest`, the 2026-09-08 render-seam guard (§8).
