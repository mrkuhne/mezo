...
- **`GraphTraversalService`** (`graph/service/`) — `seedsFor(userId, message)`: the message's folded
  ...
  - **Ranked cap** — matching nodes are ordered (title hit outranks summary-only, then more
    ...
    Two nodes tied on both keys are left in `activeNodes`' own `created_at desc` row order — the
    ranking sort is stable and deliberately carries no further tie-break, so recency (a real
    relevance signal) decides rather than a node id (a meaningless one); the same turn still
    always produces the same seed set, since `created_at desc` is itself deterministic.
...
- `mezo.companion.graph.max-seeds` = **8** (`@Min(1) @Max(50)`) — `mezo-b3pp.34`: cap on
  ...
  distinct token hits, ties left to the query's own `created_at desc` row order) so a chatty turn
