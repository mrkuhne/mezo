...
- **8 read-only tools** in `feature/companion/tools/` (spec §5 first batch), grouped by source
  ...
  `MedicationTools` (`get_reta_cycle` — merged into one scoped `get_medication(scope)` since
  mezo-xixu (scope renamed to `cycle`/`all` in `mezo-lwmq`), see the catalog below). All
