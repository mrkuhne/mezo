...
Since the daypart-faces re-composition (`mezo-j7u4`, [ADR 0014](../decisions/0014-today-daypart-faces.md)) the component takes an explicit **`variant: 'chip' | 'reflect'`** prop and **the daypart decision moved OUT of it into the caller** — whichever daypart component mounts it. The full-card `.intent*` banner is gone (its CSS retired). Since the iOS listanyelv rewrite (`mezo-e26w`) both variants render into `DayGroups`' `focus` slot as their **own `TodayList`** (`features/today/components/TodayList.tsx`) — `DayGroups` adds no heading of its own over the slot; each variant supplies its own `label`, so the chip and the reflection sit in two separate boxes, not one shared "Fókusz" group. The five states survive, redistributed:

**`variant="chip"` — the creed chip** (mounted by `DaypartMorning.tsx:43` and `DaypartDay.tsx:79` into their `DayGroups` `focus` slot, i.e. the 🌅 Reggel and ☀️ Nap dayparts). A `TodayList label="Fókusz"` (or `"Fókusz · {n} / {cap}"` once foci exist, the header's `action` slot carrying the one CTA) wrapping a `.td-creed` block plus one `TodayRow` per live focus (`IntentionBanner.tsx:57-85`):
1. **No creed** — a plain `<span className="td-creed-q">` prompt + the header CTA opens `CreedSheet`.
2. **Creed, no focus** — the creed as a `button.td-creed-q` + the header CTA `+ Mai fókusz` (opens `IntentionSheet`).
3. **Foci set** — the header counter plus **one `TodayRow` per live focus** (`tone="plain" icon="✦" accessory="none"`) underneath the creed. **This row list is load-bearing, not decoration:** without it the feature is write-only — you add a focus, save, and it appears nowhere on Today. (It was: the first cut of the chip dropped the foci, and mutating `onSave` to a no-op left all 202 `features/today` tests green. Caught in review, fixed, now covered.)
   The **creed text itself is the `CreedSheet` opener** (`button.td-creed-q`, `IntentionBanner.tsx:72-75`) — the retired banner's separate `szerkeszt` affordance. Without it `CreedSheet`/`setCreed` would be **unreachable app-wide**, since the creed line is its only surface.
   The foci rows now ride **`TodayRow`, not `shared/ui/ItemRow`** (`IntentionBanner.tsx:83`) — the Today-wide `mezo-e26w` rule that `ItemRow` stays Fuel/routine-editor territory (see [today.md](today.md)); a focus row still carries no subtitle or action, just the `✦` icon and the user's own prose as the title.

**`variant="reflect"` — the evening reflection** (mounted by `DaypartEvening.tsx:139` into the 🌙 Este daypart's `DayGroups` `focus` slot). Its own `TodayList label="Reflexió"` wrapping a `.reflect` block:
...
The reflect variant **ghosts when there is nothing to reflect on** (no creed, or no foci) — a stricter guard than the chip's, and its own: `DaypartEvening` mounts it unconditionally, so the check has to live in the component.
...
- **A napom review, `mezo-yjzhw.4` (2026-09-24) — another skin-only `NapHubPage.tsx` move, unrelated to intention.** `NapHubPage` now mounts `<NapzarasCard now={tick} />` (`frontend/src/features/today/pages/NapHubPage.tsx:61`) directly above the `.nap-center-orbit` (the Kreed tile lives inside that orbit, untouched). The card is the evening Napzárás entry point — shown 20:00–24:00 local time unless the ritual is already closed — and is unrelated to the intention/creed/reflection flow this doc covers; see [ritual.md](ritual.md) for what it is and [today.md](today.md) for the "A napom" page it links to.
