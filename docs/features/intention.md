...
Since the daypart-faces re-composition (`mezo-j7u4`, [ADR 0014](../decisions/0014-today-daypart-faces.md)) the component takes an explicit **`variant: 'chip' | 'reflect'`** prop and **the daypart decision moved OUT of it into the caller** — whichever daypart component mounts it. The full-card `.intent*` banner is gone (its CSS retired); the day half became a compact `.creedchip`, the evening half kept the `.reflect*` block. Since the napszak-tab re-composition (`mezo-puci`, [ADR 0025](../decisions/0025-today-daypart-tabs.md) — islands in between, `mezo-euze`/[ADR 0022](../decisions/0022-today-three-islands.md)) both variants render **inside `DayGroups`' `focus` slot** (under a **`Fókusz` group heading**), visible without any unfold — every daypart's content sits on screen at once now, there is no L1 layer left behind a tap. The five states survive, redistributed:

**`variant="chip"` — the creed chip** (mounted by `DaypartMorning.tsx:43` and `DaypartDay.tsx:85` into their `DayGroups` `Fókusz` group, i.e. the 🌅 Reggel and ☀️ Nap dayparts). One `.creedchip` row: a `✦` mark, the creed in quiet italics, a `{n} / {cap}` counter, and one CTA:
...
**`variant="reflect"` — the evening reflection** (mounted by `DaypartEvening.tsx:145` into the 🌙 Este daypart's `DayGroups` `Fókusz` group). A `.reflect` block:
