...
- **Planner** (`MesocyclePlannerPage.tsx`, `/train/mesocycles/new`) — a 4-step wizard: `Cél` (goal preset) → `Hossz + fázisok` (name/start/weeks + `MEV/MAV/MRV/Deload` phase-curve editor) → `Split + napok` (split + days/week + weekday picker) → `Áttekintés` (generated program review, per-day exercise add/remove). Save as `planned` or `active`.
- **Builder** (`MesocycleBuilderPage.tsx`, `/train/mesocycles/:id`) — three sub-views: `Áttekintés`, `Volumen` (provenance), `Gyakorlatok` (weekly exercise editor). Lifecycle buttons: planned → `Aktiválás`, active → `Meso lezárása`. "Heti terv másolása" is inert (deferred). The `Gyakorlatok` editor's per-exercise row (`ExerciseEditRow.tsx`) now edits the **recipe** — a `RecipeStepper` tile each for `Bemelegítő`/`Working` counts, `Rep min`/`Rep max` range, and `RIR` (`ExerciseEditRow.tsx:77-85`) — instead of the old single sets/reps pair (§4 recipe columns).
...
**Prescribed sets (hypertrophy drive, `mezo-dhdr`).** When `mezo.feature.hypertrophy-drive.enabled` is on, `GET /workouts/today` carries a per-exercise `prescribedSets: PrescribedSet[]` + a `rationale` string (§4). The active phase **pre-populates every set from `prescribedSets`** — warmup sets first, then working sets — each pre-filled with the engine's recommended weight/reps, so the athlete steps into a fully-primed session instead of an empty grid. `makeSession`/`seedFromOpen` fold the recipe (`warmupSets + workingSets`) into `session.planned`, and the pure helper `prescribedAt(session, exerciseId, setIdx)` (`logic/workoutState.ts:58`) resolves the target for a given set index; the current set's `CompactStepper` **seeds from its prescribed target** rather than from last week. Warmup vs working sets are **visually distinguished** (a `Bemelegítő` chip / set-dot styling, `ActiveWorkoutPage.tsx:706,772`), and the old static "+2.5 kg" heuristic hint is **replaced by the engine `rationale`** (`ActiveWorkoutPage.tsx:761`). **Plyo / bodyweight (weightless) sets hide the kg stepper** (reps-only), since the engine returns a null target weight for them. When the switch is off, `prescribedSets`/`rationale` are null and the screen falls back to ad-hoc logging exactly as before (no pre-fill). The prescription is computed from **completed-instance history only**, so it is stable within a live session and resume-safe (§9).
