...
- `frontend/src/data/progressionMock.ts` — deterministic seeded `gymLevelUpMock`/`sportLevelUpMock`/`runLevelUpMock` fixtures (mock-mode finish/sport/run return these)
- `frontend/src/lib/useReducedMotion.ts` — shared reduced-motion gate; `.levelup*` styles + keyframes in `frontend/src/styles/prototype.css`
