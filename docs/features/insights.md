...
- **„Életesemény-jelöltek · N"** (W2.3, `mezo-b3pp.8`) — a fact-jelölt blokk UTÁN, a tényigen listája ELŐTT; `useLifeEventCandidates()` + `LifeEventCandidateCard` (`components/`). Csak akkor renderel, ha `lifeEvents.length > 0`, de FÜGGETLENÜL a Tudástár többi részétől — **akkor is megjelenik, ha a Tudástár egyébként üres** (a fenti „Genuinely üres tudásbázis" ág feletti feltétel, saját `.length > 0` kapuval, nem a `hasNoFacts` ágtól függ). Kártyánként: dátum (`occurredOn`), cím, összefoglaló, egy magyarázó mondat („Ezt a napod szövegeiből szűrtem ki — csak akkor kerül a gráfba, ha elfogadod."), **Elfogad**/**Elvet** → `useLifeEventActions().decide(id, decision)`, és egy lábjegyzet-sor, ami kimondja a `proposedEdgeCount`-ot: `N` esetén „Elfogad → bekerül a gráfba N kapcsolattal · Elvet → eldobom.", `0` esetén a rövidebb „Elfogad → bekerül a gráfba · Elvet → eldobom." Confirm itt sem néma (ugyanaz az IDENT-6 elv, mint a fact-kártyánál). **Elfogadás után a kártya helyén megerősítés marad** (`LifeEventAcceptedCard`, `mezo-0ap9`): sage csík, „Bekerült a gráfba · N kapcsolattal" (`proposedEdgeCount === 0` esetén a rövidebb „Bekerült a gráfba"), a jelölt címe, és egy `Megnézed? → Tudásgráf` `Link` a `/me/knowledge`-re. Ez azért kell, mert a jóváhagyás itt történik, de az eredmény a MÁSIK felületen (a Tudásgráfon) landol: enélkül a kártya a query-invalidálással némán lekerül a listáról, a Tudástár tényszáma nem nő, és a felhasználó azt éli meg, hogy „elfogadtam, mégsem lett belőle semmi" (élesben pontosan ez történt: négy elfogadásból egy tény jelent meg, a három életesemény „eltűnt"). Az elfogadott jelöltet page-szintű `acceptedEvents` state tartja életben az oldal elhagyásáig — mock és real módban azonosan —, és a jelölt-lista `.filter`-rel kiejti, hogy a real-módú refetch előtti ablakban ne látszódjon egyszerre a jelölt-kártya és a megerősítés is. Elvetésnél nincs változás: a jelölt némán eltűnik.
- **Kereszt-link a Tudásgráfra** (`mezo-0ap9`) — a `KnowledgeExplainer` alatt egy halk sor: „A kapcsolatok és életesemények a **Tudásgráfon** élnek.", `Link` a `/me/knowledge`-re. A két felület szerep-elhatárolása kimondva: a **Tudástár** arra válaszol, hogy *„mit kap most a társ"* (tény- és életesemény-inbox, prompt-vödrök, be/ki kapcsoló), a **Tudásgráf** arra, hogy *„hogyan függ össze, amit rólam tud"* (node-ok, élek, archiválás). A tényeknek a Tudástár az egyetlen gazdája — a Tudás oldal duplikált `Kategóriánként` ténylistája ezért törölve ([`me.md`](me.md) §`Tudás`).
