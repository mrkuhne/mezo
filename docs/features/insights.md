...
- The two cross-link explicitly in both directions: Tudástár carries a **`Tudásgráf`** row-button (`card row`, clay `i-tudas` icon, subline „kapcsolatok és életesemények · élő mindmap") → `/me/knowledge`, and Tudásgráf carries „Tények kezelése → **Tudástár**" → `/mezo/knowledge`. Design 2.0 made each a tile on its own hub; it did not move the boundary. **The hub-tile-reorg (`mezo-o486`, 2026-09-01) retired `Tudás` as an Én-hub tile** — this Tudástár entry is now the graph page's *only* door besides deep links; the final review (`mezo-o486`) upgraded it from a footnote link to a proper row-button, and the Tudásgráf back chip was repointed from `‹ Én` to `‹ Tudástár` (see [me.md §`Tudás`](me.md)); the ownership boundary itself is unchanged.
