...
| weekly | `/me/week` | `Heti` | `WeekPage` ([me.md](me.md)) | **real** (`mezo-p2tr`) |
...
1. **Header recipe** — date eyebrow (`useToday`) + the notification bell with its unread badge and three-row „Értesítések · ma" popover (`useNotificationFeed`) + an avatar button into `/me`. The same recipe the Nap and Én hubs carry.
...
6. **The L0→L3 memory band** — a full-width row off `useMemoryOverview()`: `{daysWithAnyData} nyers nap › {summaryCount} napló › {Σ l2} ítélet › {Σ l3} tény`, tapping into `/mezo/memoria`. **With no overview** (cold load, companion switched off) it renders as a plain labelled door — „Memória-rétegek" — with **no fabricated layer counts**.

The whole panel rides an `EntranceGroup` (orb 0 ms → chat opener 70 ms → decision card 110 ms → tiles 160–360 ms → memory band 400 ms), one-shot, reduced-motion-guarded.
...
- **Mock-only demo extras:** the "Évforduló · 1 hónap" card (`anniversaryNote`) and the static "Memoir archive · 17 darab →" footer still wrap in `mode === 'mock' ? (…) : null` — **hidden in live mode** (no backend = false affordance; the Weekly „Elfogad/Hangoljuk" precedent). Anniversary + archive are a deferred epic ([`proactive.md` §9 decision o](proactive.md)). Mock render is otherwise byte-identical to Phase 1 apart from the retired reaction row above.

### 2.4 Tudástár (`pages/KnowledgeListPage.tsx`) — **real dual-mode since companion V1.2, érthetőség-redesign `mezo-9ryh` + review fix wave; Mozaik re-face `mezo-d20.5.5`**
At `/mezo/knowledge`, the hub's fourth tile; the page leads with a shared **`PageHero`** and renders its facts as `.mz-facttile` tiles (the recipe the Én tab's Tudásgráf page then borrowed for its graph nodes — [me.md §`Tudás`](me.md)). Minden alábbi viselkedési szerződés változatlan. A companion fact-memóriájának L2 confirm felülete ([`companion.md`](companion.md) §4). Betöltési sorrend, mielőtt bármi más renderelne: `isPending` → `GhostState` („A tudástár betöltése…", `useKnowledge()`-ből forwardolt real-mode-only cold-load guard, a `PatternsPage.tsx` mintáját követve — mock mode `isPending`-je mindig `false`); `isError` (genuinely failed fetch, pl. 500) → `GhostState` retry CTA-val (`refetch`), hogy egy valós hiba sose olvasson az őszinte-de-hazug „0 megy a chatbe" `realEmpty` állapotként; majd a meglévő `degraded` (companion switch-off 404) ág. A lap fentről lefelé:
...
- **A gráf-jelölt inbox (W2.3 `mezo-b3pp.8` + W5.3 `mezo-b3pp.20`)** — a fact-jelölt blokk UTÁN, a tényigen listája ELŐTT; `useLifeEventCandidates()` + `LifeEventCandidateCard` (`components/`). **Két jelölt-fajtát hordoz**, `LIFE_EVENT` (éjszakai életesemény-kivonat) és `SEASON` (negyedéves szezon-olvasat), FAJTÁNKÉNT csoportosítva, saját eyebrow-val és saját proveniencia-mondattal — `(['LIFE_EVENT','SEASON'] as const).map(...)` renderel egy-egy `„<fajta eyebrow> · N"` blokkot csak azokra a fajtákra, amelyeknek van jelöltje, a `CANDIDATE_COPY` (`data/insights/graph.ts`) fajtánkénti `{eyebrow, provenance}` szótárából. Mindkét blokk FÜGGETLEN a Tudástár többi részétől — **akkor is megjelenik, ha a Tudástár egyébként üres** (a fenti „Genuinely üres tudásbázis" ág feletti feltétel, saját `.length > 0` kapukkal fajtánként, nem a `hasNoFacts` ágtól függ). Kártyánként: egy fajtafüggő dátumsor (`formatCandidateDate` — egy `LIFE_EVENT` `occurredOn`-ja a nap, nyers ISO alakban; egy `SEASON` `occurredOn`-ja a negyedév ELSŐ napja, ezért magyar negyedév-alakban jelenik meg, pl. „2026. III. negyedév" — egy nyers dátum egy háromhónapos időszakot egyetlen napként állítana be), cím, összefoglaló, a `CANDIDATE_COPY[kind].provenance` fajtánkénti magyarázó mondata (egy `LIFE_EVENT`-nél „a napod szövegeiből", egy `SEASON`-nél „a negyedév és az előző negyedév összefoglalóiból" — a megerősítés sosem hazudik arról, honnan jött a javaslat, IDENT-6), **Elfogad**/**Elvet** → `useLifeEventActions().decide(id, decision)`, és egy lábjegyzet-sor, ami kimondja a `proposedEdgeCount`-ot: `N` esetén „Elfogad → bekerül a gráfba N kapcsolattal · Elvet → eldobom.", `0` esetén (minden `SEASON`, amely sosem javasol élt) a rövidebb „Elfogad → bekerül a gráfba · Elvet → eldobom." Nincs új backend endpoint — mindkét fajta ugyanazt a `GET /api/companion/graph/node/candidate` listát és ugyanazt a kind-agnosztikus döntés-végpontot használja ([`companion.md`](companion.md) §4 W5.3 alszakasz). Mock módban a seed egy-egy példányt hordoz mindkét fajtából. **Elfogadás után a kártya helyén megerősítés marad** (`LifeEventAcceptedCard`, `mezo-0ap9`): sage csík, „Bekerült a gráfba · N kapcsolattal" (`proposedEdgeCount === 0` esetén — pl. minden `SEASON` — a rövidebb „Bekerült a gráfba"), a jelölt címe, és egy `Megnézed? → Tudásgráf` `Link` a `/me/knowledge`-re. Ez azért kell, mert a jóváhagyás itt történik, de az eredmény a MÁSIK felületen (a Tudásgráfon) landol: enélkül a kártya a query-invalidálással némán lekerül a listáról, a Tudástár tényszáma nem nő, és a felhasználó azt éli meg, hogy „elfogadtam, mégsem lett belőle semmi" (élesben pontosan ez történt: négy elfogadásból egy tény jelent meg, a három életesemény „eltűnt"). Az elfogadottat page-szintű `acceptedEvents` state tartja életben az oldal elhagyásáig (fajtánként szűrve), mock és real módban azonosan; a `pendingLifeEvents` derivált a már elfogadottat kiejti a jelölt-listából, hogy a real-módú refetch előtti ablakban ne látszódjon egyszerre a jelölt-kártya és a megerősítés is. Ha egy fajtának már NINCS döntésre váró jelöltje, csak megerősítése, a csoport eyebrow-ja a darabszámos alakról a `CANDIDATE_COPY[kind].settled` címkére vált („Életesemények" / „Szezonok") — a „…jelöltek · 0" hazudna. Elvetésnél nincs változás: a jelölt némán eltűnik.
- **Kereszt-link a Tudásgráfra** (`mezo-0ap9`) — a `KnowledgeExplainer` alatt egy halk sor: „A kapcsolatok és életesemények a **Tudásgráfon** élnek.", `Link` a `/me/knowledge`-re. A két felület szerep-elhatárolása kimondva: a **Tudástár** arra válaszol, hogy *„mit kap most a társ"* (tény- és életesemény-inbox, prompt-vödrök, be/ki kapcsoló), a **Tudásgráf** arra, hogy *„hogyan függ össze, amit rólam tud"* (node-ok, élek, archiválás). A tényeknek a Tudástár az egyetlen gazdája — a Tudás oldal duplikált `Kategóriánként` ténylistája ezért törölve ([`me.md`](me.md) §`Tudás`).- **Genuinely üres tudásbázis** (`facts.length === 0`, nem pending/error/degraded) — a kereső/kategória-chip sor és a szakaszok helyett egy őszinte sor: „Még egy tényt sem tanultam rólad — ahogy beszélgettek, itt fognak megjelenni."
...
`useKnowledge()` backs **two** views across two tabs (it used to be three — `ProfilePage` is deleted, `mezo-d20.6.1`): the Mezo-tab `KnowledgeListPage` and the Én-tab `KnowledgePage`. **The ownership boundary is the hard-won part and it is unchanged by the rename** (`mezo-0ap9`):
- **Mezo → `Tudástár`** (`/mezo/knowledge`) answers *„mit kap most a társ"* — it is the **facts' single owner**: the candidate inbox, the on/off toggles, the prompt buckets, the `PROMPT_TOP_N` + fresh-pattern injection rules. Consumes `facts` + `candidates`.
- **Én → `Tudásgráf`** (`/me/knowledge`) answers *„hogyan függ össze, amit rólam tud"* — nodes, edges, archiving. Consumes `edges` + `activeCount` + the separate `useKnowledgeGraphNodes()` read; it deliberately does **not** re-list the facts (its duplicate `Kategóriánként` section was deleted in `mezo-0ap9` because users read it as a broken copy of the Tudástár).
- The two cross-link explicitly in both directions: Tudástár carries „A kapcsolatok és életesemények a **Tudásgráfon** élnek." → `/me/knowledge`, and Tudásgráf carries „Tények kezelése → **Tudástár**" → `/mezo/knowledge`. Design 2.0 made each a tile on its own hub; it did not move the boundary.
...
- **Feedback (W4.1, `mezo-b3pp.15`) — both modes:** `data/feedback/feedbackHooks.test.tsx` pins the
  ...
  **the in-flight draft carries none until `done` lands** — a gated-stream test; a 👎+reason writes
  ...
  `mezo-p2tr` — moved to `me.md`'s `WeekPage.test.tsx`), `PredictionsPage.test.tsx` (one row per card; a
...
- **`Heti` is the one tile whose destination is still being built — IN FLIGHT, not done.** The tile ships and lands on a real, working page (`/me/week`'s `WeekPage`, `mezo-p2tr` — score hero, stat strip, seven day cards, the generated review, discoveries, next-week prose; [me.md](me.md)). What is *not* here yet is the designed **Heti hub + four view-pages + a day page** the Design 2.0 prototype specifies, plus the two backend legs it wants: weekly knowledge candidates („A hét tanulságai") and a persisted weekly score with a trend endpoint. That work is carried on a separate machine under its own handoff — [`docs/design_2.0/2026-08-28-heti-implementation-handoff.md`](../design_2.0/2026-08-28-heti-implementation-handoff.md) — as the FE slice plus its backend slices (`mezo-d20.7.5`–`.7.8`). Two consequences worth knowing before touching either hub: the handoff's own §0 records that the anchor spec still files Heti as **F4.4 under the Mezo tab, which is stale** — the `mezo-p2tr` move made it an Én-tab page, and it appears here only as a linked tile; and the FE slice is deliberately shippable **without** its backend legs, showing the trend and the tanulság tile in mock mode and an **honest empty state** in real mode. Until it lands, do not describe the Heti tile's destination as the designed surface — it is the weekly review that existed before Design 2.0, reached through a new door.
...
- `pages/MezoHubPage.tsx` — **`mezo-d20.5.1`, the `/mezo` index and the tab's face:** header recipe (date · bell · avatar) → the numberless breathing orb hero (`ClaySpot s-orb` + one companion sentence + the honest status line) → the composer-shaped chat opener → the motor's **single** gold-ringed decision card over the shared `usePatternActions().decide` → the 6-tile `Mosaic` with per-page live lines → the full-width L0→L3 memory band. Reads the pages' own hooks; owns no data (§2.0)
...
- **`pages/WeeklyPage.tsx` is DELETED (`mezo-p2tr`)** — the 2nd sub-tab (D′ `mezo-t16y.1`) is retired; its content (score hero, growth card, tervjavaslat) moved verbatim to `/me/week`'s `WeekPage.tsx` (§2.2, [`me.md`](me.md))
...
- `weeklySuggestionApi.ts` — **W1** `weeklySuggestionApi.get(date)` → proactive `GET /api/proactive/weekly-suggestion` (wire → `prose` string, 404→null); its only FE consumer is now `/me/week`'s `WeekPage` ([`me.md`](me.md))
...
**Cross-feature seams:**
- `frontend/src/app/router.tsx` — the flat `/mezo` + `/mezo/*` routes (hub, patterns, `patterns/:pairKey`, memoir, knowledge, chat, predictions, experiments, memoria) + the two intra-tab redirects (`mezo/weekly` → `/me/week`, `mezo/motor` → `/mezo/patterns`) + **`LegacyPathRedirect`**, the `insights/*` → `/mezo` rewrite that preserves subpath and query (§2)
- `frontend/src/app/TabBar.tsx` — the `Mezo` tab (clay `i-mezo`), the entry point that replaced Today's ✨ link (§5.2)
- `frontend/src/features/me/pages/KnowledgePage.tsx` — the Én-tab `Tudásgráf`, the other `useKnowledge` consumer (§5.1; `ProfilePage.tsx`, the third one, is deleted — `mezo-d20.6.1`)
