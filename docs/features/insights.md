...
### 2.0b Rólad (`pages/BoopAboutPage.tsx`) — **the common picture + the decision inbox, U9b `mezo-zpxv7`, 2026-09-26**

At **`/mezo/rolad`**, the dock's third stop. Rebuilt per the [U9b design](../superpowers/specs/2026-09-26-rolad-kozos-kep-design.md): the page is now the ONE place the team's picture of the user lives and where the user decides about it — the approval inbox moved here from the Tudástár (§2.4), which keeps only the archive. Top to bottom, each section independently honest (absent/collapsed rather than fabricated when its layer has nothing to show):
...
2. **Döntésre vár** (`components/rolad/RoladInbox.tsx` over `hooks/useRoladInbox.ts`) — fact candidates (`useKnowledge()`) and LIFE_EVENT/SEASON candidates (`useLifeEventCandidates()`) in one list, oldest-seen-first, each a glass `FactCandidateCard`/`LifeEventCandidateCard` with the **four decisions**: **Igen, jegyezd meg** (accept) · **Pontosítom** (inline refine, unchanged semantics) · **Most ne** (`snooze` — new, §4.1: candidate leaves the inbox for 14 days, then is re-offered; stays non-terminal, re-snoozable) · **Nem igaz** (`reject`, terminal, a quiet text button). `useRoladInbox()` keeps a page-level `settled` list (`Settled[]`) so a decided card turns into its afterlife line **in place** instead of jumping or disappearing during the real-mode refetch window — kept: sage `SettledRow` (fact) or `LifeEventAcceptedCard` (graph); snoozed/rejected: a quiet dashed line (`ROLAD_COPY.snooze`/`.reject`). The fact-candidate conflict checkbox (`conflictsWithFactId` → „A régit kikapcsolom") moves here unchanged — it never fires on snooze or reject, only on an accepting path. `degraded` (companion off) hides only the fact half; graph candidates keep rendering. `?start=YYYY-MM-DD` (moved off the Tudástár) shows the week banner back to `/me/week?start=`, without filtering the inbox.
3. **A tények rólad** (`components/rolad/RoladFacts.tsx`) — up to 4 active facts, most-recently-reinforced first (`topRoladFacts`), each tagged with its owner (`factOwnerTag` — `SZUNYA`/`MOCOR`/`FALAT`/`DERŰ`/`MEZO` off the new backend `owner` field, or `TŐLED` gold for `source: manual|question`) and the existing `factCopy.ts` provenance chip. Door: „Mind a N tény → …" to `/mezo/knowledge?view=tenyek`. Hidden when `degraded` or no active fact (no fabricated zero).
4. **Életesemények** (`components/rolad/RoladTimeline.tsx`) — accepted `LIFE_EVENT`/`SEASON` graph nodes (excluding the profile node) as one amber glass card of flat rows, newest `occurredOn` first; SEASON dates render as a Hungarian quarter (`formatCandidateDate`). Omitted when empty.
5. **„A te kezedben”** — the approved note verbatim (`ROLAD_COPY.note`).
6. **Doors** — „A csapat képe rólad, dimenziónként →” (`/mezo/karakter/dimenziok`, `DimensionsPage` — the dimension list that used to be embedded directly, D3 in the spec), „Így beszélj velem” (`/settings/mezo/communication`), „Kapcsolatok” (`/mezo/knowledge?view=kategoriak`).

The user-facing sentences and picks are pure, unit-tested helpers in `logic/roladCopy.ts` (`pickQuoteClaim`, `factOwnerTag`, `candidateByline`, `topRoladFacts`) — no copy is composed inline in the components.
...
- **Rólad (§2.0b, U9b `mezo-zpxv7`):** `hooks/useRoladInbox.ts` (the four-decision inbox hook over `useKnowledge`/`useLifeEventCandidates`, page-level `settled` afterlife state) + `components/rolad/{RoladQuote,RoladInbox,RoladFacts,RoladTimeline,riseStyle}.tsx` + `logic/roladCopy.ts` (`pickQuoteClaim`, `factOwnerTag`, `candidateByline`, `topRoladFacts`, `ROLAD_COPY`) — all pure/tested, composed by `pages/BoopAboutPage.tsx`.
