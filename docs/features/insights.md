...
**Feedback** (W4.1, `data/feedback/feedbackTypes.ts` — its own module, NOT `types.ts`, because it is companion-owned and spans four features): `FeedbackArtifactKind = 'chat_message'|'feed_message'|'weekly_suggestion'|'memoir'|'prediction'`, `FeedbackVerdict = 'up'|'down'`, `FeedbackReason = 'inaccurate'|'too_much'|'bad_timing'|'not_about_me'` (down-verdicts only — the backend 400s a reason sent with `up`), `ArtifactFeedback { artifactKind; artifactId; verdict; reason: FeedbackReason|null; updatedAt }`, and the handle `FeedbackHandle { get(id); vote(id, verdict, reason?); pending }`. The three enums are `pattern`-constrained strings on the wire (openapi-typescript cannot narrow them), so `data/feedback/feedbackApi.ts`'s `toArtifactFeedback` narrows once at the boundary. **Endpoints are companion-owned**, not Insights: `GET/PUT/DELETE /api/companion/feedback` (`api/feature/companion-feedback/companion-feedback.yml`, tag `CompanionFeedback`) — full table/endpoint/semantics writeup in [`companion.md` §4](companion.md). The mock seed (`feedbackMock.ts`) is **deliberately empty**: feedback is something the USER produces, and pre-seeding thumbs would fake a history the demo never had; mock-mode votes accumulate in the TanStack cache for the session.
...
### 5.7 Feedback chips — an Insights-hosted component four features mount (✅ W4.1, `mezo-b3pp.15`)
...
second Insights component with a cross-feature consumer (the `LifecycleSection`/`RefTag` precedent).
