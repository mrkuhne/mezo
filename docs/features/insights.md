...
- **Feedback chips (W4.1, `mezo-b3pp.15`):** under the tervjavaslat card, **both modes**, a `FeedbackChips` row (👍 `Segített` / 👎 `Nem talált`) — the suggestion is an AI artifact wherever it comes from. It renders **only when there is an artifact to vote on**: `useWeekly()` now returns `weeklySuggestionId` alongside the prose (null exactly when the prose is null), so the honest „hamarosan" placeholder carries no chips. `WeeklyPage` calls `useFeedback('weekly_suggestion', [id])` once and keys the row by the id (§4/§5.7).
...
**Weekly** (`types.ts:406-408`): `WeeklyTrend = 'up'|'down'|'flat'`, `WeeklyItem { label; value; trend }`, `WeeklyReview { title; score; delta; items }` — mock `weekly` + `weeklySuggestion` seed. **Real mode (D′)** builds the same shape client-side in `useWeekly` (`weeklyHooks.ts`), returning `WeeklyView { weekly; deltaLabel; weeklySuggestion: string|null; weeklySuggestionId: string|null; growthWeek: WeeklyGrowth|null; mode }` (**`weeklySuggestionId` is new at W4.1** — the `weekly_suggestion` feedback artifactId, null exactly when the prose is null; `weeklySuggestionApi.get` now returns `{id, prose}` instead of the bare prose string). Two of its fields come from real backend reads (the rest stays client-composed): **`weeklySuggestion`** (W1 — the proactive `GET /api/proactive/weekly-suggestion` → `prose`, 404→null) and **`growthWeek`** (E3 `mezo-6ng8` — the Progression `GET /api/progression/growth-week/{start}`, `.catch(()=>null)`).
...
- **Feedback (W4.1, `mezo-b3pp.15`) — both modes:** `data/feedback/feedbackHooks.test.tsx` pins the
  ...
  **the in-flight draft carries none until `done` lands** — a gated-stream test; a 👎+reason writes
  ...
  regression anchor), `WeeklyPage.test.tsx`, `PredictionsPage.test.tsx` (one row per card; a
