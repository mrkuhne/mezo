...
> **2026-09-25 — Üveg U7 (`mezo-me75u.7`).** `/me/naplo` wears the dark glass material with its own `mzj-*` classes (it no longer borrows Mezo's `mzh-*`/`mzp-*`/`mem-*`); the gratitude card, the decision card (lit "5 · bevált") and `DecisionReviewSheet` (the shared capture-sheet glass in gold). Behavior unchanged. Parity reference [`uveg-en2.html`](../design_2.0/prototypes/uveg-en2.html).
...
- `frontend/src/features/quickinput/sheets/QuickInputSheet.tsx` — the three-option picker phase (`naplo-pick` with Aktivitás/Napló/Hála as clay `Tile`s); the `'gratitude'` phase renders `JournalSheet` with `initialMode="gratitude"`, and every branch takes an `onBack` returning to the picker. Its full `Phase` union also carries `'water'` and `'sport'` (`mezo-7lst`), unrelated to journal — they swap in `WaterLogSheet`/`SportLogSheet` in place, same as `'weight'`/`WeightLogSheet`. Opened from `frontend/src/app/QuickLogFab.tsx`, the floating coral FAB that replaced the centre-FAB tab bar ([ADR 0032](../decisions/0032-five-tab-ia-dissolved-section-shells.md)).
