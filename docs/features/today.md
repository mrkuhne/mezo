...
- `TodayPage.tsx` — composition root, AnchorMode branch, hero-slot swap (workout vs. rest-day volleyball), `CheckInSheet` wiring. Since the gamified-header slice (`mezo-k7rn`) the render order is: **`AppHero`** (`@/features/progression/components/AppHero`, `utilities` = search + ✨ Insights) → `GreetingHeader` → `DayArc` → hero (`WorkoutTeaser` | rest-day `VolleyballCard`) → `BriefingCard` → `CompanionNoteCard`? → `VulnerabilityCard`? → `CheckInStrip` → secondary `VolleyballCard`? → `QuickStatsRow` → `FuelTimelinePreview` → `GrowthTodayRow` → `CheckInSheet`?.
...
- `components/`: `GreetingHeader.tsx` (daypart greeting + reta day, `mezo-8141` — replaces `RetaPhaseSection.tsx`/`DateMesoHeader.tsx`, both **deleted**), `DayArc.tsx` (SVG day-progress arc over `logic/dayArc.ts`, `mezo-8141`), `BriefingCard.tsx`, `CompanionNoteCard.tsx` (H1 — the in-day heartbeat note; data from `data/today/heartbeatHooks.ts`, see [proactive.md](proactive.md)), `WorkoutTeaser.tsx`, `VolleyballCard.tsx`, `VulnerabilityCard.tsx`, `FuelTimelinePreview.tsx`, `QuickStatsRow.tsx`, `GrowthTodayRow.tsx` (one-row Growth summary over `logic/growthToday.ts`, `mezo-8141` — replaces the on-page `InsightsTeaser.tsx`'s old slot; `DailyQuestsCard.tsx`/`ActivityLogCard.tsx` **stay in this directory but now mount on `GrowthPage.tsx`**, see [growth.md](growth.md)). **Deleted:** `RetaPhaseSection.tsx`, `DateMesoHeader.tsx`, `InsightsTeaser.tsx` (`mezo-8141`, + their inline tests in `topSections.test.tsx`/`bottomSections.test.tsx`); **`BrandRow.tsx`** (`mezo-k7rn`, gamified-header slice — superseded by the cross-feature `AppHero`, see [growth.md](growth.md) / [`_platform-design-system.md`](_platform-design-system.md)).
