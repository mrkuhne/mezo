...
5. **FE swap (dual-mode obligation):** add a `*Api.ts` client mirroring `lib/biometricsApi.ts`; in the hook, branch on `isMockMode()` with TanStack Query — mock seeds synchronously via `initialData`, real loads (mirror `useGoals`/`useSleep`). Both `pnpm test` (real default) **and** `VITE_USE_MOCK=true pnpm test` must stay green.
