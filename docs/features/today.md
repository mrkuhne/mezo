...
related: [_platform-data-layer, _platform-design-system, me, insights, train, growth, habit]
---
...
- **Habit chain — `RoutineCard` ("Teendők ma" zone, `mezo-d1jb`):** directly under `TodayQuestsCard` (above the check-in strip), a **daypart-aware** card renders the morning or evening habit chain — mornings the MORNING chain, evenings the EVENING chain, midday a one-row summary linking to `/me/growth`. Each row shows a status mark + the `anchorCopy` cue + `+N XP`; the **first pending item glows**; MANUAL rows carry a `Pipa` check and DERIVED rows a `Logolás` button that taps through to the underlying log surface (never self-completing — ADR 0010). Chain completion fires a quiet `🌅`/`🌙` toast, and a completion's level-up rides the shared overlay (HABIT source meta). **Ghosts** (`null`) when the habit switch is off / no data. Domain doc: [habit.md](habit.md).
...
- `TodayPage.tsx` — composition root, AnchorMode branch, hero-slot swap (workout vs. rest-day volleyball), `CheckInSheet` wiring (incl. the quest card's next-open-slot callback). Since the action-first re-composition (`mezo-gj2y`) the render order is: **`AppHero`** (`@/features/progression/components/AppHero`, `utilities` = search + ✨ Insights) → `GreetingHeader` → `DayArc` → hero (`WorkoutTeaser` | rest-day `VolleyballCard`) → `VulnerabilityCard`? → `ZoneDivider "Teendők ma"` → `TodayQuestsCard` → `RoutineCard` (habit chain, [habit.md](habit.md)) → `CheckInStrip` → `ZoneDivider "A napod"` → `BriefingCard` → `CompanionNoteCard`? → secondary `VolleyballCard`? → `QuickStatsRow` → `FuelTimelinePreview` → `CheckInSheet`?.
