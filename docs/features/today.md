...
- `useInsightsTeaser` (real): first `status === 'proposed'` pattern (fallback: first row) from `usePatterns()`; `confidence` absent → „tanulom" eyebrow; none/degraded → `null`. **Orphaned since `mezo-8141`** (§9) — no component renders its result anymore, and its only test coverage (the `InsightsTeaser (mock)`/`InsightsTeaser (real)` cases in `bottomSections.test.tsx`) was deleted along with the `InsightsTeaser.tsx` component in Task 8 — the hook is now **no longer tested / dead code**, removal deferred to a future cleanup pass.
