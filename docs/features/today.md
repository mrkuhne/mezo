...
- **Daily quests inline — `TodayQuestsCard` ("Teendők ma" zone, `mezo-gj2y`):** the 3 daily quests render as compact rows (state mark `◦`/`✓`/`—` + 2-line-clamped title), each **offered** row carrying ONE smart log-CTA chip mapped from the quest's `metric` by the pure `questAction` (`logic/questAction.ts`): `water_target` → **instant `+250 ml`** (`useWaterActions.logWater`, stays on Today), `checkin_full` → opens the next open slot's `CheckInSheet` (callback from `TodayPage`; hidden when every slot is done), `activity_match`/ACTIVITY → `Naplózz` (the existing `ActivityLogSheet` flow with the quest banner), and the rest navigate (`weight_logged`→`/me/weight`, `sleep_target`→`/me/sleep`, `protein_target`→`/fuel`, `own_recipe_meal`→`/fuel/recipes`, `gym_session_done`→`/train`); an unknown metric renders a state-only row. Completed rows show a `+N XP` chip; expired rows dim quietly (ADR 0010 — no CTA, no red). **No reroll on Today** — `Csere` (and the why lines) live on Growth's full `DailyQuestsCard` (Today = act, Growth = manage). The header's right side (`{done}/{total} · +{xp} XP ›`, from `growthTodaySummary` — quest + activity XP combined) links to `/me/growth`, taking over the retired `GrowthTodayRow`'s job; the card **ghosts** (renders `null`) on an empty quest day. Level-up payloads carried by the quest read fire the global overlay once (same consume-once dance as `DailyQuestsCard` — the two never mount simultaneously).
