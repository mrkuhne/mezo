...
- **Header (gamified-header slice, `mezo-k7rn`, 2026-07-18):** `BrandRow` is **retired** — Today mounts the shared **`AppHero`** (`@/features/progression/components/AppHero`, `TodayPage.tsx:36-43`) at the top instead, passing Today's per-tab utilities as its `utilities` prop: the 🔍 search chip and the ✨ `<Link to="/insights">` Insights entry point (the same ✨ affordance `BrandRow` used to carry, relocated verbatim). `AppHero` itself is domain-free identity/progression chrome (avatar + XP ring + level badge + equipped title + 🔥/⚡/🪙 stat chips) shared by all 4 main tabs — see [growth.md](growth.md) for the account-progression domain behind it and [`_platform-design-system.md` §1a](_platform-design-system.md) for the `.apphero` CSS family. Today owns only the `utilities` it passes in, nothing else about the hero.
