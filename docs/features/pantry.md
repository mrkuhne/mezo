...
Backend: `PantryCatalogMigrationIT` (standalone Liquibase run against a throwaway schema), `PantryCatalogApiIT` (search, from-catalog idempotency, 403/409 edit gates, delete keeps the definition), `PantryCatalogServiceIT` (revive, `ensureItem`, the concurrent-insert race), `PantryCatalogLoaderIT`, `PantryItemRepositoryIT`, `PantryNameIndexTest`, plus the cross-feature auto-add cases in `MealAiDraftServiceIT`/`RecipeWorkshopApiIT`. `PantryApiIT` (`mezo-xaq5`, `mezo-uhe5`, `mezo-3vb1`) covers the honest-null read model end to end: null `category`/`brand`/`pkg`/`price`/`priceUnit` on both the ingredient and stash responses instead of fabricated `""`/`0`, a genuinely-zero price staying distinct from "no price recorded", and a guard test pinning the Postgres-side `lower(trim(...))` natural-key folding assumption (`mezo-imet`) that the honest-null read path relies on. FE: `CatalogSearchSheet.test.tsx`, `KamraCard.test.tsx`, `pantryHooks.test.tsx`, `KamraItemDetailPage.test.tsx` (`inputFromItem`'s per-field `!= null` guards), `FuelStackManagePages.test.tsx`, `pantryPickables.test.ts` (both `pnpm test` and `VITE_USE_MOCK=false pnpm test`).
...
- **Tests:** `backend/src/test/java/io/mrkuhne/mezo/feature/pantry/{PantryCatalogMigrationIT,PantryCatalogApiIT,PantryApiIT,PantryCatalogServiceIT,PantryCatalogLoaderIT,PantryItemRepositoryIT,service/PantryNameIndexTest}.java`, `backend/src/test/java/io/mrkuhne/mezo/support/populator/PantryCatalogPopulator.java`, `frontend/src/data/fuel/{pantryHooks.test.tsx,pantryPickables.test.ts}`, `frontend/src/features/fuel/pages/{KamraItemDetailPage.test.tsx,FuelStackManagePages.test.tsx}`
