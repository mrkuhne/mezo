...
**`kind` and macros contract (`mezo-4orh`, `mezo-6omv`).** `IngredientResponse` itself now carries `kind` (server-computed off `pantry_catalog.kind`), so FE consumers read it straight off the response instead of re-deriving it client-side — the two derivations this replaced, `kamraItems.ts`'s own kind lookup and `pantryPickables.foodKind`, are gone. `PantryMacros`' wire fields — `kcal`, `p`, `c`, `f` (short names on the contract; `PantryCatalogEntity`'s Java getters `getProteinG()`/`getCarbsG()`/`getFatG()` map onto `p`/`c`/`f` in `PantryMapper`) — are all nullable in the contract and on the wire: `PantryMapper` no longer zero-fills a missing macro with `nz()` before serializing, so "no data recorded" (`null`) and "recorded as zero" (`0`) are two distinct, round-tripping states — a FE macro consumer that used to treat every value as a definite number must null-check before formatting/summing (see the split calculator consumers — `recipeMacros.ts`, `RecipeIngredientRow.tsx`, `RecipeEditorPage.tsx`, `fuelHooks.ts`, `MealComposer.tsx` — versus the display consumers — `MacroCells.tsx`, `KamraCard.tsx`, `KamraPickSheet.tsx`, `KamraItemDetailPage.tsx`). Since `mezo-qjwy`, `SupplementStashResponse.macros` also has exactly ONE representation: the object is always present (`required`, `nullable` dropped) and its four fields are individually nullable — the old object-level null said the same thing as an all-null object, and that redundancy is what let a truthiness gate on the detail page show the "log this meal" CTA for a pure dose/protocol item that carries no nutrition at all.

**Honest nulls, the rest of the definition-echo family (`mezo-xaq5`).** `IngredientResponse.brand`/`category`/`price`/`priceUnit`/`pkg` and `SupplementStashResponse.brand`/`category`/`form` are all `nullable: true` on the contract, and `PantryMapper` no longer fabricates a `""`/`0` default for any of them — same root cause as the macros fix above, just the rest of the field list. Why it matters, and — corrected from the original design note — exactly how much each field actually bites:

- `PantryMapper#definitionDiffers` compares the merged request against the stored catalog row field-by-field. A client that echoes a fabricated `""`/`0` default back on an unrelated save (e.g. a price-only edit) makes that field look like a genuine definition CHANGE, which for a non-author/non-OWNER caller means a 403 `PANTRY_CATALOG_NOT_EDITABLE`, and for the author/an OWNER means a silent overwrite of the SHARED row every other user reads — the write only *looks* like a no-op.
- **`pkg` and `form` are the fields that actually 403 today.** `definitionDiffers` has no null-normalization for either, so an echoed `""` reads as a real change and trips the 403 for anyone but the row's author/an OWNER.
- **`brand` and `category` do NOT 403 today**, for two independent reasons that happen to both land on "harmless" — this corrects the original plan, which named `category` as the worst offender because `""` "is not a legal enum value":
  - `brand`: `definitionDiffers` already null-normalizes both sides for this field, so an echoed `""` is treated as equal to a stored `null` — no diff, no 403, no silent rewrite.
  - `category`: the generated `PantryItemRequest.CategoryEnum#fromValue("")` is `@JsonCreator`-annotated and returns `null` for an unrecognized/blank value instead of throwing — so an echoed `""` deserializes to `null` before `definitionDiffers` ever sees it, and the request behaves as if the field were omitted. `""` is still not a *legal* value to have chosen on the wire, but the current server-side handling absorbs it rather than erroring.
- `price` is per-user state, not a shared definition field, so it never reaches `definitionDiffers` and never 403s — but a fabricated `0` used to be indistinguishable from a genuinely free item (a real `Ft 0` row), which is a data-honesty bug on its own, independent of the 403 story.
- **Closing this class of bug needed both ends, not just the mapper.** The server-side honest null only stops the SERVER from fabricating a default on the way OUT. A client can still send `pkg: ""`/`form: ""` on the way IN regardless of what the server ever returned — `KamraItemDetailPage.tsx#inputFromItem` closes that side with a per-field `!= null` guard (`brand`, `category`, `price`, `priceUnit`, `pkg`, `form`) so a field the FE domain model holds as `null` is left OUT of the request instead of round-tripping as `''`/`0`. Either half alone is insufficient: the mapper fix alone doesn't stop a *different* client (a script, a future integration) from sending `pkg: ""`; the FE guard alone doesn't stop the response itself from lying about "no data" vs "empty string" to any consumer, mezo's own FE included, that reads the field before this fix.
...
Backend: `PantryCatalogMigrationIT` (standalone Liquibase run against a throwaway schema), `PantryCatalogApiIT` (search, from-catalog idempotency, 403/409 edit gates, delete keeps the definition), `PantryCatalogServiceIT` (revive, `ensureItem`, the concurrent-insert race), `PantryCatalogLoaderIT`, `PantryItemRepositoryIT`, `PantryNameIndexTest`, plus the cross-feature auto-add cases in `MealAiDraftServiceIT`/`RecipeWorkshopApiIT`. `PantryApiIT` (`mezo-xaq5`) covers the honest-null read model end to end: null `category`/`brand`/`pkg`/`price`/`priceUnit` on both the ingredient and stash responses instead of fabricated `""`/`0`. `PantryImportApiIT` (`mezo-uhe5`) covers a genuinely-zero price staying distinct from "no price recorded" through the import path. `PantryCatalogLoaderIT` (`mezo-3vb1`) carries a guard test pinning the Postgres-side `lower(trim(...))` natural-key folding assumption (`mezo-imet`) that the natural-key lookup (`findByNaturalKey`, §4/§9) relies on — this is unrelated to the honest-null read model. FE: `CatalogSearchSheet.test.tsx`, `KamraCard.test.tsx`, `pantryHooks.test.tsx`, `KamraItemDetailPage.test.tsx` (`inputFromItem`'s per-field `!= null` guards), `FuelStackManagePages.test.tsx`, `pantryPickables.test.ts` (both `pnpm test` and `VITE_USE_MOCK=false pnpm test`).
...
- **Tests:** `backend/src/test/java/io/mrkuhne/mezo/feature/pantry/{PantryCatalogMigrationIT,PantryCatalogApiIT,PantryApiIT,PantryImportApiIT,PantryCatalogServiceIT,PantryCatalogLoaderIT,PantryItemRepositoryIT,service/PantryNameIndexTest}.java`, `backend/src/test/java/io/mrkuhne/mezo/support/populator/PantryCatalogPopulator.java`, `frontend/src/data/fuel/{pantryHooks.test.tsx,pantryPickables.test.ts}`, `frontend/src/features/fuel/pages/{KamraItemDetailPage.test.tsx,FuelStackManagePages.test.tsx}`
