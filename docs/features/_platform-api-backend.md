...
| **Progression** (skill XP · level-up ledger · perk unlocks) | `levelUp` field on the **Train** `WorkoutInstanceResponse` (gym finish) + `RunSessionLogResponse` (run log) — no own fragment | 🟡 `feature/progression` — P1 foundation + **P2 gym engine** + **P3a run engine** | — (FE still mock; wires in P5/P6) | 🟡 P1+P2 (gym) + P3a (run) done (`mezo-8e4`): curve math, 3 owned tables, entities/repos, perk catalog, the shared `award(...)` tail, `GymSignalCalculator`/`RunSignalCalculator` + `ProgressionService.applyGym`/`applyRun`, and the switch-gated `finishWorkout` (gym) + `logSession` (run) triggers returning `levelUp`. **No standalone REST surface yet** — SPORT triggers (P3b) + `GET /progression/profile` (P4) remain. See §4e. |
...
- **Gym finish + run log triggers (P2/P3a, switch-gated)** — both attach the `levelUp` **only** when the progression feature switch is on, each inside the host write's existing `@Transactional` (atomic with the save). The switch is read the house way — **no `@Value`/`Environment`**: a marker bean `feature/progression/ProgressionGate.java` (`@Component @ConditionalOnProperty(name = FeaturesConfiguration.PROGRESSION_SWITCH /* mezo.feature.progression.enabled */, havingValue = "true")`, no `matchIfMissing`) exists **only** when the property is `true`; the host services inject it via `ObjectProvider<ProgressionGate>` and `getIfAvailable() != null` ⇔ switch on. **Gym:** `WorkoutService.finishWorkout` runs `GymSignalCalculator.compute` → `applyGym` → `LevelUpResultMapper.toDto` → `setLevelUp` on `WorkoutInstanceResponse` (the shared `toInstanceResponse` is **unchanged**, so `startWorkout`/resume never carry a `levelUp`; idempotent on the instance id, so a re-finish does not double-award). **Run:** `RunningService.logSession` runs `RunSignalCalculator.compute(userId, savedLogId)` → `applyRun` → `LevelUpResultMapper.toDto` → `setLevelUp` on `RunSessionLogResponse` (only on the POST/save path, never the GET list; idempotent on the saved log id). Contract ITs: `feature/train/WorkoutFinishLevelUpApiIT` (gym finish → `levelUp` source GYM, totalXp > 0, a `chest` gain) + `feature/train/RunSessionLevelUpApiIT` (sprint run log → `levelUp` source RUN, a `sprint_speed` gain).
...
**Progression (P1 foundation + P2 gym engine + P3a run engine, `mezo-8e4` — `levelUp` on the Train contract only; see §4e)**
- `feature/progression/{ProgressionCurve,PerkCatalog,ProgressionGate}.java` + `config/ProgressionProperties.java` (`mezo.progression.{curve,gym,run}`); content `src/main/resources/content/progression-perks.json`
...
- engine `feature/progression/service/{ProgressionService,RobustnessCalculator}.java` (`applyGym`/`applyRun` over the shared private `award(...)` tail); gym signal `gym/{GymSignal,GymSignalCalculator}.java`; run signal `run/{RunSignal,RunSignalCalculator}.java`; DTO map `mapper/LevelUpResultMapper.java`
- triggers (switch-gated) on the Train side: `WorkoutService.finishWorkout` (gym → `WorkoutInstanceResponse.levelUp`) + `RunningService.logSession` (run → `RunSessionLogResponse.levelUp`)
- **no** `api/feature/progression/` fragment, controller, or FE wiring — SPORT trigger is P3b, the `GET /progression/profile` read surface is P4
...
- canonical ITs: `feature/biometrics/BiometricsContractIT.java`, `feature/train/ProvenanceRoundTripIT.java`; G1: `feature/goal/{GoalServiceIT,GoalContractIT}.java`, `feature/biometrics/profile/{BiometricProfileServiceIT,BiometricProfileContractIT}.java`; G3: `feature/goal/{GoalPlanLinkServiceIT,GoalTimelineServiceIT,GoalTimelineContractIT}.java`; Pantry (`mezo-9xu`): `feature/pantry/{PantryItemRepositoryIT,PantryServiceIT,PantryApiIT}.java`; Progression P1+P2+P3a (`mezo-8e4`): `feature/progression/{ProgressionCurveTest,PerkCatalogIT,ProgressionPersistenceIT,ProgressionPopulatorIT}.java` + engine `service/{ProgressionServiceIT,ProgressionRunIT}.java` + `run/RunSignalCalculatorIT.java` + Train-side triggers `feature/train/{WorkoutFinishLevelUpApiIT,RunSessionLevelUpApiIT}.java`
