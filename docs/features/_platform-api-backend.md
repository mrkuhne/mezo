...
| **People** | — | ❌ | `data/me/people.ts` | 🔶 mock-only. |
...
Confirmed by grep: only `data/hooks.ts`, `data/train/trainHooks.ts`, `data/train/runningHooks.ts`, `data/me/weightHooks.ts`, `data/me/biometricHooks.ts`, `data/me/goalHooks.ts`, `data/fuel/pantryHooks.ts` reference `isMockMode`; `fuel.ts`/`insights.ts`/`people.ts`/`pantry.ts` (the static seeds) contain **no** `apiFetch` (real Pantry traffic flows through `data/fuel/pantryApi.ts`, not the `data/fuel/pantry.ts` seed). `api/base.yml` `info.title` is "mezo API".
...
- mock-only (no backend yet): `frontend/src/data/fuel/fuel.ts`, `pantry.ts` (the static Pantry/recipe seed — the backed inventory CRUD flows through `data/fuel/pantryApi.ts`, not this file), `fuelWeek.ts`, `insights.ts`, `people.ts`
