...
| **Proactive** (morning briefing) | `api/feature/proactive/proactive.yml` | ✅ `feature/proactive` (dual-switch: companion + proactive) | — (FE wiring ⛔ until B1.2) | 🟡 B1.1 — lazy `GET /api/proactive/briefing` (generation-on-read; honest 404 on empty window / unusable answer, idempotent row). Reuses the companion reads + `CompanionLlm` behind both `mezo.feature.companion.enabled` **and** `mezo.feature.proactive.enabled`. See [`companion.md`](companion.md). |
