...
### 4e. The progression domain (P1 foundation + P2 gym-path engine, NO HTTP surface yet, `mezo-8e4`)

> **Engine landed, HTTP surface still deferred.** P1 landed the persistence + pure-logic spine for the skill-progression / "level-up" system; **P2 (gym path) adds the engine that *grants* XP** — `GymSignalCalculator` (extracts a `GymSignal` from a finished gym instance) + `ProgressionService.applyGym` (awards XP, levels, perks, robustness — see below). The gym/sport/run **completion triggers** that call the engine and the **HTTP read surface** remain deferred (later P2 tasks + P4). There is still **no `api/feature/progression/` fragment, no controller/mapper, and no FE wiring** yet — so unlike every other §4 aggregate, progression contributes nothing to `api/openapi.yml` or `frontend/src/lib/api.gen.ts`. The frontend's progression views stay mock-only until the read surface lands.
