...
| **Progression** (skill XP · level-up ledger · perk unlocks) | `levelUp` field on the **Train** `WorkoutInstanceResponse` (gym finish) + `RunSessionLogResponse` (run log) — no own fragment | 🟡 `feature/progression` — P1 foundation + **P2 gym engine** + **P3a run engine** + **P3b sport engine** | — (FE still mock; wires in P5/P6) | 🟡 P1+P2 (gym) + P3a (run) + **P3b (sport)** done (`mezo-8e4`): curve math, 3 owned tables, entities/repos, perk catalog, the shared `award(...)` tail, `Gym`/`Run`/`SportSignalCalculator` + `ProgressionService.applyGym`/`applyRun`/`applySport`, and the switch-gated `finishWorkout` (gym) + `logSession` (run) + `logSportSession` (sport) triggers returning `levelUp`. **No standalone REST surface yet** — `GET /progression/profile` (P4) remains. See §4e. |
...
**Progression (P1 foundation + P2 gym engine + P3a run engine + P3b sport engine, `mezo-8e4` — `levelUp` on the Train contract only; see §4e)**
- `feature/progression/{ProgressionCurve,PerkCatalog,ProgressionGate}.java` + `config/ProgressionProperties.java` (`mezo.progression.{curve,gym,run,sport,robustness}`); content `src/main/resources/content/progression-perks.json`
...
- engine `feature/progression/service/{ProgressionService,RobustnessCalculator}.java` (`applyGym`/`applyRun`/`applySport` over the shared private `award(...)` tail); gym signal `gym/{GymSignal,GymSignalCalculator}.java`; run signal `run/{RunSignal,RunSignalCalculator}.java`; sport signal `sport/{SportSignal,SportSignalCalculator}.java`; DTO map `mapper/LevelUpResultMapper.java`
