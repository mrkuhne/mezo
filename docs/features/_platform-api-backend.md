...
| **Proactive** (morning briefing) | `api/feature/proactive/proactive.yml` | ✅ `feature/proactive` (dual-switch: companion + proactive; + `briefing-job` cron switch) | ✅ Today card real (`useBriefing` → `useToday`, honest fallback) | 🟢 B1.2 — `GET /api/proactive/briefing` (persisted row · dawn `BriefingJob` 05:45 today-only · sleep-triggered capped regen `regen_count`; honest 404 on empty window / unusable answer / failed regen). Reuses the companion reads + `CompanionLlm` behind both `mezo.feature.companion.enabled` **and** `mezo.feature.proactive.enabled`. See [`proactive.md`](proactive.md). |
