...
| **AI brain / Companion** (Spring AI, pgvector, RAG) | — | 🟡 `feature/companion` — V0.1 skeleton | — | 🟡 Phase-3 started (`mezo-fnnq.1`, [ADR 0008](../../docs/decisions/0008-companion-llm-spring-ai-2-gemini.md)): Spring AI 2.0 BOM + Gemini starter in `pom.xml`; `CompanionLlm` port + real/fake adapters behind `mezo.feature.companion.enabled` (fake under the `companion-fake` profile — ITs never touch the network); model tiers in `mezo.companion.llm.*` (`CompanionProperties`). No endpoint/table yet — the chat spine + `ai_conversation`/`ai_message` land at V0.2, pgvector at V2.1. |
