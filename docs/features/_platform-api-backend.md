...
| **AI brain / Companion** (Spring AI, pgvector, RAG) | `api/feature/companion/companion.yml` (V0.2) | 🟡 `feature/companion` — V0.1 port + **V0.2 chat spine** | — (FE mock until V0.4) | 🟡 Phase-3 in progress. **V0.1** (`mezo-fnnq.1`, [ADR 0008](../../docs/decisions/0008-companion-llm-spring-ai-2-gemini.md)): Spring AI 2.0 BOM + Gemini starter in `pom.xml`; `CompanionLlm` port + real/fake adapters behind `mezo.feature.companion.enabled` (fake under the `companion-fake` profile — ITs never touch the network); tiers in `mezo.companion.llm.*`. **V0.2** (`mezo-fnnq.2`): the chat spine — `ai_conversation` + `ai_message` tables (typed-but-null `tool_calls`/`refs` jsonb), the `companion.yml` fragment (`GET/POST /api/companion/conversation`, `GET .../messages`, `POST .../message` — **sync JSON**, SSE deferred to V0.4), `ConversationService` + `ChatService` (static HU voice + last-20-msg windowing → `CompanionLlm.complete`); every bean switch-gated (off ⇒ `/api/companion/*` 404s). Streaming/FE at V0.4, pgvector at V2.1. See [`companion.md`](companion.md) §3–§4. |
