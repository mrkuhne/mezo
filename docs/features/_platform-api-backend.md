...
| **Progression** (skill XP · level-up ledger · perk unlocks) | `levelUp` field on the **Train** `WorkoutInstanceResponse` (no own fragment) | 🟡 `feature/progression` — P1 foundation + **P2 gym engine + gym finish trigger** | — (FE still mock; wires in P5/P6) | 🟡 P1+P2 gym done (`mezo-8e4`): curve math, 3 owned tables, entities/repos, perk catalog, `GymSignalCalculator` + `ProgressionService.applyGym`, and the switch-gated `finishWorkout` trigger returning `levelUp`. **No standalone REST surface yet** — sport/run triggers + `GET /progression/profile` are P3/P4. See §4e. |
...
### 4e. The progression domain (P1 foundation + P2 gym-path engine + gym finish trigger, `mezo-8e4`)

> **Engine + gym trigger landed; standalone HTTP read surface still deferred.** P1 landed the persistence + pure-logic spine for the skill-progression / "level-up" system; **P2 (gym path) adds the engine that *grants* XP** — `GymSignalCalculator` (extracts a `GymSignal` from a finished gym instance) + `ProgressionService.applyGym` (awards XP, levels, perks, robustness — see below) — **and wires it into the gym finish flow** (`WorkoutService.finishWorkout`, see below). The gym finish now returns a `levelUp` on `WorkoutInstanceResponse` (the only progression HTTP surface so far — it rides the existing Train contract, not a new fragment). The **sport/run completion triggers** and the **standalone progression read surface** (`GET /api/progression/profile`, radar/streak) remain deferred (P3 + P4). There is still **no `api/feature/progression/` fragment and no FE wiring** yet; the frontend's progression views stay mock-only until P5/P6. So unlike every other §4 aggregate, progression contributes only the `levelUp` field on the Train contract to `api/openapi.yml` / `frontend/src/lib/api.gen.ts`.
...
- **Gym-path XP engine (P2)** — `service/ProgressionService.applyGym(createdBy, GymSignal) → LevelUpResult` (`@Transactional`, `@Service`): **idempotent** per `(GYM, instanceId)` via the `level_up_event` lookup — a re-applied workout returns the **stored `payload`** without re-awarding. It computes per-skill XP deltas from `ProgressionProperties.gym()` (muscle `volume/volumeUnit*volumeXpPerUnit`, integer; `max_strength = bestE1rm.intValue()*e1rmXpPerKg + prBonusXp` where the **v1 PR rule** awards the bonus only on the **first-ever weighted session** — no prior `max_strength` row; `strength_endurance = workSetCount*xpPerSet + bodyweightRepCount*xpPerRep`), upserts `skill_progress`, derives level-ups via `ProgressionCurve.levelFor`, unlocks perks once per `perk_key` at every milestone strictly crossed (5/10/15/20/25/30) via `PerkCatalog.find`, and persists exactly one `level_up_event`. **Robustness** is a streak-only **absolute target** (`streakWeeks*perWeekXp`) recomputed each apply → idempotent within a week. Streak comes from `service/RobustnessCalculator.streakWeeks` — consecutive ISO weeks (Europe/Budapest) with ≥1 logged gym instance / sport / run, gathered via `WorkoutSessionRepository.findInstanceDates` (added here), `SportSessionRepository.findByCreatedByAndDeletedFalseOrderByDateDesc`, `RunSessionLogRepository.findByCreatedByAndDeletedFalseOrderByDateDesc` (known v1 limitation: the week-key walk-back under-counts across ISO-year boundaries). IT: `feature/progression/service/ProgressionServiceIT` (3 tests: first-apply XP/level-up, idempotency, prior-row PR rule).
- **Gym finish trigger (P2, switch-gated)** — `WorkoutService.finishWorkout` (`feature/train/service/WorkoutService.java`) computes + attaches the `levelUp` **only** when the progression feature switch is on, inside its existing `@Transactional` (atomic with the completion). The switch is read the house way — **no `@Value`/`Environment`**: a marker bean `feature/progression/ProgressionGate.java` (`@Component @ConditionalOnProperty(name = FeaturesConfiguration.PROGRESSION_SWITCH /* mezo.feature.progression.enabled */, havingValue = "true")`, no `matchIfMissing`) exists **only** when the property is `true`; `WorkoutService` injects it via `ObjectProvider<ProgressionGate>` and `getIfAvailable() != null` ⇔ switch on. When on, it runs `GymSignalCalculator.compute(createdBy, instanceId)` → `ProgressionService.applyGym` → `LevelUpResultMapper.toDto` and sets the result on the response via the generated `setLevelUp` setter (the shared `toInstanceResponse` is **unchanged**, so `startWorkout`/resume never carry a `levelUp`). Because `applyGym` is idempotent on the instance id, a re-finish does not double-award. Contract IT: `feature/train/WorkoutFinishLevelUpApiIT` (gym finish returns a populated `levelUp` — source GYM, totalXp > 0, a `chest` gain).
