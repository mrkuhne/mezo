...
- **Token cascade.** `:root` defines all vars (dark); `:root[data-theme="light"]` overrides the contrast-sensitive subset (`prototype.css:96–154`). Components reference `var(--…)` only; nothing hardcodes hex except a couple of intentionally-branded data palettes (`data/nova.ts`, `data/pantrySources.ts`). **Theme-invariant exception:** `--text-on-media` / `--text-on-media-dim` (defined once in `:root`, deliberately NOT overridden for light) label text that overlays the recipe hero/list image-placeholder bands — those bands are hardcoded-dark in _both_ themes, so their overlay text must stay light in both (`RecipeCard.tsx`, `RecipeDetailPage.tsx` hero). Their values mirror the dark `--text-primary`/`--text-secondary` tones, keeping dark mode pixel-identical.
- **Elevation & bezel tokens (theme-driven).** Light mode lifts cards off the canvas; dark stays flat. Three tokens carry this, defined in `:root` (dark) and overridden in the light block: `--card-elevation` (light = a two-layer drop-shadow, dark = `none`), `--card-border` (light `rgba(15,23,35,0.10)`, dark `var(--border-subtle)`), and `--page-glow` (the desktop bezel's radial-gradient center stop — `.app-root` is now token-driven in both themes, `prototype.css:191`). `.card` applies both via `border: 1px solid var(--card-border)` + `filter: var(--card-elevation)` (`prototype.css:435–438`). **Why `drop-shadow` (a `filter`), not `box-shadow`:** the notch `clip-path` on `.card.notch-*` clips a `box-shadow` to the chamfered silhouette (the shadow disappears), whereas `filter: drop-shadow()` is applied _after_ the clip and traces the notched outline correctly. Glass surfaces (`PatternCard`, `MacroCells`, `RecipeDetailPage`'s `MacroHeroCell`) use `var(--surface-glass)` rather than a hardcoded white `rgba()` so they read correctly in both themes.
