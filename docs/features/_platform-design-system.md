...
- **`.eyebrow` / `.label-mono`** — **mono is retired from both**; they are now the Jakarta small-caps idiom (`--ff-body`, 10–11px / 700–800, letter-spaced, uppercase, `--faint`; `.eyebrow.brand` → `--coral-deep`, `prototype.css:338–351`). **`.toolchip` deliberately keeps `--ff-mono`** — the tool-transparency row stays monospace per spec §3.6 (`prototype.css:430–440`).
- **`.h-display` / `.page-title`** — **uppercase retired**; both render sentence-case Bricolage display (`prototype.css:352–371`).
- **`.chip`** — a pill again (`border-radius:999px`), with **`.chip.brand`** the coral *selected* state (`--wash-gym` fill + `--coral` border + `--coral-deep` text, `prototype.css:401–417`).
...
- **Visual regression — self-baselined Playwright harness (S8, `mezo-mifi`).** The old-prototype pixel-parity suite (`frontend/tests/parity/`, `pnpm parity`) was retired by the Napív redesign (2026-07-13, `mezo-8141`); S8 replaced it with a **self-baseline** harness at **`frontend/tests/visual/`** (`visual.spec.ts` + `playwright.config.ts`): **14 key screens × 2 themes = 28 committed darwin goldens** (`visual.spec.ts-snapshots/*-darwin.png`). Run `pnpm test:visual` (compare) / `pnpm test:visual:update` (re-baseline). **Determinism levers** (all must hold or the shots flake): the **clock is frozen** to `2026-05-21T13:42` (délután) before `goto` — pins the daypart sky/greeting AND matches the StatusBar's hardcoded 13:42; the **theme** is set via a `localStorage['mezo-theme']` **init script before `goto`** (the pre-paint `index.html` script then stamps `data-theme`); **`reducedMotion:'reduce'`** + Playwright's default `animations:'disabled'` kill in-flight transitions; and the test **waits for `document.fonts.ready`** so the pixel compare runs on real Bricolage/Jakarta metrics, not fallbacks. **LOCAL-only for now** — the goldens are `-darwin` platform snapshots; a **CI Linux snapshot-generation job is a filed follow-up** (they'd mismatch `ubuntu-latest` font rendering). The vitest render tests above stay the mode-agnostic unit layer beneath it.
