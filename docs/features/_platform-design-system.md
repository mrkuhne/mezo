...
- **Sport → `--tag-sport`/`--wash-sport`** (rose) — `SportPage.tsx`. **Superseded by Napiv S4** the pre-S4 `--cat-tendency` (pink), which still lingers on `TrainTodayPage`'s volleyball-hero status chip (train.md §9).
- **Running / "Futás" → `--tag-run`/`--wash-run`** (sky) — `RunningPage.tsx`. **Superseded by Napiv S4** the pre-S4 `--info` (blue), which still lingers on `TrainTodayPage`'s run-hero status chip and on the un-re-skinned `RunningBlockBuilderPage`/`WeekdayGrid`/`RunWeekEditor`.
