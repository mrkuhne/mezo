...
- `SubNavDropdown.tsx` — compact sub-nav popover for `AppHero`'s `utilities` slot (chip + `role="menu"` popover, `SubNavItem`/`SubNavExtraAction` types; compact-header redesign, `mezo-ugqb`); replaces the retired per-domain `*SubNav` components. Consumed by Train/Fuel/Me/Insights — see §3/§5.
...
**Tests**
- `frontend/src/shared/ui/*.test.tsx` (16, incl. `SubNavDropdown.test.tsx`) — render/unit tests.
