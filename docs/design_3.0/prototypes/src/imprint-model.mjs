export function imprintWeek(s) {
  ...
  const logged = s.full?.training.gymHistory.filter(h=>h.date==='2026-09-08') || [];
  logged.forEach(h=>today.push({id:`gym-${h.id}`,kind:'movement',title:h.name||h.title,detail:`Rögzített edzés · ${h.sets} sorozat · ${h.volume} kg összvolumen.`,time:'Ma',planned:false}));
  if(!logged.length) today.push({id:'gym',kind:'movement',title:s.training.cycle.session,detail:`A hypertrophy mezociklus ${s.training.cycle.week}. hetének tervezett edzése.`,time:s.training.cycle.time,planned:true});
