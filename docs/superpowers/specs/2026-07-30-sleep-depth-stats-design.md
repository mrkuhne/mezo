...
> **`durationH` rides along too — and this spec originally missed that.** Manual mode derives `durationH` from the bedtime→wakeup span, i.e. *in-bed* time, while `SleepEntry.duration` means *asleep* hours everywhere else in the codebase. Carrying the phase payload without the duration writes a row where the 48px hero number (8.3 h for the canonical night) contradicts the rail directly beneath it (7.5 h of phases), and inflates `efficiencyPct` to ~99% against a true ~90%. The contradiction is **created by the leak fix succeeding** — before it, a manual row carried no phases, so there was nothing to disagree with. Whenever a draft exists, the draft's duration wins (the editable duration input still overrides it); with no draft, manual behaviour is untouched.
