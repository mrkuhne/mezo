...
6. **Kihívások** (csak ha van) — soronként: állapot-roundel (hit: sage ✓ · miss: warning ◯ · skipped/inconclusive: neutrális ⊘/◌) + `typeLabel · exercise` + mono részletsor (`detail ?? target`) + jobbra **kimenet-chip** a meglévő vocabulary-vel (`megcsináltad` / `nem jött össze` / `skippelted` / `nem értékelhető`) a státusz wash-színében. Section-label számlálóval (`1 megvan · 1 kimaradt`).
...
Minden adat meglévő hookból: `useWorkoutDetail` (closed), a session state + `useChallenges` + `sessionMedals` (closing). A `/train/review` ma egyáltalán nem kap medált — a redesignnal a meglévő `useMedals()` (dual-mode, `GET /api/train/medals`) szolgálja ki, `m.workoutSessionId === workoutId` szűréssel (a `complete` fázis marad a memóriabeli `sessionMedals`-on); üres szűrés → nincs medál-section, ahogy eddig. `Medal.setIndex` a logolt szettek 0-bázisú pozíciója (a session-flow `medalsBySet` kulcsa és a perzisztált `setIndex` is ez), így mindkét módban a szett-tömb indexére illeszthető. Nincs backend-hívás-változás. Őszinteség: Ø RIR 0 logolt szettnél `–`; medál/kihívás-section üresen **nem renderel**; `durationEst` hiányában (0/undefined) az idő-tag kimarad a subtitle-ből; a jegyzet closed módban nem jelenik meg, mert nem perzisztál (`mezo-s52z`); a Rekord/Célszett cella 0-nál nem színez (nem ünneplünk semmit).
