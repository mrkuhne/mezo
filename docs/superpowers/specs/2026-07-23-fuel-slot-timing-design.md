...
| D4 | Re-flow semantics | **Now-aware redistribution.** On every plan build, PENDING meal windows are re-spaced evenly between `max(now, last logged meal + 90 min)` and kitchen-close, keeping training snaps and the 90-min minimum gap; a pending slot never renders in the past. Deterministic — same inputs, same plan (`nowHHmm` stays an injected input). Done slots keep rendering at their actual `loggedAt`. |
