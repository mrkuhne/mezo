...
| Edit payload | `per`/`unit` **omitted entirely** | backend `applyRequestPartial` is PATCH-merge (`setIfPresent`) — the stored basis stays untouched, protecting the intentional per-serving legacy row (`Iso Whey Nutriversum Vanilla`, per=30) |
