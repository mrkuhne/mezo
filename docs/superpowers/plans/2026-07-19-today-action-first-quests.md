...
- [ ] **Step 1:** In `quest.yml` add `metric` to `QuestResponse.required` and properties:
  ```yaml
  ...
  ```
- [ ] **Step 2:** Regenerate: `cd api/generate && npm run generate:api`, then `cd frontend && pnpm generate:api`.
- [ ] **Step 3:** `QuestMapper`: add `@Mapping(target = "metric", source = "target.metric")` above `toQuestResponse`.
- [ ] **Step 4:** Extend `QuestApiIT`: in `testGetQuestDay_shouldLazilyGenerateThreeSlots_whenTodayAndNoRows` add
...
- [ ] **Step 5:** Run: `cd backend && docker compose up -d && ./mvnw clean test -Dtest='QuestApiIT'` → PASS.
- [ ] **Step 6:** Commit: `feat(api): expose quest target metric on the wire (mezo-gj2y)`
...
- [ ] **Step 1:** `types.ts`: add `metric: string` to `DailyQuest` (after `targetLabel`).
- [ ] **Step 2:** `questApi.toQuest`: add `metric: w.metric ?? ''` (defensive default keeps old fixtures alive).
- [ ] **Step 3:** `questMock.ts`: seed metrics — `dq1: gym_session_done`, `dq2: weight_logged`, `dq3g: activity_match`; history `qh1: weight_logged`, `qh2: gym_session_done`, `qh3: activity_match`, `qh4: water_target`; spare `mockRerollSpare: sleep_target`.
- [ ] **Step 4:** MSW reroll fixture gets `metric: 'weight_logged'`.
- [ ] **Step 5:** `questHooks.ts`: `staleTime: mock ? Infinity : 0` with a comment (read-triggered evaluation heartbeat — the quest domain wants frequent reads).
- [ ] **Step 6:** `useWaterActions` real `onSuccess` also invalidates `{ queryKey: ['dailyQuests', date] }`; `useCheckins`'s mutation `onSuccess` likewise.
- [ ] **Step 7:** Extend the three hook tests: metric present on the mock seed / parsed wire; water + check-in saves invalidate the quest key (spy on `queryClient.invalidateQueries`).
- [ ] **Step 8:** Run: `cd frontend && pnpm test src/data` (both modes) → PASS.
- [ ] **Step 9:** Commit: `feat(data): quest metric on FE types + quest-day freshness invalidation (mezo-gj2y)`
...
- [ ] **Step 1:** Write `questAction.test.ts` (table-driven over the 8 metrics + ACTIVITY precedence + unknown→null). Run → FAIL (module missing).
- [ ] **Step 2:** Implement `questAction.ts`. Run → PASS.
- [ ] **Step 3:** Commit: `feat(today): questAction smart-CTA mapping (mezo-gj2y)`
...
- [ ] **Step 1:** Write `TodayQuestsCard.test.tsx` (header + link, water CTA → `logWater(250)`, checkin CTA → callback / hidden without it, nav CTA navigates, ACTIVITY opens sheet, completed `+XP` chip, expired dimmed + no chip, ghost, level-up fire+consume). Run → FAIL.
- [ ] **Step 2:** Implement `ZoneDivider.tsx` + `.zonediv` CSS + `TodayQuestsCard.tsx` (2-line title clamp, `.card`/`.chip`/`.eyebrow` idiom, same level-up effect as `DailyQuestsCard`).
- [ ] **Step 3:** Run the component test in both modes → PASS.
- [ ] **Step 4:** Commit: `feat(today): compact TodayQuestsCard with smart log CTAs + ZoneDivider (mezo-gj2y)`
...
- [ ] **Step 1:** Update `TodayPage.test.tsx`: order assertions (both dividers via `role="separator"` names, quest header before `Hogy vagy ma?`, briefing text after it), GrowthTodayRow markup (`Növekedés ma`) absent. Run → FAIL.
- [ ] **Step 2:** Rewrite `TodayPage.tsx` composition; delete `GrowthTodayRow.tsx` + its test; remove the `.growrow` CSS block if now-unused (grep first).
- [ ] **Step 3:** Run the whole today feature suite in both modes → PASS.
- [ ] **Step 4:** Commit: `feat(today): action-first zones — quests inline, briefing below check-in, GrowthTodayRow retired (mezo-gj2y)`
...
- [ ] **Step 1:** Update both feature docs to the new composition (GrowthTodayRow → TodayQuestsCard; metric on the wire; freshness rules).
- [ ] **Step 2:** `node scripts/lint-docs.mjs` → clean.
- [ ] **Step 3:** Full FE gate: `cd frontend && pnpm build && pnpm test && VITE_USE_MOCK=true pnpm test` → all green.
- [ ] **Step 4:** Commit: `docs(features): today action-first re-composition + quest metric seam (mezo-gj2y)`

### Task 7: Ship (self-PR CI gate)

- [ ] **Step 1:** `git push -u origin feat/today-action-first`; `gh pr create` (self-PR = CI trigger).
- [ ] **Step 2:** Wait for CI green (`gh pr checks --watch`).
- [ ] **Step 3:** `git checkout main && git pull --rebase && git merge --no-ff feat/today-action-first && git push`; delete the branch.
- [ ] **Step 4:** `bd close mezo-gj2y`; `bd dolt push`; `git status` → up to date.
