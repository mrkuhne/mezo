...
```java
     ...
     * {@code ZoneId.systemDefault()} — ahogy a {@code MealCoachService} is teszi. Következmény: egy
     * csupasz {@code Z}-instanttal logolt régi sor (mezo-g8qm) MÁS {@code MealRole}-lal jöhet
     * vissza; ez a fix vállalt ára, nem meglepetés.
     *
