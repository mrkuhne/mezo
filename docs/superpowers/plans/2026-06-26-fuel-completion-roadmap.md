...
**Where we are.** 5 of 9 Fuel hooks are real/dual-mode (`useFuelDay`, `useMealActions`, `useRecipeLogs`, **`useStack`, `useProtocol`** — plus `usePantry`/`useRecipes`/`useMedication`). 4 remain mock-only one-liner const spreads (`useFuelTimeline`, `useFuelWeek`, `useReplanScenarios`, `useStackRecommendations` — the last is mode-aware, `[]` in real). `api/feature/fuel/fuel.yml` + `backend/.../feature/fuel/` + `frontend/src/data/fuel/fuelApi.ts` now **exist** (the Stack/Protocol slice `P2`/`mezo-09g` created them); the day/timeline/week surfaces (`P4`/`P5`) extend the same fragment. Source map: `docs/features/fuel.md` (living doc), master design spec `docs/superpowers/specs/2026-06-10-phase2-backend-design.md` (Slice C row).
...
## P2 — Stack / Protocol ✅ SHIPPED (mezo-09g, 2026-07-02)

> **Shipped — the Stack tab is backend-backed.** First Fuel-owned package (`backend/.../feature/fuel` + the new `api/feature/fuel/fuel.yml`): `protocol` + `protocol_item` + `supplement_intake` tables, `ProtocolService` (activate = supersede + `v+1`, confidence from `mezo.fuel.protocol.default-confidence`) + `IntakeService`, `FuelController`. FE: dual-mode `useStack`/`useProtocol` + `useStackActions`/`useProtocolActions` (`data/fuel/fuelApi.ts` + `data/fuel/stackHooks.ts`); `FuelStackPage` "Bekapcsolás · ma" is a real activation + slot tap-to-log intakes; `FuelMaiPage` protocol-meta reads the real protocol (hidden at v0). Living doc: `docs/features/fuel.md` §1–§5/§9/§10. Design spec: `docs/superpowers/specs/2026-07-02-fuel-stack-protocol-water-design.md`.
> **Key divergences from the brief below:**
> - The protocol persists **selection + version metadata ONLY** — `buildProtocol` stays deterministic and the timing **slots are FE-computed**, not stored (recompute-on-read, via `protocol` + `protocol_item` join to `pantry_item`).
> - Endpoints landed as `GET/POST /api/fuel/protocol` + `GET/POST /api/fuel/intake` + **`DELETE /api/fuel/intake/entry/{id}`** — the `entry` path segment avoids the `/{date}` path-param collision (NOT the brief's `/api/fuel/supplement-intake`, and NOT `/api/fuel/stack`).
> - **`"Mentés protokollként"` is deferred** (`· hamarosan`, disabled) — only "Bekapcsolás · ma" (activate) shipped.
> - **`mezo-4nu` item #1 (FuelStackView `/api/goals` decouple) shipped here** via the new `useStackContext()` (static meso week + short-title seed consts) — items 2-4 of mezo-4nu stay open.
> - `useStackRecommendations` stays **`[]` in real mode** until `P8` (mode-aware; mock keeps the seed).
> - `useStack` **composes** `usePantry().stash` (single source of truth) + an intake overlay — no 2nd stash store.
