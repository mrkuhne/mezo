...
**Out/deferred:** a personalized appetite curve fitted to logged pacing → `P8`. Cross-domain Reta strings in Insights/Train/Goals/Chat stay frozen mock until Slice D.
**Backend:** `medication` (drug def: Retatrutide, 6 mg, subQ, weekly cadence, kinetic-cycle params) + `medication_dose` (each injection: `administered_at`, dose). Derivation service: `retaDay = days-since-last-dose mod 7 (+1)`, phase from config boundaries. `fuel.yml`: `GET /api/fuel/medication`, `POST /api/fuel/medication-dose`, derived `{retaDay, phase, retaWeek}` folded into the day/timeline payload. Config: phase boundaries + kcal-floor-per-phase as `mezo.*` props. demodata: a recent Monday dose so derived `retaDay` matches the demo.
**FE hook swap:** `useTodayScenario().retaDay` derives from the real last-dose in real mode (**keep the `?retaDay=` URL override for demos/tests** — `hooks.test.tsx` asserts it). `useFuelWeek` `retaWeek` reflects the real medication; new `useMedicationActions.logDose`. This is the highest-leverage swap (every Reta surface reads `retaDay`).
**Depends on:** `P2` (shares the supplement/medication seam + the `fuel.yml` fragment); `P0b`. **Size:** M. **Deterministic** (config + date-math).
**Open decisions:** model `reta` as `pantry_item` / `medication` / both (likely: `medication`+`medication_dose` own the cycle, `pantry_item` keeps the shelf card, de-dup via `stashRefId`); server-computed vs FE-computed `retaDay`; generalize cadence beyond weekly-Monday.
**bd:** `mezo-d94`.
