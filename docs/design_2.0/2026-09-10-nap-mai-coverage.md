...
| C4 | Életjel tile: needs ring gradient → `/nap/eletjel` | NapHubPage.tsx:371-379, `useNeeds(tick)`, `needRingGradient` | Day panel; FE-computed rings | Daily glance | Needs colors tint/illuminate the companion form itself (companion as live gauge); tap opens `/nap/eletjel` | MERGE | `NapHubPage.test.tsx` — "C4: a társ maga az Életjelek ajtaja"; `NapCompanion.test.tsx` — "tap opens the signals surface and the button is labelled", "aura colors come from the need meta, per band" (a jel a visszaöltöztetés óta az agyag Mezo-szimbólum + szükséglet-halo, mezo-ju4j6.10) |
