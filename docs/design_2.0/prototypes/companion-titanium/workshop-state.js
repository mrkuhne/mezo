...
const TEMPLATES={
 high_protein:{name:'Fehérjés csirkés tál',servings:2,lines:[pantryLine('k-csirke','Csirkemell',300),pantryLine('k-rizs','Jázmin rizs',140),pantryLine('k-joghurt','Görög joghurt',150),estimateLine('Szezámmag',15,{kcal:86,p:2.7,c:3.5,f:7.5,fiber:1.8,sugar:.1,salt:0,satfat:1.1,nova:1})],steps:['A rizst főzd meg enyhén sós vízben.','A csirkét csíkozd fel, és süsd aranybarnára.','A joghurtot keverd ki citrommal és fokhagymával.','Tálald a rizsre, és szórd meg szezámmal.']},
 pre_workout:{name:'Banános zabkása edzés előtt',servings:1,lines:[pantryLine('k-zab','Zabpehely',70),pantryLine('k-banan','Banán',120),pantryLine('k-joghurt','Görög joghurt',150),estimateLine('Méz',10,{kcal:30,p:0,c:8.2,f:0,fiber:0,sugar:8.2,salt:0,satfat:0,nova:2})],steps:['A zabot főzd puhára vízzel.','Keverd bele a joghurtot.','Tetejére banán és egy csurgatás méz.']},
 post_workout:{name:'Edzés utáni csirke-rizs',servings:1,lines:[pantryLine('k-csirke','Csirkemell',200),pantryLine('k-rizs','Jázmin rizs',100),estimateLine('Brokkoli',120,{kcal:41,p:3.4,c:8,f:.4,fiber:3.1,sugar:2,salt:.04,satfat:0,nova:1})],steps:['Főzd meg a rizst.','A csirkét és a brokkolit süsd egy serpenyőben.','Tálald együtt.']},
 before_bed:{name:'Esti joghurtos tál',servings:1,lines:[pantryLine('k-joghurt','Görög joghurt',250),pantryLine('k-zab','Zabpehely',30),estimateLine('Dió',15,{kcal:98,p:2.3,c:2,f:9.8,fiber:1,sugar:.4,salt:0,satfat:.9,nova:1})],steps:['A joghurtot tedd tálba.','Szórd meg zabbal és dióval.']},
...
export function draftFromRecipe(recipe,pantry){
 ...
 const lines=recipe.lines.map(([name,amountText,kcal,nova])=>{
  ...
  return estimateLine(name,amount,{kcal:kcal||0,p:recipe.p*whole*share,c:recipe.c*whole*share,f:recipe.f*whole*share,nova:nova??null},unit);
 });
...
// Live preview of what the saved recipe page will show: quality inputs + per-serving micronutrients.
// A rollup is null only when no line knew the value; otherwise it is the sum of the known ones.
const PLANT=/rizs|zab|banán|brokkoli|dió|szezám|gyümölcs|zöldség|paprika|tortilla|bulgur|lencse|alma|méz/;
export function draftNutrition(draft,pantry){
 const sums={fiber:null,sugar:null,salt:null,satfat:null},qualityLines=[];let plants=0;
 for(const line of draft.lines){
  const item=line.source==='pantry'?pantry.find(k=>k.id===line.refId):null;
  const grams=line.unit==='db'?line.amount*(line.gramsPerUnit||55):line.amount;
  const factor=line.source==='pantry'?grams/100:(line.estAmount>0?line.amount/line.estAmount:0);
  const facts=item?{fiber:item.fiber100,sugar:item.sugar100,salt:item.salt100,satfat:item.satfat100}:(line.est??{});
  for(const key of Object.keys(sums)){if(facts[key]==null)continue;sums[key]=(sums[key]??0)+facts[key]*factor;}
  const macros=lineMacros(line,pantry);
  qualityLines.push({amount:`${line.amount} ${line.unit}`,kcal:macros.known?macros.kcal:null,nova:item?.nova??line.est?.nova??null});
  if(PLANT.test(line.key))plants++;
 }
 const perServing=Object.fromEntries(Object.entries(sums).map(([k,v])=>[k,v==null?null:v/draft.servings]));
 return {perServing,qualityLines,plants};
}
...
export function workshopTurn({draft,message,goal=null,context=[]}){
 ...
 else if(/zöldség|rost/.test(text)){if(!next.lines.some(l=>l.key==='brokkoli')){next.lines.push(estimateLine('Brokkoli',120,{kcal:41,p:3.4,c:8,f:.4,fiber:3.1,sugar:2,salt:.04,satfat:0,nova:1}));reply='Tettem bele brokkolit a rost miatt. Ez becsült sor — cseréld kamraelemre, ha van otthon.';}}
