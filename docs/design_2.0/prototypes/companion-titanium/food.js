...
const foodHistory={
 ...
 '2026-09-07':{kcal:2115,p:148,c:220,f:69,fiber:24,training:'Teljes test · 68 perc',meals:[{id:'h0907-1',time:'08:20',name:'Zabkása gyümölccsel',slot:'Reggeli',kcal:420,score:8.2},{id:'h0907-2',time:'12:15',name:'Lazacos rizstál',slot:'Ebéd',kcal:735,score:8.7},{id:'h0907-3',time:'16:00',name:'Görög joghurt',slot:'Uzsonna',kcal:240,score:7.8},{id:'h0907-4',time:'19:45',name:'Csirkés tortilla',slot:'Vacsora',kcal:720,score:7.5}]},
};
...
<div class="food-item rv-card rv-block" style="--d:4;--item-color:#8ed2e8"><div class="food-item-head">${icon('bowl')}<span><small>01 · TEJTERMÉK</small><strong>Joghurt</strong></span><span class="estimate-pill">BECSÜLT</span></div><label class="food-label">Milyen joghurt?<select name="yogurt"><option value="yogurt" ${draft.items[0].key==='yogurt'?'selected':''}>Natúr joghurt</option><option value="greek" ${draft.items[0].key==='greek'?'selected':''}>Görög joghurt</option></select></label>${portion('yogurtGrams',draft.items[0].grams,'Joghurt mennyisége grammban','Egy pohár ≈ 150 g')}</div>
<div class="food-item rv-card rv-block" style="--d:5;--item-color:#d9c395"><div class="food-item-head">${icon('sun')}<span><small>02 · GYÜMÖLCS</small><strong>Banán</strong></span><span class="estimate-pill">BECSÜLT</span></div>${portion('bananaGrams',draft.items[1].grams,'Banán mennyisége grammban','Egy közepes darab ≈ 120 g, héj nélkül')}</div>
...
function updatePreview(){const el=$('#food-preview');if(!el)return;readDraft();if(draft.items.some(i=>!Number.isFinite(i.grams)||i.grams<1||i.grams>2000)){el.textContent='Adj meg 1–2000 g közötti mennyiséget.';lastKcal=null;return;}const v=nutrition(draft);
 el.innerHTML=`<strong><em id="rv-kcal-n">${fmt(v.kcal)}</em><small>kcal</small></strong><div>${[['fehérje',v.p,4,'#e08a7c'],['szénhidrát',v.c,4,'#d9c395'],['zsír',v.f,9,'#cdd170']].map(([l,g,k,col])=>`<span><b>${fmt(g)} g</b>${l}<i class="rv-macro-bar"><u style="width:${v.kcal?Math.min(100,Math.round(g*k/v.kcal*100)):0}%;background:${col}"></u></i></span>`).join('')}</div>`;
...
export function fuelOverview(date=FOOD_TODAY){const current=date===FOOD_TODAY,record=foodHistory[date],values=current?{...totals(day),water:1250}:record?{...record,water:record.water??2400}:{kcal:0,p:0,c:0,f:0,fiber:0,water:0};const meals=current?dayMeals():(record?.meals||[]).map(m=>({...m,editable:false}));return{current,record,values,remaining:2400-values.kcal,mealCount:meals.length,meals};}
export function initFood(options){callbacks=options;document.addEventListener('mezo:day-render',syncOverview);document.addEventListener('click',e=>{const el=e.target.closest('button');if(!el)return;
 ...
 if(el.dataset.foodBlock)openFood('','photo',el.dataset.foodBlock);
 if(el.dataset.foodMode){mode=el.dataset.foodMode;render();}
 if(el.dataset.foodShot){if(el.dataset.foodShot==='known'){source='Fotó a tányérról';draft=sampleDraft(blockTime||'15:30');stage='review';react('connect',2000);}else stage='failed';render();}
 if(el.dataset.foodFallback){stage='input';mode=el.dataset.foodFallback;render();}
 if(el.dataset.foodSay){const said=el.dataset.foodSay;if(/joghurt/i.test(said)&&/banán/i.test(said)){source=said;draft=sampleDraft(blockTime||'15:30');stage='review';react('connect',2000);}else{mode='usual';}render();}
 if(el.dataset.foodUsual!==undefined){const row=usualMeals(daypart())[Number(el.dataset.foodUsual)];if(row.sample){source=row.name;draft=sampleDraft(blockTime||'15:30');}else draft=fixedDraft(row.name,row.fixed,blockTime||row.time,row.score);stage='review';render();}
