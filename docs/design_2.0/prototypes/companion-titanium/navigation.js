...
function content(d,p,date){
 ...
 card('AHOGYAN MEGISMERLEK','Alakuló kép rólad.','Állítások, bizonyítékok, pontosítások.','person',detail('Karakter','Ez egy alakítható értelmezés. Minden állításnál: Talál · Nem igaz · Pontosítom.'))+row('heart','Ritmus és belső működés','Szereted előre látni a napod menetét',detail('Ritmus','Demóállítás. A bizonyítékai és a pontosítás helye itt jelenik meg.'))+row('chat','Konzílium','Hogyan állt össze ez a kép?',detail('Konzílium','A szakértői nézőpontok, a kételyek és Mezo összegzése egy olvasható beszélgetésben.')),
 card('AMIT SZÁMON TARTOK','Összekapcsolt emlékek','Tények, történetek és a köztük lévő szálak.','stack',detail('Tudástár','A saját tények, felismert minták és kapcsolatok közös otthona.'))+row('gem','Tények rólad','Megnézheted és javíthatod',detail('Tények','Röplabdázol. Fokozatos erőépítés a célod. Ezek itt mintatételek.'))+row('book','Memória és Memoár','Visszatérő történetek',detail('Emlékek','Korábbi napok és heti történetek. Innen nyílik a memória részletes nézete.'))][p];
 return [
 card('A CÉLJAID','Erő és egyensúly','A súlyod, az alvásod és a teljesítményed együtt.','person',detail('Céljaid','Erőépítés · következetes alvás · saját ritmus. A célkezelés innen érhető el.'))+stats([['81,4 kg','súlytrend'],['7 ó 42 p','alvás'],['3 / hét','gym terv']])+row('ring','Súly és cél','A trend számít',jump('me',1))+row('moon','Alvás és regeneráció','Az éjszakád hatása',jump('me',2))+row('person','Emberek és profil','Kapcsolatok, saját beállítások',detail('Saját világod','Emberek, profil és beállítások másodlagos belépője.')),
 card('HETI TREND','81,4 kg','A napi ingadozás mögött a hosszabb ívet nézzük.','ring',detail('Súlytrend','Mintaadat: a napi mérésekből simított trend.'))+`<svg class="trend" viewBox="0 0 340 100" role="img" aria-label="Minta súlytrend, lassan csökkenő"><path d="M5 25L50 34 95 27 140 55 185 50 230 70 280 64 335 78" fill="none" stroke="var(--domain-color)" stroke-width="3"/><path d="M5 95H335" stroke="#ffffff18"/></svg>`+row('ring','Mai mérés','Adj hozzá új súlyadatot','data-weight')+row('bowl','Étkezési keret','A cél és a sportok hatása',jump('fuel',0))+row('dumbbell','Edzésteljesítmény','Hogyan változik közben az erőd?',jump('train',2)),
 card('AZ ÉJSZAKÁD','7 óra 42 perc','Minőség · 8 / 10','moon','data-open="sleep"')+stats([['23:15','lefekvési cél'],['06:57','ébredés'],['8 / 10','minőség']])+row('moon','Alvás részletei','Időtartam és saját visszajelzés','data-open="sleep"')+row('bolt','Terhelés és pihenés','A sportjaid is a kép részei',jump('train',1))+row('gem','Mit lát Mezo?','Esti étkezés × alvás',jump('mezo',0)),
...
document.addEventListener('submit',e=>{
 ...
 if(form.id==='custom-form'){e.preventDefault();closeSheet();openWorkout();toast(`${data.get('name')} · a demóban a mintaedzés indul`);}
});
