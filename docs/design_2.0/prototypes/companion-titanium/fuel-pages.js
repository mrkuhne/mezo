...
function trendek(){
 ...
 <div class="tx-tiles">${Object.entries(TX_STAT).map(([kind,[label,art,color,unit]])=>{const v=values[kind],delta=isCurrent?deltas[kind]:null;
  return `<button class="tx-tile" style="--kx:${color}" data-tx-stat="${kind}"><span class="tx-tile-art">${icon(art)}</span><strong>${v==null?'—':`<span data-fuel-count="${v}" data-fuel-dec="${kind==='avg'?0:1}">0</span>`}${unit?`<small>${unit}</small>`:''}</strong><span class="tx-tile-label">${label}</span>${delta?`<em>${delta>0?'▲':'▼'} ${fmt1(Math.abs(delta))}${unit?` ${unit}`:''}</em>`:''}</button>`;}).join('')}</div>
