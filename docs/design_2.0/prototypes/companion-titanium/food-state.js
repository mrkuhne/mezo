...
export function nutrition(draft){return draft.items.reduce((out,item)=>{const food=foods[item.key];if(!food)return out;for(const k of ['kcal','p','c','f','fiber'])out[k]+=food[k]*item.grams/100;return out;},{kcal:0,p:0,c:0,f:0,fiber:0});}
...
export function totals(day){return day.meals.reduce((out,m)=>{const n=nutrition(m);for(const k of ['kcal','p','c','f','fiber'])out[k]+=n[k];return out;},{kcal:1180,p:86,c:128,f:36,fiber:18});}
