...
export function ingredientStyle(name){
 ...
 if(/zab|rizs|tortilla|bulgur|kenyér|tészta/.test(n))return ['#d9c395','carb'];
 if(/olaj|sajt|vaj|avok|mogyoró|mandula/.test(n))return ['#cdd170','avocado'];
 if(/méz|cukor/.test(n))return ['#f0a8c8','sugar'];
...
function mealDetailPage(id){
 ...
 const facts=m.raw&&!m.raw.fixed?{lines:m.raw.items.map(i=>{const f=foods[i.key];return [f.name,`${i.grams} g`,'kamra',Math.round(f.kcal*i.grams/100),1];}),plants:mealFacts[m.name]?.plants??1}:mealFacts[m.name]??null;
 const lines=facts?.lines??[],lineKcal=lines.reduce((s,l)=>s+(l[3]||0),0)||1;
 const n=m.nutrients||{},mk=m.macros;
 const block=mealBlocks.find(b=>b.key===blockFor(m.time))??mealBlocks[0];
 const dayShare=Math.round(mk.kcal/2400*100);
 ...
 <div class="meal-hero3" style="--block-color:${block.color}"><span class="mh-glow"></span>
  <div class="mh-left"><span class="mh-art">${icon('bowl')}</span><div class="mh-kcal"><strong data-fuel-count="${mk.kcal}">0</strong><small>kcal</small></div></div>
  <div class="mh-right"><div class="mh-info"><span class="mh-info-art">${icon(block.art)}</span><span><strong>${block.label}</strong><small>${m.time}-kor logoltad</small></span></div><div class="mh-info" style="--block-color:#8ed2e8"><span class="mh-info-art">${icon('ring')}</span><span><strong>a napod ${dayShare}%-a</strong><small>a 2 400 kcal-os keretből</small></span></div></div>
 </div>
 <div class="lf-section"><h2>Makrók</h2></div>
...
export function fuelDashboardContent(domain,page,date){
 ...
 return `${heroSection(values,remaining,current,record,heroVariant)}<section class="fuel-rings" aria-label="Makrók, víz és rost">${ring('Fehérje','meat',values.p,160,'#e08a7c')}${ring('Szénhidrát','carb',values.c,270,'#d9c395')}${ring('Zsír','avocado',values.f,76,'#cdd170')}${ring('Víz','water',(values.water||0)/1000,2.5,'#8ed2e8','l')}${ring('Rost','fiber',values.fiber,30,'#8fd97a')}</section><div class="food-list-heading"><h2>${current?'A mai blokkjaid':'Ezen a napon'}</h2><span class="heading-tools"><span>${overview.mealCount} ÉTKEZÉS</span>${current?`<button class="blocks-settings" data-goto="fuel/0/ablakok" aria-label="Étkezés-ablakok beállítása">${icon('clock')}<b>Ablakok</b></button>`:''}</span></div>${current||record?blocksSection(overview):`<div class="food-day-empty flat">${icon('bowl')}<strong>Nincs étkezés.</strong></div>`}<button class="fuel-log-action" ${current?'data-food':'data-day-today'}><span class="fuel-log-icon">${icon('chat')}</span><span><strong>${current?'Étkezés logolása':'Vissza a mai naphoz'}</strong><small>${current?'Kamera · hang · gépelés · szokásosak':'Logolni mindig a mai naphoz tudsz'}</small></span><b>${current?'＋':'→'}</b></button><details class="fuel-more flat"><summary>További részletek <span>KERET · ABLAKOK · RECEPTEK</span></summary><button class="fuel-secondary" data-goto="fuel/0/ablakok">${icon('clock')}<span><strong>Étkezés-ablakok</strong></span><b>↗</b></button><button class="fuel-secondary" data-food-budget>${icon('ring')}<span><strong>Keretszámítás</strong></span><b>↗</b></button>${record||current?`<button class="fuel-secondary" data-route="train/0">${icon('dumbbell')}<span><strong>${current?'Felsőtest A':record.training}</strong></span><b>↗</b></button>`:''}<button class="fuel-secondary" data-route="fuel/1">${icon('bowl')}<span><strong>Konyha</strong></span><b>↗</b></button></details>`;
}
