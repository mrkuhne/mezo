...
| E4 | Nap-típus: tervezett edzés dönti a keretet, ad-hoc nem | `DayTargetProjector` + `hasScheduledTrainingOn` | KEEP | Ad-hoc session does not flip the day TYPE — de `mezo-rilew` óta a logolt ad-hoc sportolás bekerül a nap mozgás-energiájába (`eat`), tehát a kalóriakeret attól emelkedik |
