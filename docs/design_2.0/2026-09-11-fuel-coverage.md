...
| E4 | Nap-típus: csak a RÖGZÍTETT mozgás emeli a keretet, a terv nem (`mezo-u13jv`, 2026-09-24) | `DayTargetProjector` + `hasLoggedTrainingOn` / FE `deriveLoggedBlocks` | CHANGED | Tervezett, de el nem végzett edzés sem az edzésnapi keretet, sem a mozgás-energiát (`eat`) nem adja; logolt edzés/sport/futás (tervezett vagy ad-hoc) mindkettőt |
