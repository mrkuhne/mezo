 ...
 * provider-native usage block (thoughts/cached) still exists per round — and adds it into the
 * {@link LlmRoundUsage} tally the adapter planted in the request context.
 *
...
final class LlmRoundUsageAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    LlmRoundUsageAdvisor(LlmUsageExtractor llmUsageExtractor) {
    ...
    private void tally(ChatClientRequest request, ChatClientResponse response) {
        if (!(request.context().get(LlmRoundUsage.CONTEXT_KEY) instanceof LlmRoundUsage tally)) {
            return;
        }
        // The extractor's all-zero guard makes a usage-less chunk/round a null — never counted.
        UsageInfo usage = llmUsageExtractor.extract(response == null ? null : response.chatResponse());
