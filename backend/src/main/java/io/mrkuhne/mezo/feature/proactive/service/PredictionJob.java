...
public class PredictionJob {
    ...
    public void runWeekly() {
        ...
        userFanOut.forEachActiveUser("Prediction weekly", user -> {
            try {
                generated.addAndGet(generator.generate(user.getId(), weekStart).size());
            } catch (Exception e) {
                log.warn("Prediction generation failed for user {} week {}", user.getId(), weekStart, e);
            }
        });
        log.info("Prediction weekly run for {}: {} prediction(s) generated", weekStart, generated.get());
    }
    ...
    public void runValidation() {
        ...
        userFanOut.forEachActiveUser("Prediction validation", user -> {
            try {
                closed.addAndGet(validationService.validateClosedWindows(user.getId(), today));
            } catch (Exception e) {
                log.warn("Prediction validation failed for user {} on {}", user.getId(), today, e);
            }
        });
        log.info("Prediction validation run for {}: {} window(s) closed", today, closed.get());
    }
}
