...
alter table challenge add column source_pattern_id uuid;

alter table challenge add constraint fk_challenge_source_pattern_id_pattern_id
    foreign key (source_pattern_id) references pattern (id) on delete set null;

create index idx_challenge_source_pattern_id on challenge (source_pattern_id)
