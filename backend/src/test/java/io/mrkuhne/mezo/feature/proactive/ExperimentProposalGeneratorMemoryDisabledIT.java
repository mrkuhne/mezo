 ...
 * experiment-proposal generator's gather ships with no {@code [Hosszú távú memória]} block and
 * writes no {@code memory_retrieval_run} row. Separate class — same precedent as {@code
...
class ExperimentProposalGeneratorMemoryDisabledIT extends AbstractIntegrationTest {
    ...
    @Test
    void testPropose_shouldShipWithNoMemoryBlockAndWriteNoRun_whenThePolicyIsDisabled() {
        UUID user = userPopulator.createUser("experiment-memory-disabled@test.local").getId();
