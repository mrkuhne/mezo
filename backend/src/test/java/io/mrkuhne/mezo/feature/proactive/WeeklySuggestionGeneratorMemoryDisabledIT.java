 ...
 * suggestion's gather ships with no {@code [Hosszú távú memória]} block and writes no {@code
 * memory_retrieval_run} row. Separate class — the {@code MemoirGeneratorMemoryDisabledIT}/{@code
...
class WeeklySuggestionGeneratorMemoryDisabledIT extends AbstractIntegrationTest {
    ...
    void testGenerate_shouldShipWithNoMemoryBlockAndWriteNoRun_whenThePolicyIsDisabled() {
        UUID user = userPopulator.createUser("weekly-suggestion-memory-disabled@test.local").getId();
