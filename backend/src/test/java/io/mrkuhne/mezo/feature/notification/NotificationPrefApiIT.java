...
class NotificationPrefApiIT extends ApiIntegrationTest {
    ...
    @Test
    void testGetNotificationPrefs_shouldReturnAllTwentyThreeWithSpecDefaults_whenFreshUser() {
        ...
        assertThat(response.getPrefs()).hasSize(23);
        ...
        // + weekly_review, default ON (mezo-p2tr), on top of the retired `weekly`'s own default ON.
        assertThat(response.getPrefs().stream().filter(NotificationPref::getEnabled).count()).isEqualTo(19);
    }
