 ...
 * nightly person extractor ships with no {@code KORÁBBI KAPCSOLÓDÓ EMLÉKEK} section and writes no
...
class PersonExtractionMemoryDisabledIT extends AbstractIntegrationTest {
    ...
    void testExtractFor_shouldShipWithNoMemorySectionAndWriteNoRun_whenThePolicyIsDisabled() {
        UUID owner = userPopulator.createUser("person-memory-disabled@test.local").getId();
        journalPopulator.createEntry(owner, DAY,
                "délben futottam Marcival a gáton, este megint Marci hívott. "
        ...
        MemoryItemEntity item = memoryPopulator.item(owner, "journal_entry", UUID.randomUUID(),
                "Napló", "Tavaly is sokat futottak együtt Marcival.", DAY.minusDays(3),
