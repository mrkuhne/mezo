...
/**
 * {@code compare_periods} render (the CompanionToolsRenderIT idiom): direct call with a
 * hand-built ToolContext, refs asserted on the audit. {@code find_similar_past_days}'s own
 * render tests moved to {@code MemoryToolsSimilarDaysIT} (mezo-eq85.10) — that call now goes
 * through the memory platform, whose retriever fan-out must NOT run inside this class's
 * {@code @Transactional} test wrapper (see the S7 hazard note on thread-pool exhaustion).
 */
@Transactional
